"""Tests for `mapmmd reflect` and the work-memory reflection layer.

`mapmmd reflect` reads the outcome-tagged Q&A docs that `mapmmd save-result`
files into mapmmd-out/memory/ and writes a deterministic lessons artifact
(mapmmd-out/reflections/LESSONS.md) an agent can load next session: preferred
...
def _write_raw_doc(mem: Path, filename: str, date: str, *, outcome="dead_end",
    ...
    lines = ["---", 'type: "query"', f'date: "{date}"', f'question: "{question}"',
             'contributor: "mapmmd"', f'outcome: "{outcome}"']
    if nodes:
...
def test_cli_reflect_end_to_end(tmp_path):
    ...
    lessons = cwd / "mapmmd-out" / "reflections" / "LESSONS.md"
...
def test_cli_reflect_cold_start_writes_empty_lessons(tmp_path):
    """First run with no mapmmd-out/memory/ still succeeds and writes a valid doc."""
    r = _run(["reflect"], tmp_path)
    assert r.returncode == 0, r.stderr
    assert "Reflected 0 memories" in r.stdout
    lessons = tmp_path / "mapmmd-out" / "reflections" / "LESSONS.md"
    assert lessons.exists()
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels in tmp_path/mapmmd-out/.
    ...
    (out / ".mapmmd_analysis.json").write_text(json.dumps({
