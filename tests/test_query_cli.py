"""Tests for map_mmd query CLI context filtering."""
...
def test_query_cli_explicit_context_filter(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["map_mmd", "query", "extract", "--context", "call", "--graph", str(graph_path)],
    )
    mainmod.main()
...
def test_query_cli_heuristic_context_filter(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["map_mmd", "query", "who calls extract", "--graph", str(graph_path)],
    )
    mainmod.main()
    out = capsys.readouterr().out
...
def test_query_cli_rejects_oversized_graph(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["map_mmd", "query", "extract", "--graph", str(graph_path)],
    )
