...
a source/doc file outside mapmmd-out/ when a graph exists, and otherwise stays
silent and fails open.
"""
import json
import subprocess

from mapmmd.__main__ import _READ_SETTINGS_HOOK
...
def _run(tool_input, cwd, *, graph: bool):
    if graph:
        (cwd / "mapmmd-out").mkdir(parents=True, exist_ok=True)
        (cwd / "mapmmd-out" / "graph.json").write_text("{}", encoding="utf-8")
...
def test_nudge_payload_is_valid_pretooluse_json(tmp_path):
    ...
    assert "mapmmd query" in payload["hookSpecificOutput"]["additionalContext"]


def test_silent_on_mapmmd_out_targets(tmp_path):
    """Reading the graph's own report must not start a go-read-the-graph loop."""
    out = _run({"file_path": "mapmmd-out/GRAPH_REPORT.md"}, tmp_path, graph=True).stdout
    assert out.strip() == ""
...
def test_nudges_on_framework_source(tmp_path):
    ...
    for path in ("src/components/Hero.astro", "src/App.vue", "src/Card.svelte"):
        out = _run({"file_path": path}, tmp_path, graph=True).stdout
        assert "mapmmd query" in out, f"{path} should nudge"
...
def test_fails_open_on_malformed_stdin(tmp_path):
    (tmp_path / "mapmmd-out").mkdir()
    (tmp_path / "mapmmd-out" / "graph.json").write_text("{}", encoding="utf-8")
    r = subprocess.run(
