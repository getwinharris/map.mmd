...
`mapmmd install --platform agents` (and the friendly `--platform skills`
...
The bare ``mapmmd install`` behaviour (claude/windows only) is unchanged; the
named platform is opt-in. The ``mapmmd agents install`` subcommand is the
...
def test_agents_user_destination_is_user_global_dot_agents(tmp_path):
    ...
    assert dst == tmp_path / ".agents" / "skills" / "mapmmd" / "SKILL.md"
...
def _run(tmp_path, argv, home):
    ...
    try:
        os.chdir(tmp_path)
        with patch.object(sys, "argv", ["mapmmd", *argv]):
...
def test_install_platform_agents_writes_user_global_skill_only(tmp_path, platform_arg):
    """`mapmmd install --platform agents|skills` writes ~/.agents/skills/...
    ...
    skill = home / ".agents" / "skills" / "mapmmd" / "SKILL.md"
    assert skill.exists()
    assert (skill.parent / ".mapmmd_version").read_text() == mainmod.__version__
...
def test_uninstall_platform_agents_removes_user_global_skill(tmp_path):
    """Bare `mapmmd uninstall` clears the ~/.agents/skills skill the AGENTS.md and
...
def test_uninstall_platform_flag_global_removes_skill(tmp_path, platform_arg):
    """`mapmmd uninstall --platform agents|skills` (global) clears ~/.agents/skills.
...
def test_project_uninstall_all_removes_agents_skill(tmp_path):
    """`mapmmd uninstall --project` (no platform) removes the agents project skill
    ...
    project_skill = proj / ".agents" / "skills" / "mapmmd" / "SKILL.md"
...
def test_install_platform_agents_project_writes_dot_agents(tmp_path):
    """`mapmmd install --project --platform agents` writes ./.agents/skills and
...
# --- the amp-twin subcommand (mapmmd agents install) -------------------------


def test_agents_subcommand_install_also_wires_agents_md(tmp_path):
    """`mapmmd agents install` is the amp-twin: skill at ~/.agents/skills PLUS a
    `## mapmmd` section in AGENTS.md (so the rendered hooks reference, which
    points at `mapmmd agents install`, stays honest)."""
    home = tmp_path / "home"
    cwd = tmp_path / "cwd"
    home.mkdir()
    cwd.mkdir()
    ...
    assert "## mapmmd" in agents_md.read_text(encoding="utf-8")
    ...
    assert not agents_md.exists() or "## mapmmd" not in agents_md.read_text(encoding="utf-8")


def test_agents_subcommand_install_is_idempotent(tmp_path):
    """Running `mapmmd agents install` twice leaves a single AGENTS.md section."""
    home = tmp_path / "home"
    cwd = tmp_path / "cwd"
    home.mkdir()
    cwd.mkdir()
    ...
    assert body.count("## mapmmd") == 1, "AGENTS.md gained a duplicate mapmmd section"


def test_skills_subcommand_is_the_agents_subcommand(tmp_path):
    """`mapmmd skills install`/`uninstall` behaves exactly like the agents form:
...
def test_bare_install_does_not_touch_dot_agents(tmp_path):
    """`mapmmd install` (no platform) stays single-platform claude/windows and
