...
def test_log_query_writes_jsonl(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MAPMMD_QUERY_LOG", str(log_file))
    monkeypatch.delenv("MAPMMD_QUERY_LOG_DISABLE", raising=False)
...
def test_disable_env(tmp_path, monkeypatch):
    log_file = tmp_path / "q.log"
    monkeypatch.setenv("MAPMMD_QUERY_LOG", str(log_file))
    monkeypatch.setenv("MAPMMD_QUERY_LOG_DISABLE", "1")
...
def test_disable_env_true(tmp_path, monkeypatch):
    log_file = tmp_path / "q.log"
    monkeypatch.setenv("MAPMMD_QUERY_LOG", str(log_file))
    monkeypatch.setenv("MAPMMD_QUERY_LOG_DISABLE", "true")
...
def test_responses_not_logged_by_default(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MAPMMD_QUERY_LOG", str(log_file))
    monkeypatch.delenv("MAPMMD_QUERY_LOG_DISABLE", raising=False)
    monkeypatch.delenv("MAPMMD_QUERY_LOG_RESPONSES", raising=False)
...
def test_responses_optin(tmp_path, monkeypatch):
    log_file = tmp_path / "q.log"
    monkeypatch.setenv("MAPMMD_QUERY_LOG", str(log_file))
    monkeypatch.setenv("MAPMMD_QUERY_LOG_RESPONSES", "1")
    monkeypatch.delenv("MAPMMD_QUERY_LOG_DISABLE", raising=False)
...
def test_log_never_raises(tmp_path, monkeypatch):
    ...
    bad_path.mkdir()
    monkeypatch.setenv("MAPMMD_QUERY_LOG", str(bad_path))
    monkeypatch.delenv("MAPMMD_QUERY_LOG_DISABLE", raising=False)
