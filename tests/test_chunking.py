"""Tests for token-aware chunking and parallel chunk execution in mapmmd.llm."""
import time
...
def test_pack_chunks_packs_small_files_together(tmp_path):
    ...
    from mapmmd.llm import _pack_chunks_by_tokens
...
def test_corpus_parallel_runs_chunks_concurrently(tmp_path):
    ...
    from mapmmd.llm import extract_corpus_parallel
    ...
    with patch("mapmmd.llm.extract_files_direct", side_effect=slow_extract):
        t0 = time.time()
...
def test_corpus_parallel_sequential_when_max_concurrency_is_one(tmp_path):
    ...
    with patch("mapmmd.llm.extract_files_direct", side_effect=record):
...
def test_corpus_parallel_continues_after_chunk_failure(tmp_path, capsys):
    ...
    with patch("mapmmd.llm.extract_files_direct", side_effect=maybe_fail):
        result = extract_corpus_parallel(
            files, backend="kimi", token_budget=None, chunk_size=1, max_concurrency=1
        )
...
def test_adaptive_retry_returns_directly_when_not_truncated(tmp_path):
    ...
    from mapmmd.llm import _extract_with_adaptive_retry
    ...
    with patch("mapmmd.llm.extract_files_direct", side_effect=stub):
...
def test_adaptive_retry_caps_at_max_depth(tmp_path, capsys):
    ...
    with patch("mapmmd.llm.extract_files_direct", side_effect=always_truncate):
