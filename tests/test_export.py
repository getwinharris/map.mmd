...
from map_mmd.export import to_json, to_cypher, to_graphml, to_html, to_canvas, to_obsidian
...
def test_backup_no_graph_json(tmp_path):
    ...
    from map_mmd.export import backup_if_protected
...
def test_backup_semantic_marker(tmp_path):
    """graph.json + .map_mmd_semantic_marker → backup taken."""
    from map_mmd.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / "GRAPH_REPORT.md").write_text("# Report")
    (tmp_path / ".map_mmd_semantic_marker").write_text('{"output_tokens": 1234}')
    result = backup_if_protected(tmp_path)
    assert result is not None
    assert result.is_dir()
    assert (result / "graph.json").exists()
    assert (result / "GRAPH_REPORT.md").exists()
    assert (result / ".map_mmd_semantic_marker").exists()


def test_backup_curated_labels(tmp_path):
    """graph.json + non-default label in .map_mmd_labels.json → backup taken."""
    import json
    from map_mmd.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".map_mmd_labels.json").write_text(json.dumps({"0": "Auth Pipeline", "1": "Community 1"}))
    result = backup_if_protected(tmp_path)
    assert result is not None
...
def test_backup_default_labels_only(tmp_path):
    ...
    from map_mmd.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".map_mmd_labels.json").write_text(json.dumps({"0": "Community 0", "1": "Community 1"}))
    assert backup_if_protected(tmp_path) is None
...
def test_backup_same_day_no_accumulation(tmp_path):
    ...
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".map_mmd_semantic_marker").write_text("{}")
...
def test_backup_env_disable(tmp_path, monkeypatch):
    """MAP_MMD_NO_BACKUP=1 disables backup entirely."""
    from map_mmd.export import backup_if_protected
    monkeypatch.setenv("MAP_MMD_NO_BACKUP", "1")
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".map_mmd_semantic_marker").write_text("{}")
    assert backup_if_protected(tmp_path) is None
