"""Tests for mapmmd/security.py - URL validation, safe fetch, path guards, label sanitisation."""
...
def test_safe_fetch_returns_bytes(tmp_path):
    ...
    with patch("mapmmd.security._build_opener") as mock_opener_fn:
...
def test_validate_graph_path_default_base_discovers_output_dir(tmp_path):
    ...
    parents for the configured output-dir name (default 'mapmmd-out')."""
    base = tmp_path / "mapmmd-out"
    base.mkdir()
    graph = base / "graph.json"
    graph.write_text("{}")
...
def test_validate_graph_path_default_base_honours_mapmmd_out_override(tmp_path, monkeypatch):
    ...
    graph.write_text("{}")
    # No base passed → must discover custom-out by name rather than mapmmd-out.
    assert validate_graph_path(str(graph)) == graph.resolve()
...
def test_graph_size_cap_at_boundary_passes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mapmmd.security._MAX_GRAPH_FILE_BYTES", 32)
    assert check_graph_file_size_cap(p) is None
    monkeypatch.setattr("mapmmd.security._MAX_GRAPH_FILE_BYTES", 31)
    with pytest.raises(ValueError):
        check_graph_file_size_cap(p)
