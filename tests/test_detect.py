...
from map_mmd.detect import classify_file, count_words, detect, detect_incremental, save_manifest, FileType, _looks_like_paper, _is_ignored, _load_map_mmdignore, _is_sensitive
...
def test_detect_skips_noise_dot_dirs():
    """Noise dot dirs (.next, .nuxt, .map_mmd cache, …) are skipped (#873).
    Non-noise dot dirs (.github, .claude, …) are now allowed through."""
    result = detect(FIXTURES)
    for files in result["files"].values():
        for f in files:
            # map_mmd's own cache is always skipped
            assert "/.map_mmd/" not in f
...
def test_classify_attention_paper():
    ...
    paper_path = Path("/home/safi/map_mmd_eval/papers/attention_is_all_you_need.md")
    if paper_path.exists():
        result = classify_file(paper_path)
        assert result == FileType.PAPER


def test_map_mmdignore_excludes_file(tmp_path):
    """Files matching .map_mmdignore patterns are excluded from detect()."""
    (tmp_path / ".map_mmdignore").write_text("vendor/\n*.generated.py\n")
    ...
    assert result["map_mmdignore_patterns"] == 2


def test_map_mmdignore_missing_is_fine(tmp_path):
    """No .map_mmdignore is not an error."""
    (tmp_path / "main.py").write_text("x = 1")
    result = detect(tmp_path)
    assert result["map_mmdignore_patterns"] == 0


def test_map_mmdignore_comments_ignored(tmp_path):
    """Comment lines in .map_mmdignore are not treated as patterns."""
    (tmp_path / ".map_mmdignore").write_text("# this is a comment\n\nmain.py\n")
    (tmp_path / "main.py").write_text("x = 1")
...
def test_map_mmdignore_hermetic_without_vcs(tmp_path):
    """Without a VCS root, parent .map_mmdignore does NOT apply (hermetic)."""
    (tmp_path / ".map_mmdignore").write_text("vendor/\n")
    ...
    # parent .map_mmdignore must NOT leak into a non-VCS scan
    assert any("vendor" in f for f in code_files)
    assert result["map_mmdignore_patterns"] == 0


def test_map_mmdignore_discovered_from_parent_in_vcs(tmp_path):
    """Inside a VCS repo, parent .map_mmdignore applies to subdirectory scans."""
    (tmp_path / ".git").mkdir()
    (tmp_path / ".map_mmdignore").write_text("vendor/\n")
    sub = tmp_path / "packages" / "mylib"
    sub.mkdir(parents=True)
    ...
    assert result["map_mmdignore_patterns"] >= 1


def test_map_mmdignore_stops_at_git_boundary(tmp_path):
    """Upward search stops at the git repo root (.git directory)."""
    (tmp_path / ".map_mmdignore").write_text("main.py\n")
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    sub = repo / "sub"
...
def test_map_mmdignore_at_git_root_is_included(tmp_path):
    """A .map_mmdignore at the git repo root is included when scanning a subdir."""
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    (repo / ".map_mmdignore").write_text("vendor/\n")
...
def test_detect_incremental_propagates_follow_symlinks(tmp_path, monkeypatch):
    ...
    # Store manifest inside map.mmd-out/ so it is pruned by _SKIP_DIRS
    # and doesn't get re-detected as a code file now that .json is indexed.
    manifest_dir = tmp_path / "map.mmd-out"
    manifest_dir.mkdir()
...
def test_classify_video_extensions():
    ...
    from map_mmd.detect import FileType
...
def test_detect_converts_google_workspace_shortcuts_when_enabled(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("map_mmd.detect.convert_google_workspace_file", fake_convert)
...
def test_detect_skips_map_mmd_own_cache(tmp_path):
    """.map_mmd/ (extraction cache) must never be re-indexed as source (#873)."""
    cache = tmp_path / ".map_mmd" / "cache"
    cache.mkdir(parents=True)
    (cache / "abc123.json").write_text('{"nodes": [], "edges": []}')
    (tmp_path / "app.py").write_text("def go(): pass")
    result = detect(tmp_path)
    all_files = [f for files in result["files"].values() for f in files]
    assert not any(".map_mmd" in f for f in all_files)
    assert any("app.py" in f for f in all_files)
...
def test_negation_cannot_rescue_file_under_excluded_dir(tmp_path):
    ...
    from map_mmd.detect import _is_ignored, _load_map_mmdignore
    android = tmp_path / "android" / "app" / "src"
    android.mkdir(parents=True)
    victim = android / "Main.kt"
    victim.write_text("fun main() {}")
    (tmp_path / ".map_mmdignore").write_text("android/\n!src/\n")
    patterns = _load_map_mmdignore(tmp_path)
...
def test_negation_works_when_no_ancestor_excluded(tmp_path):
    ...
    keep = src / "keep.py"
    keep.write_text("x = 1")
    (tmp_path / ".map_mmdignore").write_text("*.py\n!src/keep.py\n")
    patterns = _load_map_mmdignore(tmp_path)
...
def test_negation_does_not_disable_directory_pruning(tmp_path, monkeypatch):
    ...
    """
    import os
    import map_mmd.detect as det

    (tmp_path / ".map_mmdignore").write_text("myignored/\n*.md\n!docs/**\n")
...
def test_anchored_dir_not_matched_at_depth(tmp_path):
    ...
    (tmp_path / ".map_mmdignore").write_text("/inbox/\n")
    patterns = _load_map_mmdignore(tmp_path)
    assert not _is_ignored(f, tmp_path, patterns), (
...
def test_anchored_file_not_matched_at_depth(tmp_path):
    ...
    (tmp_path / ".map_mmdignore").write_text("/build\n")
    patterns = _load_map_mmdignore(tmp_path)
...
def test_anchored_multi_segment_pattern(tmp_path):
    ...
    (tmp_path / ".map_mmdignore").write_text("/src/inbox/\n")
    patterns = _load_map_mmdignore(tmp_path)
...
def test_is_ignored_cache_matches_uncached_results(tmp_path):
    ...
    for p in paths:
        if p.suffix:
            p.write_text("x")
    (tmp_path / ".map_mmdignore").write_text(
        "build/\n*.log\n!logs/keep.log\n"
    )
    patterns = _load_map_mmdignore(tmp_path)
...
def test_is_ignored_cache_evaluates_each_dir_once():
    ...
    """
    from map_mmd.detect import _is_ignored
...
def test_save_manifest_skips_semantic_hash_for_files_without_cache(tmp_path):
    ...
    import json
    from map_mmd.cache import save_cached
...
# Regression tests for #945 - .gitignore fallback when no .map_mmdignore exists

def test_gitignore_fallback_when_no_map_mmdignore(tmp_path):
    """When no .map_mmdignore exists, .gitignore patterns are honored (#945)."""
    (tmp_path / ".git").mkdir()
...
def test_map_mmdignore_and_gitignore_are_merged(tmp_path):
    """When both exist, their patterns are MERGED — a file excluded only by
    .gitignore stays excluded even though .map_mmdignore says nothing about it
    (#1363). Previously the presence of a .map_mmdignore silently disabled the
    ...
    (tmp_path / ".map_mmdignore").write_text("other.py\n")  # says nothing about main.py
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "other.py").write_text("x = 2")
    ...
    assert not any("other.py" in f for f in code)  # map_mmdignore applied
    assert any("keep.py" in f for f in code)       # neither excludes it


def test_map_mmdignore_negation_overrides_gitignore(tmp_path):
    """.map_mmdignore is evaluated after .gitignore, so a `!` negation in it can
    re-include a file the .gitignore excluded (last-match-wins, #1363)."""
    (tmp_path / ".git").mkdir()
    (tmp_path / ".gitignore").write_text("*.py\n")           # exclude all .py
    (tmp_path / ".map_mmdignore").write_text("!keep.py\n")  # but rescue keep.py
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "keep.py").write_text("x = 2")

    result = detect(tmp_path)
    code = result["files"]["code"]
    assert any("keep.py" in f for f in code)      # rescued by map_mmdignore negation
...
def test_shebang_interpreter_plain(tmp_path):
    ...
    from map_mmd.detect import _shebang_interpreter
...
# relative keys so a committed ``map.mmd-out/`` round-trips across machines
...
def test_save_manifest_relativizes_keys_when_root_given(tmp_path):
    ...
    import json
    from map_mmd.detect import save_manifest, load_manifest

    (tmp_path / "src").mkdir()
    (tmp_path / "src" / "foo.py").write_text("def x(): pass\n")
    (tmp_path / "doc.md").write_text("hello\n")

    manifest_path = str(tmp_path / "map.mmd-out" / "manifest.json")
    files = {
...
def test_save_manifest_without_root_keeps_absolute_keys(tmp_path):
    ...
    import json
    from map_mmd.detect import save_manifest
...
def test_load_manifest_absolutizes_relative_keys(tmp_path):
    ...
    import json
    from map_mmd.detect import load_manifest

    manifest_path = tmp_path / "map.mmd-out" / "manifest.json"
...
def test_detect_incremental_portable_across_paths(tmp_path):
    ...
    import json
    from map_mmd.detect import save_manifest, detect_incremental
    ...
    manifest_a = str(repo_a / "map.mmd-out" / "manifest.json")
    files = {
    ...
    (repo_b / "map.mmd-out").mkdir()
    manifest_b = repo_b / "map.mmd-out" / "manifest.json"
