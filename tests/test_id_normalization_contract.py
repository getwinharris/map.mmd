...
Both callers now delegate to :mod:`map_mmd.ids`, so they share one
...
from map_mmd.build import _normalize_id
from map_mmd.extract import _make_id
from map_mmd.ids import make_id, normalize_id
...
def test_both_callers_share_one_implementation():
    ...
    underlying function object as map_mmd.ids.normalize_id."""
    # build._normalize_id is imported directly from map_mmd.ids.
    ...
    from map_mmd.mcp_ingest import _make_id as _mcp_make_id
    from map_mmd.symbol_resolution import _bash_make_id
