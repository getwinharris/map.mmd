...
the SKILL.md but never wrote `.agents/rules/mapmmd.md` or
`.agents/workflows/mapmmd.md` - even though the uninstall path removes them.
"""
import mapmmd.__main__ as m


def test_antigravity_project_install_writes_rules_and_workflows(tmp_path):
    m._project_install("antigravity", tmp_path)
    skill = tmp_path / ".agents" / "skills" / "mapmmd" / "SKILL.md"
    rules = tmp_path / ".agents" / "rules" / "mapmmd.md"
    workflow = tmp_path / ".agents" / "workflows" / "mapmmd.md"
...
def test_antigravity_project_uninstall_clears_rules_and_workflows(tmp_path):
    ...
    assert not (tmp_path / ".agents" / "rules" / "mapmmd.md").exists()
    assert not (tmp_path / ".agents" / "workflows" / "mapmmd.md").exists()
    assert not (tmp_path / ".agents" / "skills" / "mapmmd" / "SKILL.md").exists()
