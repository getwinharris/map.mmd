...
Pre-fix, the installers wrote a "## mapmmd" section with report-first
instructions and skipped writing if the marker was already present. So users
who installed mapmmd and then upgraded to the fixed package still had the
...
# wrote a variant of "ALWAYS read mapmmd-out/GRAPH_REPORT.md before ...".
_OLD_CLAUDE_SECTION = """\
## mapmmd
...
Rules:
- ALWAYS read mapmmd-out/GRAPH_REPORT.md before reading any source files, running grep/glob searches, or answering codebase questions. The graph is your primary map of the codebase.
- IF mapmmd-out/wiki/index.md EXISTS, navigate it instead of reading raw files
- For cross-module "how does X relate to Y" questions, prefer `mapmmd query "<question>"`, `mapmmd path "<A>" "<B>"`, or `mapmmd explain "<concept>"` over grep — these traverse the graph's EXTRACTED + INFERRED edges instead of scanning files
...
code, your **first tool call must be** to read `mapmmd-out/GRAPH_REPORT.md` (if it exists).
...
- Before answering architecture or codebase questions, read mapmmd-out/GRAPH_REPORT.md for god nodes and community structure
- If mapmmd-out/wiki/index.md exists, navigate it instead of reading raw files
"""
...
mapmmd: A knowledge graph of this project lives in `mapmmd-out/`. \
If `mapmmd-out/GRAPH_REPORT.md` exists, read it before answering architecture questions, \
...
_OLD_HOOK_PAYLOAD_SNIPPET = "Read mapmmd-out/GRAPH_REPORT.md for god nodes and community structure before searching raw files"


def _assert_no_report_first(text: str, ctx: str) -> None:
    assert "ALWAYS read mapmmd-out/GRAPH_REPORT.md" not in text, (
...
def _assert_query_first(text: str, ctx: str) -> None:
    assert "mapmmd query" in text, (
        f"{ctx}: new 'mapmmd query' guidance missing after upgrade"
    )
...
def test_claude_install_upgrades_stale_section(tmp_path, monkeypatch):
    ...
    `mapmmd claude install` again after upgrading to a fixed package."""
    ...
    # Pre-existing non-mapmmd content must be preserved
...
def test_claude_install_upgrades_stale_hook_payload(tmp_path, monkeypatch):
    ...
    assert "mapmmd query" in new_settings_text, (
        "new hook payload should route to `mapmmd query`"
    )
...
def test_cursor_install_upgrades_stale_rule(tmp_path, monkeypatch):
    """Same upgrade behavior for .cursor/rules/mapmmd.mdc.
    The Cursor rule file is wholly mapmmd-owned; overwrite on upgrade."""
    monkeypatch.chdir(tmp_path)
    rule_path = tmp_path / ".cursor" / "rules" / "mapmmd.mdc"
    ...
    assert "read mapmmd-out/GRAPH_REPORT.md for god nodes and community structure" not in after
    _assert_query_first(after, ".cursor/rules/mapmmd.mdc")
...
def test_kiro_install_upgrades_stale_steering(tmp_path, monkeypatch):
    """Same upgrade behavior for .kiro/steering/mapmmd.md (wholly owned)."""
    monkeypatch.chdir(tmp_path)
    steering = tmp_path / ".kiro" / "steering" / "mapmmd.md"
    ...
    _assert_query_first(after, ".kiro/steering/mapmmd.md")
...
def test_kiro_install_ships_references_sidecar_and_version_stamp(tmp_path, monkeypatch):
    ...
    and .mapmmd_version stamp are written alongside SKILL.md (#1142).
    ...
    skill_dir = tmp_path / ".kiro" / "skills" / "mapmmd"
    ...
    # .mapmmd_version stamp written
    version_file = skill_dir / ".mapmmd_version"
    assert version_file.exists(), ".mapmmd_version stamp must be written (#1142)"
    ...
    assert (tmp_path / ".kiro" / "steering" / "mapmmd.md").exists()
    ...
    assert not (tmp_path / ".kiro" / "steering" / "mapmmd.md").exists()
