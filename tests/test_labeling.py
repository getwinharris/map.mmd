...
from mapmmd.llm import label_communities, generate_community_labels
...
def test_label_communities_happy_path(monkeypatch):
    ...
    monkeypatch.setattr("mapmmd.llm._call_llm", fake_call)
...
def test_label_cli_passes_model_override(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mapmmd.llm.generate_community_labels", fake_generate)
    monkeypatch.setattr("mapmmd.export.to_html", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_label_communities_partial_reply_fills_placeholder(monkeypatch):
    G, communities = _graph()
    monkeypatch.setattr("mapmmd.llm._call_llm",
...
def test_label_communities_all_batches_fail_raises(monkeypatch):
    ...
    monkeypatch.setattr("mapmmd.llm._call_llm", always_fail)
