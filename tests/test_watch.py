...
from mapmmd.watch import _notify_only, _WATCHED_EXTENSIONS, _rebuild_lock, _check_shrink
...
def test_notify_only_creates_flag(tmp_path):
    ...
    flag = tmp_path / "mapmmd-out" / "needs_update"
    assert flag.exists()
    assert flag.read_text() == "1"

def test_notify_only_creates_flag_dir(tmp_path):
    # mapmmd-out dir does not exist yet
    assert not (tmp_path / "mapmmd-out").exists()
    _notify_only(tmp_path)
    assert (tmp_path / "mapmmd-out").is_dir()
...
def test_watch_raises_without_watchdog(tmp_path, monkeypatch):
    ...
    from mapmmd.watch import watch
...
def test_mapmmd_root_preserves_relative_when_invoked_with_relative_path(tmp_path, monkeypatch):
    """#777: ``.mapmmd_root`` stores the user-supplied path (``.``), not the
    resolved absolute, so a committed ``mapmmd-out/.mapmmd_root`` is
    ...
    saved = (corpus / "mapmmd-out" / ".mapmmd_root").read_text(encoding="utf-8")
    assert saved == ".", (
        f".mapmmd_root must preserve the user-supplied path; got {saved!r}"
    )


def test_mapmmd_root_preserves_absolute_when_user_supplied(tmp_path):
    """When the caller supplies an absolute path, ``.mapmmd_root`` stores
...
def test_rebuild_code_evicts_nodes_from_deleted_files(tmp_path):
    """#1007: mapmmd update (_rebuild_code with no changed_paths) must remove
    ...
    assert _rebuild_code(corpus, acquire_lock=False) is True
    graph_path = corpus / "mapmmd-out" / "graph.json"
...
def test_rebuild_code_evicts_removed_symbol_from_surviving_file(tmp_path):
    """#1116: mapmmd update (_rebuild_code with no changed_paths) must prune a
    ...
    # shrink, so the shrink-guard must let `mapmmd update` refresh the graph
...
def test_rebuild_code_preupgrade_marker_less_node_one_cycle_lag(tmp_path):
    ...
    markers. On the first `mapmmd update` after upgrading, a symbol removed
...
def test_rebuild_code_is_idempotent_when_cluster_ids_flap(tmp_path, monkeypatch):
    from mapmmd import cluster as cluster_mod
    from mapmmd.watch import _rebuild_code
    ...
    report_path = tmp_path / "mapmmd-out" / "GRAPH_REPORT.md"
...
# --- .mapmmdignore honored in watch handler (gh-928) ---
...
def test_watch_handler_honors_mapmmdignore(tmp_path, monkeypatch):
    """gh-928: the watch Handler must short-circuit paths matching
    .mapmmdignore so busy volumes (node_modules churn, build artefacts,
    Time Machine writes, …) don't wake the rebuild pipeline.
    """
    import threading
    from mapmmd import watch as watch_mod

    watch_root = tmp_path / ".hidden-parent" / "corpus"
    watch_root.mkdir(parents=True)
    (watch_root / ".mapmmdignore").write_text("node_modules/\nbuild/\n", encoding="utf-8")
...
@pytest.mark.skipif(not _watchdog_available(), reason="watchdog not installed")
def test_watch_loads_mapmmdignore_once(tmp_path, monkeypatch):
    """gh-928: .mapmmdignore must be parsed exactly once at watch() startup,
    ...
    (tmp_path / ".mapmmdignore").write_text("ignored/\n", encoding="utf-8")
    (tmp_path / "ignored").mkdir()

    calls = {"n": 0}
    real_loader = detect_mod._load_mapmmdignore
    ...
    monkeypatch.setattr(watch_mod, "_load_mapmmdignore", counting_loader)
    ...
    time.sleep(0.7)
    assert calls["n"] == 1, f"_load_mapmmdignore called {calls['n']} times; expected 1"
...
def test_rebuild_code_accepts_repo_relative_changed_path_for_subdir_root(tmp_path):
    ...
    try:
        ...
        graph_path = src / "mapmmd-out" / "graph.json"
        before = json.loads(graph_path.read_text(encoding="utf-8"))
...
def test_queue_and_drain_pending_round_trip(tmp_path):
    ...
    from mapmmd.watch import _queue_pending, _drain_pending, _PENDING_FILENAME
...
def test_drain_pending_dedupes_and_skips_blank_lines(tmp_path):
    ...
    from mapmmd.watch import _queue_pending, _drain_pending
...
def test_queue_pending_noop_on_empty_list(tmp_path):
    ...
    from mapmmd.watch import _queue_pending, _PENDING_FILENAME
...
def test_rebuild_code_queues_on_lock_contention(tmp_path, monkeypatch, capsys):
    ...
    from mapmmd.watch import _rebuild_code, _rebuild_lock, _PENDING_FILENAME
...
def test_rebuild_code_drains_late_arrivals(tmp_path, monkeypatch):
    ...
    from mapmmd import watch as watch_mod
    from mapmmd.watch import _rebuild_code as orig_rebuild
...
def test_merge_changed_paths_dedupes_in_order():
    ...
    from mapmmd.watch import _merge_changed_paths
