...
skillgen renders map_mmd's committed skill artifacts from human-edited
...
def test_no_version_or_timestamp_in_output():
    ...
    from map_mmd.__main__ import __version__
...
def _claude_artifacts():
    ...
    core = next(a for a in arts if a.path == "map_mmd/skill.md")
    refs = {a.path.rsplit("/", 1)[-1]: a.content for a in arts if a.path != "map_mmd/skill.md"}
...
def test_lean_core_has_no_reference_only_content():
    ...
    assert "from map_mmd.build import build_merge" not in core
    assert "map_mmd cluster-only ." not in core
    ...
    assert "map_mmd export wiki" not in core
    assert "map_mmd export neo4j" not in core
    # The add / watch / hook flows live only in their references.
    assert "from map_mmd.ingest import ingest" not in core
    assert "map_mmd hook install" not in core
    assert "python3 -m map_mmd.watch" not in core
...
def test_lean_core_runs_default_pipeline_with_zero_references():
    ...
    for needed in (
        "### Step 1 - Ensure map_mmd is installed",
        "### Step 2 - Detect files",
...
def test_extraction_states_no_api_key_required_for_every_host():
    ...
    for a in bodies:
        assert "map_mmd needs no API key" in a.content, a.path
        ...
        if tip in a.content:
            assert a.content.index("map_mmd needs no API key") < a.content.index(tip), \
...
def test_query_heading_is_homed_in_core_stub_only():
    ...
    assert "## For /map_mmd query" in core_headings
    assert "## For /map_mmd query" not in query_headings
    # The deeper query content moved into the reference.
    assert "## For /map_mmd path" in query_headings
    assert "## For /map_mmd explain" in query_headings
    assert "## For /map_mmd path" not in core_headings
...
UNIFIED_DESCRIPTION = (
    ...
    "or project content — especially when map.mmd-out/ exists, where the question "
    "should be treated as a map_mmd query first. Turns any input (code, docs, "
...
def test_every_platform_query_has_expansion_and_fallback():
    ...
    for key in ("claude", "codex", "windows", "opencode"):
        ...
        assert "## For /map_mmd path" in q
        assert "## For /map_mmd explain" in q
...
def test_monoliths_render_inline_single_file_no_references():
    ...
    for key in ("aider", "devin"):
        ...
        assert arts[0].path == f"map_mmd/skill-{key}.md"
...
def test_monoliths_carry_the_1392_runbook_fixes():
    ...
    for key in ("aider", "devin"):
        ...
        assert ".map_mmd_cached.json').unlink(missing_ok=True)" in body
...
def test_generated_runbooks_pass_root_to_save_manifest():
    ...
    """
    targets = [
        REPO_ROOT / "map_mmd" / "skill.md",
        REPO_ROOT / "map_mmd" / "skill-aider.md",
        REPO_ROOT / "map_mmd" / "skill-devin.md",
    ]
    targets += sorted((REPO_ROOT / "map_mmd" / "skills").glob("*/references/update.md"))
    checked = 0
    for path in targets:
...
def test_always_on_renders_six_blocks():
    ...
    assert paths == [
        "map_mmd/always_on/agents-md.md",
        "map_mmd/always_on/antigravity-rules.md",
        "map_mmd/always_on/claude-md.md",
        "map_mmd/always_on/gemini-md.md",
        "map_mmd/always_on/kiro-steering.md",
        "map_mmd/always_on/vscode-instructions.md",
    ]
...
def test_always_on_included_in_full_render_not_per_platform():
    ...
    assert "map_mmd/always_on/claude-md.md" in full
    assert "map_mmd/always_on/claude-md.md" not in claude_only
...
def test_always_on_roundtrip_is_byte_faithful():
    ...
    map_mmd.__main__, so the packaged markdown must round-trip exactly or those
...
def test_extracted_constants_equal_the_packaged_always_on_files():
    ...
    from map_mmd import __main__ as mainmod
...
def test_always_on_files_are_guarded_by_check(tmp_path):
    ...
    mutated = [
        ...
        if a.path == "map_mmd/always_on/claude-md.md"
        else a
        for a in arts
    ]
...
def test_audit_reads_each_host_against_its_own_v8_body():
    ...
    """
    assert gen._v8_baseline_ref("claude") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:map_mmd/skill.md"
    assert gen._v8_baseline_ref("trae") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:map_mmd/skill-trae.md"
    assert gen._v8_baseline_ref("vscode") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:map_mmd/skill-vscode.md"
...
def test_trae_renders_native_agents_md_integration_not_claude():
    """trae wires `map_mmd trae install` -> AGENTS.md, never `map_mmd claude install`."""
    ...
    assert "map_mmd trae install" in hooks
    assert "map_mmd trae-cn install" in hooks
    assert "writes a `## map_mmd` section to the local `AGENTS.md`" in hooks
    # The claude-flavored install command must NOT appear for trae.
    assert "map_mmd claude install" not in hooks
    ...
    assert "wire map_mmd into a project's AGENTS.md" in core
...
def test_trae_hooks_reference_includes_the_pretooluse_note():
    ...
    assert "Run `/map_mmd --update` manually after code changes" in hooks
...
def test_claude_flavored_hosts_keep_their_hooks_text_unchanged():
    ...
    for key in ("claude", "droid", "codex", "windows", "kilo", "vscode"):
        core, refs = _platform_artifacts(key)
        hooks = refs["hooks.md"]
        assert "map_mmd claude install" in hooks, f"[{key}] lost the claude install command"
...
def test_amp_renders_native_agents_md_integration_v8_faithfully():
    """amp wires `map_mmd amp install` -> AGENTS.md exactly as its v8 body had it.
    ...
    assert "make map_mmd always-on in Amp sessions" in hooks
    assert "instructs Amp to check the graph" in hooks
    # amp's single-line install/uninstall, no trae-cn alt comments.
    assert "map_mmd amp install" in hooks
    assert "map_mmd amp uninstall  # remove the section" in hooks
    assert "map_mmd trae install" not in hooks
    assert "map_mmd trae-cn" not in hooks
    assert "or: map_mmd" not in hooks
...
def test_amp_audit_coverage_passes_against_its_own_v8():
    ...
    against a lean split. The audit reads origin/v8:map_mmd/skill-amp.md and
    confirms every heading single-homes in amp's core + references.
    """
    platforms = gen.load_platforms()
    assert gen._v8_baseline_ref("amp") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:map_mmd/skill-amp.md"
...
def test_agents_renders_its_own_agents_md_hooks_wording():
    ...
    `map_mmd agents install` and is worded for an unspecified host.
    """
    core, refs = _platform_artifacts("agents")
    hooks = refs["hooks.md"]
    assert "## For native AGENTS.md integration" in hooks
    assert "## For native AGENTS.md integration (Trae)" not in hooks
    assert "make map_mmd always-on in your agent sessions" in hooks
    assert "map_mmd agents install" in hooks
    assert "map_mmd agents uninstall  # remove the section" in hooks
    # No amp/trae/claude wording leaks into the agents render.
    assert "map_mmd amp install" not in hooks
    assert "map_mmd trae" not in hooks
    assert "map_mmd claude install" not in hooks
...
def test_agents_audit_baseline_is_amps_v8_body():
    ...
    platforms = gen.load_platforms()
    assert gen._v8_baseline_ref("agents") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:map_mmd/skill-amp.md"
