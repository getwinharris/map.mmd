...
These strings live in map_mmd/__main__.py and are written into project-local
files (CLAUDE.md, AGENTS.md, GEMINI.md, .cursor/rules/, .kiro/steering/, etc.)
or into in-process hook payloads. Earlier versions of map_mmd told every
assistant to "ALWAYS read map.mmd-out/GRAPH_REPORT.md before answering" —
...
def test_every_install_surface_recommends_map_mmd_query():
    """All ten install surfaces must point the assistant at `map_mmd query`
    ...
    for name, text in _INSTALL_TEXTS.items():
        if "map_mmd query" not in text:
            missing.append(name)
    assert not missing, (
        f"these install surfaces no longer mention `map_mmd query`: {missing}. "
...
def test_agents_section_does_not_skip_dirty_graph_output():
    assert "Dirty map.mmd-out/ files are expected" in _AGENTS_MD_SECTION
    assert "not a reason to skip map_mmd" in _AGENTS_MD_SECTION
