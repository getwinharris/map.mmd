"""Tests for map_mmd install --platform routing."""
import os
...
PLATFORMS = {
    "claude": (".claude/skills/map_mmd/SKILL.md",),
    "codebuddy": (".codebuddy/skills/map_mmd/SKILL.md",),
    "codex": (".codex/skills/map_mmd/SKILL.md",),
    "opencode": (".config/opencode/skills/map_mmd/SKILL.md",),
    "kilo": (
        ".config/kilo/skills/map_mmd/SKILL.md",
        ".config/kilo/command/map_mmd.md",
    ),
    "claw": (".openclaw/skills/map_mmd/SKILL.md",),
    "droid": (".factory/skills/map_mmd/SKILL.md",),
    "trae": (".trae/skills/map_mmd/SKILL.md",),
    "trae-cn": (".trae-cn/skills/map_mmd/SKILL.md",),
    "windows": (".claude/skills/map_mmd/SKILL.md",),
}
...
def test_install_default_claude(tmp_path):
    ...
    assert (tmp_path / ".claude" / "skills" / "map_mmd" / "SKILL.md").exists()


def test_install_codebuddy(tmp_path):
    _install(tmp_path, "codebuddy")
    assert (tmp_path / ".codebuddy" / "skills" / "map_mmd" / "SKILL.md").exists()


def test_install_codex(tmp_path):
    _install(tmp_path, "codex")
    assert (tmp_path / ".codex" / "skills" / "map_mmd" / "SKILL.md").exists()


def test_install_opencode(tmp_path):
    _install(tmp_path, "opencode")
    assert (
        tmp_path / ".config" / "opencode" / "skills" / "map_mmd" / "SKILL.md"
    ).exists()
...
def test_install_positional_platform_opencode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["map_mmd", "install", "opencode"])
    with patch("map_mmd.__main__.Path.home", return_value=tmp_path):
        main()
    assert (tmp_path / ".config" / "opencode" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert not (tmp_path / ".claude" / "skills" / "map_mmd" / "SKILL.md").exists()
...
def test_install_project_claude_writes_project_scope(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["map_mmd", "install", "--project"])
    with patch("map_mmd.__main__.Path.home", return_value=home):
        main()
    assert (project / ".claude" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert (project / ".claude" / "CLAUDE.md").exists()
    assert not (home / ".claude" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert ".claude/skills/map_mmd/SKILL.md" in (project / ".claude" / "CLAUDE.md").read_text()
    assert "~/.claude/skills/map_mmd/SKILL.md" not in (project / ".claude" / "CLAUDE.md").read_text()
...
def test_install_project_codex_writes_skill_and_agents(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["map_mmd", "install", "--project", "--platform", "codex"])
    with patch("map_mmd.__main__.Path.home", return_value=home):
        main()
    assert (project / ".codex" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert (project / "AGENTS.md").exists()
    assert (project / ".codex" / "hooks.json").exists()
    assert not (home / ".codex" / "skills" / "map_mmd" / "SKILL.md").exists()
...
def test_claude_subcommand_project_install_and_uninstall_are_project_scoped(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    user_skill = home / ".claude" / "skills" / "map_mmd" / "SKILL.md"
    user_skill.parent.mkdir(parents=True)
    user_skill.write_text("user skill")
    monkeypatch.chdir(project)
    with patch("map_mmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["map_mmd", "claude", "install", "--project"])
        main()
        assert (project / ".claude" / "skills" / "map_mmd" / "SKILL.md").exists()
        assert (project / ".claude" / "CLAUDE.md").exists()
        ...
        monkeypatch.setattr(sys, "argv", ["map_mmd", "claude", "uninstall", "--project"])
        main()

    assert user_skill.exists()
    assert not (project / ".claude" / "skills" / "map_mmd" / "SKILL.md").exists()
...
def test_codex_subcommand_project_install_and_uninstall_are_project_scoped(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    user_skill = home / ".codex" / "skills" / "map_mmd" / "SKILL.md"
    user_skill.parent.mkdir(parents=True)
    user_skill.write_text("user skill")
    monkeypatch.chdir(project)
    with patch("map_mmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["map_mmd", "codex", "install", "--project"])
        main()
        assert (project / ".codex" / "skills" / "map_mmd" / "SKILL.md").exists()
        assert (project / "AGENTS.md").exists()
        assert (project / ".codex" / "hooks.json").exists()
        ...
        monkeypatch.setattr(sys, "argv", ["map_mmd", "codex", "uninstall", "--project"])
        main()

    assert user_skill.exists()
    assert not (project / ".codex" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert not (project / "AGENTS.md").exists()
    hooks_path = project / ".codex" / "hooks.json"
    assert hooks_path.exists()
    assert "map_mmd" not in hooks_path.read_text()
...
def test_antigravity_install_project_writes_project_skill(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["map_mmd", "antigravity", "install", "--project"])
    with patch("map_mmd.__main__.Path.home", return_value=home):
        main()
    assert (project / ".agents" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert not (home / ".agents" / "skills" / "map_mmd" / "SKILL.md").exists()


def test_install_help_does_not_install_default(tmp_path, monkeypatch, capsys):
    from map_mmd.__main__ import main
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(sys, "argv", ["map_mmd", "install", "opencode", "--help"])
    with patch("map_mmd.__main__.Path.home", return_value=tmp_path):
        main()
    out = capsys.readouterr().out
    assert "Usage: map_mmd install" in out
...
def test_install_claw(tmp_path):
    ...
    assert (tmp_path / ".openclaw" / "skills" / "map_mmd" / "SKILL.md").exists()


def test_install_droid(tmp_path):
    _install(tmp_path, "droid")
    assert (tmp_path / ".factory" / "skills" / "map_mmd" / "SKILL.md").exists()


def test_install_trae(tmp_path):
    _install(tmp_path, "trae")
    assert (tmp_path / ".trae" / "skills" / "map_mmd" / "SKILL.md").exists()


def test_install_trae_cn(tmp_path):
    _install(tmp_path, "trae-cn")
    assert (tmp_path / ".trae-cn" / "skills" / "map_mmd" / "SKILL.md").exists()
...
def test_codex_skill_contains_spawn_agent():
    ...
    skill = (Path(map_mmd.__file__).parent / "skill-codex.md").read_text()
    assert "spawn_agent" in skill


def test_codex_skill_uses_map_mmd_with_existing_graph():
    ...
    assert "skip Steps 1–5 entirely and jump straight to `## For /map_mmd query`" in skill
    assert "map_mmd query" in skill
    assert "map_mmd explain" in skill
    assert "map_mmd path" in skill


def test_codex_agents_install_mentions_dirty_graph_output(tmp_path):
    _agents_install(tmp_path, "codex")
    content = (tmp_path / "AGENTS.md").read_text()
    assert "Dirty map.mmd-out/ files are expected" in content
    assert "not a reason to skip map_mmd" in content
...
def test_opencode_skill_contains_mention():
    ...
    skill = (Path(map_mmd.__file__).parent / "skill-opencode.md").read_text()
...
def test_kilo_skill_mentions_task_tool():
    ...
    skill = (Path(map_mmd.__file__).parent / "skill-kilo.md").read_text()
...
def test_claw_skill_uses_agent_tool_dispatch():
    ...
    skill = (Path(map_mmd.__file__).parent / "skill-claw.md").read_text()
    b2 = skill[skill.index("**Step B2"):skill.index("**Step B3")]
...
def test_all_skill_files_exist_in_package():
    ...
    pkg = Path(map_mmd.__file__).parent
    for name in (
        "skill.md",
        "skill-codex.md",
        "skill-opencode.md",
...
def test_codebuddy_install_writes_codebuddy_md(tmp_path):
    ...
    assert md.exists()
    assert "map.mmd-out/GRAPH_REPORT.md" in md.read_text()
...
def test_codebuddy_install_merges_existing_codebuddy_md(tmp_path):
    ...
    assert "map.mmd-out/GRAPH_REPORT.md" in content
...
def test_uninstall_project_removes_project_skill_only(tmp_path, monkeypatch):
    ...
    with patch("map_mmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["map_mmd", "install", "--project", "--platform", "codex"])
        main()
        monkeypatch.setattr(sys, "argv", ["map_mmd", "uninstall", "--project", "--platform", "codex"])
        main()
    assert user_skill.exists()
    assert not (project / ".codex" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert not (project / "AGENTS.md").exists()
...
def test_uninstall_project_without_platform_removes_project_installs(tmp_path, monkeypatch):
    ...
    with patch("map_mmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["map_mmd", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["map_mmd", "uninstall", "--project"])
        main()
    assert user_skill.exists()
    assert not (project / ".claude" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert not (project / ".claude" / "CLAUDE.md").exists()
...
def test_antigravity_uninstall_project_removes_project_skill_only(tmp_path, monkeypatch):
    ...
    global_skill = home / ".gemini" / "config" / "skills" / "map_mmd" / "SKILL.md"
    ...
    with patch("map_mmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["map_mmd", "antigravity", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["map_mmd", "antigravity", "uninstall", "--project"])
        main()
    assert global_skill.exists(), "project uninstall must not touch global skill"
    assert not (project / ".agents" / "skills" / "map_mmd" / "SKILL.md").exists()


def test_antigravity_global_install_writes_gemini_config_skills(tmp_path, monkeypatch):
    """Global `map_mmd antigravity install` must write to ~/.gemini/config/skills/ (#1079)."""
    from map_mmd.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    monkeypatch.chdir(project)
    with patch("map_mmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["map_mmd", "antigravity", "install"])
        main()
    global_skill = home / ".gemini" / "config" / "skills" / "map_mmd" / "SKILL.md"
    wrong_skill = home / ".agents" / "skills" / "map_mmd" / "SKILL.md"
    ...
    assert (project / ".agents" / "rules" / "map_mmd.md").exists()
    assert (project / ".agents" / "workflows" / "map_mmd.md").exists()


def test_antigravity_global_uninstall_removes_gemini_config_skill(tmp_path, monkeypatch):
    """Global `map_mmd antigravity uninstall` must remove from ~/.gemini/config/skills/ (#1079)."""
    from map_mmd.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("map_mmd.__main__.Path.home", return_value=home):
        ...
        monkeypatch.setattr(sys, "argv", ["map_mmd", "antigravity", "uninstall"])
        main()
    assert not global_skill.exists(), f"skill not removed from {global_skill} after uninstall"
    # workspace files also cleaned up
    assert not (project / ".agents" / "rules" / "map_mmd.md").exists()
    assert not (project / ".agents" / "workflows" / "map_mmd.md").exists()
...
def _agents_install(tmp_path, platform):
    from map_mmd.__main__ import _agents_install as _install_fn

    _install_fn(tmp_path, platform)


def _agents_uninstall(tmp_path, platform=""):
    from map_mmd.__main__ import _agents_uninstall as _uninstall_fn

    _uninstall_fn(tmp_path, platform=platform)


def _kilo_install(project_dir, home_dir):
    from map_mmd.__main__ import _kilo_install as _install_fn

    with patch("map_mmd.__main__.Path.home", return_value=home_dir):
        _install_fn(project_dir)


def _kilo_uninstall(project_dir, home_dir):
    from map_mmd.__main__ import _kilo_uninstall as _uninstall_fn
...
def test_codex_agents_install_writes_agents_md(tmp_path):
    ...
    assert "map_mmd" in agents_md.read_text()
...
def test_agents_uninstall_removes_section(tmp_path):
    ...
    agents_md = tmp_path / "AGENTS.md"
    # File deleted when it only contained map_mmd section
...
def test_opencode_agents_install_writes_plugin(tmp_path):
    """opencode install writes .opencode/plugins/map_mmd.js."""
    _agents_install(tmp_path, "opencode")
    plugin = tmp_path / ".opencode" / "plugins" / "map_mmd.js"
...
def test_opencode_plugin_reminder_has_no_backticks(tmp_path):
    ...
    the very map_mmd command we are only suggesting.
    """
    _agents_install(tmp_path, "opencode")
    plugin = tmp_path / ".opencode" / "plugins" / "map_mmd.js"
...
def test_opencode_agents_install_registers_plugin_in_config(tmp_path):
    ...
    assert any("map_mmd.js" in p for p in config.get("plugin", []))
...
def test_opencode_agents_install_merges_existing_config(tmp_path):
    ...
    assert any("map_mmd.js" in p for p in config["plugin"])
...
def test_opencode_agents_uninstall_removes_plugin(tmp_path):
    ...
    if config_file.exists():
        config = _json.loads(config_file.read_text())
        assert not any("map_mmd.js" in p for p in config.get("plugin", []))
...
def test_kilo_agents_install_writes_plugin(tmp_path):
    _agents_install(tmp_path, "kilo")
    plugin = tmp_path / ".kilo" / "plugins" / "map_mmd.js"
    assert plugin.exists()
    assert "tool.execute.before" in plugin.read_text()
...
def test_kilo_agents_install_registers_plugin_in_config(tmp_path):
    ...
    config = _json.loads(config_file.read_text())
    assert (
        tmp_path / ".kilo" / "plugins" / "map_mmd.js"
...
def test_kilo_agents_install_idempotent(tmp_path):
    ...
    plugin_uri = (tmp_path / ".kilo" / "plugins" / "map_mmd.js").resolve().as_uri()
    assert content.count("## map_mmd") == 1
...
def test_kilo_install_writes_global_and_project_artifacts(tmp_path):
    ...
    assert (home_dir / ".config" / "kilo" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert (home_dir / ".config" / "kilo" / "command" / "map_mmd.md").exists()
    assert (project_dir / "AGENTS.md").exists()
    assert (project_dir / ".kilo" / "plugins" / "map_mmd.js").exists()
...
def test_kilo_uninstall_removes_plugin_registration_and_command(tmp_path):
    ...
    assert not (home_dir / ".config" / "kilo" / "command" / "map_mmd.md").exists()
    assert not (
        home_dir / ".config" / "kilo" / "skills" / "map_mmd" / "SKILL.md"
    ).exists()
    assert not (project_dir / ".kilo" / "plugins" / "map_mmd.js").exists()
    config_file = project_dir / ".kilo" / "kilo.json"
    if config_file.exists():
        config = _json.loads(config_file.read_text())
        assert (
            project_dir / ".kilo" / "plugins" / "map_mmd.js"
        ).resolve().as_uri() not in config.get("plugin", [])
...
def test_cursor_install_writes_rule(tmp_path):
    """cursor install writes .cursor/rules/map_mmd.mdc."""
    from map_mmd.__main__ import _cursor_install

    _cursor_install(tmp_path)
    rule = tmp_path / ".cursor" / "rules" / "map_mmd.mdc"
    assert rule.exists()
...
def test_cursor_uninstall_removes_rule(tmp_path):
    ...
    from map_mmd.__main__ import _cursor_install, _cursor_uninstall
...
def test_cursor_uninstall_noop_if_not_installed(tmp_path):
    ...
    from map_mmd.__main__ import _cursor_uninstall
...
def test_gemini_install_writes_gemini_md(tmp_path):
    from map_mmd.__main__ import gemini_install
...
def test_gemini_uninstall_removes_section(tmp_path):
    from map_mmd.__main__ import gemini_install, gemini_uninstall
...
def test_gemini_uninstall_noop_if_not_installed(tmp_path):
    from map_mmd.__main__ import gemini_uninstall

    gemini_uninstall(tmp_path)  # should not raise


def test_amp_user_install_lands_in_config_agents(tmp_path, monkeypatch):
    """`map_mmd amp install` (user scope) must drop the skill into an Amp search
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["map_mmd", "amp", "install"])
    with patch("map_mmd.__main__.Path.home", return_value=home):
        main()

    correct = home / ".config" / "agents" / "skills" / "map_mmd" / "SKILL.md"
    old = home / ".amp" / "skills" / "map_mmd" / "SKILL.md"
...
def test_amp_install_cleans_legacy_amp_skills_dir(tmp_path, monkeypatch):
    """A pre-fix ~/.amp/skills/map_mmd install is removed on the next install."""
    from map_mmd.__main__ import main

    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    legacy = home / ".amp" / "skills" / "map_mmd"
    ...
    assert not legacy.exists(), "legacy ~/.amp/skills/map_mmd should be cleaned up"
    assert (home / ".config" / "agents" / "skills" / "map_mmd" / "SKILL.md").exists()


def test_amp_user_uninstall_removes_skill_and_agents(tmp_path, monkeypatch):
    """`map_mmd amp uninstall` removes the user-scope skill and AGENTS.md section."""
    from map_mmd.__main__ import main
    ...
    with patch("map_mmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["map_mmd", "amp", "install"])
        main()
        skill = home / ".config" / "agents" / "skills" / "map_mmd" / "SKILL.md"
        assert skill.exists()

        monkeypatch.setattr(sys, "argv", ["map_mmd", "amp", "uninstall"])
        main()
...
def test_amp_project_install_lands_in_dot_agents(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["map_mmd", "amp", "install", "--project"])
    with patch("map_mmd.__main__.Path.home", return_value=home):
        main()

    assert (project / ".agents" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert not (project / ".amp" / "skills" / "map_mmd" / "SKILL.md").exists()
    assert (project / "AGENTS.md").exists()
    # User scope untouched.
    assert not (home / ".config" / "agents" / "skills" / "map_mmd" / "SKILL.md").exists()


def test_uninstall_all_removes_amp_user_skill(tmp_path, monkeypatch):
    """The user-scope `map_mmd uninstall` enumeration removes the amp skill."""
    from map_mmd.__main__ import main
...
def test_hermes_skill_destination_windows_uses_localappdata():
    ...
    with patch("map_mmd.__main__.platform.system", return_value="Windows"), \
         patch.dict(os.environ, {"LOCALAPPDATA": str(Path("/tmp/AppDataLocal"))}):
        dst = _platform_skill_destination("hermes", project=False)
    assert dst == Path("/tmp/AppDataLocal") / "hermes" / "skills" / "map_mmd" / "SKILL.md", dst


def test_hermes_skill_destination_posix_uses_home():
    """Non-Windows hermes destination is unchanged (~/.hermes/skills)."""
    from map_mmd.__main__ import _platform_skill_destination
    with patch("map_mmd.__main__.platform.system", return_value="Linux"):
        dst = _platform_skill_destination("hermes", project=False)
    assert str(dst).endswith(".hermes/skills/map_mmd/SKILL.md"), dst
