"""Tests for mapmmd install --platform routing."""
import os
...
PLATFORMS = {
    "claude": (".claude/skills/mapmmd/SKILL.md",),
    "codebuddy": (".codebuddy/skills/mapmmd/SKILL.md",),
    "codex": (".codex/skills/mapmmd/SKILL.md",),
    "opencode": (".config/opencode/skills/mapmmd/SKILL.md",),
    "kilo": (
        ".config/kilo/skills/mapmmd/SKILL.md",
        ".config/kilo/command/mapmmd.md",
    ),
    "claw": (".openclaw/skills/mapmmd/SKILL.md",),
    "droid": (".factory/skills/mapmmd/SKILL.md",),
    "trae": (".trae/skills/mapmmd/SKILL.md",),
    "trae-cn": (".trae-cn/skills/mapmmd/SKILL.md",),
    "windows": (".claude/skills/mapmmd/SKILL.md",),
}
...
def test_install_default_claude(tmp_path):
    ...
    assert (tmp_path / ".claude" / "skills" / "mapmmd" / "SKILL.md").exists()


def test_install_codebuddy(tmp_path):
    _install(tmp_path, "codebuddy")
    assert (tmp_path / ".codebuddy" / "skills" / "mapmmd" / "SKILL.md").exists()


def test_install_codex(tmp_path):
    _install(tmp_path, "codex")
    assert (tmp_path / ".codex" / "skills" / "mapmmd" / "SKILL.md").exists()


def test_install_opencode(tmp_path):
    _install(tmp_path, "opencode")
    assert (
        tmp_path / ".config" / "opencode" / "skills" / "mapmmd" / "SKILL.md"
    ).exists()
...
def test_install_positional_platform_opencode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["mapmmd", "install", "opencode"])
    with patch("mapmmd.__main__.Path.home", return_value=tmp_path):
        main()
    assert (tmp_path / ".config" / "opencode" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert not (tmp_path / ".claude" / "skills" / "mapmmd" / "SKILL.md").exists()
...
def test_install_project_claude_writes_project_scope(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["mapmmd", "install", "--project"])
    with patch("mapmmd.__main__.Path.home", return_value=home):
        main()
    assert (project / ".claude" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert (project / ".claude" / "CLAUDE.md").exists()
    assert not (home / ".claude" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert ".claude/skills/mapmmd/SKILL.md" in (project / ".claude" / "CLAUDE.md").read_text()
    assert "~/.claude/skills/mapmmd/SKILL.md" not in (project / ".claude" / "CLAUDE.md").read_text()
...
def test_install_project_codex_writes_skill_and_agents(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["mapmmd", "install", "--project", "--platform", "codex"])
    with patch("mapmmd.__main__.Path.home", return_value=home):
        main()
    assert (project / ".codex" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert (project / "AGENTS.md").exists()
    assert (project / ".codex" / "hooks.json").exists()
    assert not (home / ".codex" / "skills" / "mapmmd" / "SKILL.md").exists()
...
def test_claude_subcommand_project_install_and_uninstall_are_project_scoped(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    user_skill = home / ".claude" / "skills" / "mapmmd" / "SKILL.md"
    user_skill.parent.mkdir(parents=True)
    user_skill.write_text("user skill")
    monkeypatch.chdir(project)
    with patch("mapmmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["mapmmd", "claude", "install", "--project"])
        main()
        assert (project / ".claude" / "skills" / "mapmmd" / "SKILL.md").exists()
        assert (project / ".claude" / "CLAUDE.md").exists()
        ...
        monkeypatch.setattr(sys, "argv", ["mapmmd", "claude", "uninstall", "--project"])
        main()

    assert user_skill.exists()
    assert not (project / ".claude" / "skills" / "mapmmd" / "SKILL.md").exists()
...
def test_codex_subcommand_project_install_and_uninstall_are_project_scoped(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    user_skill = home / ".codex" / "skills" / "mapmmd" / "SKILL.md"
    user_skill.parent.mkdir(parents=True)
    user_skill.write_text("user skill")
    monkeypatch.chdir(project)
    with patch("mapmmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["mapmmd", "codex", "install", "--project"])
        main()
        assert (project / ".codex" / "skills" / "mapmmd" / "SKILL.md").exists()
        assert (project / "AGENTS.md").exists()
        assert (project / ".codex" / "hooks.json").exists()
        ...
        monkeypatch.setattr(sys, "argv", ["mapmmd", "codex", "uninstall", "--project"])
        main()

    assert user_skill.exists()
    assert not (project / ".codex" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert not (project / "AGENTS.md").exists()
    hooks_path = project / ".codex" / "hooks.json"
    assert hooks_path.exists()
    assert "mapmmd" not in hooks_path.read_text()
...
def test_antigravity_install_project_writes_project_skill(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["mapmmd", "antigravity", "install", "--project"])
    with patch("mapmmd.__main__.Path.home", return_value=home):
        main()
    assert (project / ".agents" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert not (home / ".agents" / "skills" / "mapmmd" / "SKILL.md").exists()


def test_install_help_does_not_install_default(tmp_path, monkeypatch, capsys):
    from mapmmd.__main__ import main
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(sys, "argv", ["mapmmd", "install", "opencode", "--help"])
    with patch("mapmmd.__main__.Path.home", return_value=tmp_path):
        main()
    out = capsys.readouterr().out
    assert "Usage: mapmmd install" in out
...
def test_install_claw(tmp_path):
    ...
    assert (tmp_path / ".openclaw" / "skills" / "mapmmd" / "SKILL.md").exists()


def test_install_droid(tmp_path):
    _install(tmp_path, "droid")
    assert (tmp_path / ".factory" / "skills" / "mapmmd" / "SKILL.md").exists()


def test_install_trae(tmp_path):
    _install(tmp_path, "trae")
    assert (tmp_path / ".trae" / "skills" / "mapmmd" / "SKILL.md").exists()


def test_install_trae_cn(tmp_path):
    _install(tmp_path, "trae-cn")
    assert (tmp_path / ".trae-cn" / "skills" / "mapmmd" / "SKILL.md").exists()
...
def test_codex_skill_contains_spawn_agent():
    ...
    skill = (Path(mapmmd.__file__).parent / "skill-codex.md").read_text()
    assert "spawn_agent" in skill


def test_codex_skill_uses_mapmmd_with_existing_graph():
    ...
    assert "skip Steps 1–5 entirely and jump straight to `## For /mapmmd query`" in skill
    assert "mapmmd query" in skill
    assert "mapmmd explain" in skill
    assert "mapmmd path" in skill


def test_codex_agents_install_mentions_dirty_graph_output(tmp_path):
    _agents_install(tmp_path, "codex")
    content = (tmp_path / "AGENTS.md").read_text()
    assert "Dirty mapmmd-out/ files are expected" in content
    assert "not a reason to skip mapmmd" in content
...
def test_opencode_skill_contains_mention():
    ...
    skill = (Path(mapmmd.__file__).parent / "skill-opencode.md").read_text()
...
def test_kilo_skill_mentions_task_tool():
    ...
    skill = (Path(mapmmd.__file__).parent / "skill-kilo.md").read_text()
...
def test_claw_skill_uses_agent_tool_dispatch():
    ...
    skill = (Path(mapmmd.__file__).parent / "skill-claw.md").read_text()
    b2 = skill[skill.index("**Step B2"):skill.index("**Step B3")]
...
def test_all_skill_files_exist_in_package():
    ...
    pkg = Path(mapmmd.__file__).parent
    for name in (
        "skill.md",
        "skill-codex.md",
        "skill-opencode.md",
...
def test_codebuddy_install_writes_codebuddy_md(tmp_path):
    ...
    assert md.exists()
    assert "mapmmd-out/GRAPH_REPORT.md" in md.read_text()
...
def test_codebuddy_install_merges_existing_codebuddy_md(tmp_path):
    ...
    assert "mapmmd-out/GRAPH_REPORT.md" in content
...
def test_uninstall_project_removes_project_skill_only(tmp_path, monkeypatch):
    ...
    with patch("mapmmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["mapmmd", "install", "--project", "--platform", "codex"])
        main()
        monkeypatch.setattr(sys, "argv", ["mapmmd", "uninstall", "--project", "--platform", "codex"])
        main()
    assert user_skill.exists()
    assert not (project / ".codex" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert not (project / "AGENTS.md").exists()
...
def test_uninstall_project_without_platform_removes_project_installs(tmp_path, monkeypatch):
    ...
    with patch("mapmmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["mapmmd", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["mapmmd", "uninstall", "--project"])
        main()
    assert user_skill.exists()
    assert not (project / ".claude" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert not (project / ".claude" / "CLAUDE.md").exists()
...
def test_antigravity_uninstall_project_removes_project_skill_only(tmp_path, monkeypatch):
    ...
    global_skill = home / ".gemini" / "config" / "skills" / "mapmmd" / "SKILL.md"
    ...
    with patch("mapmmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["mapmmd", "antigravity", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["mapmmd", "antigravity", "uninstall", "--project"])
        main()
    assert global_skill.exists(), "project uninstall must not touch global skill"
    assert not (project / ".agents" / "skills" / "mapmmd" / "SKILL.md").exists()


def test_antigravity_global_install_writes_gemini_config_skills(tmp_path, monkeypatch):
    """Global `mapmmd antigravity install` must write to ~/.gemini/config/skills/ (#1079)."""
    from mapmmd.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    monkeypatch.chdir(project)
    with patch("mapmmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["mapmmd", "antigravity", "install"])
        main()
    global_skill = home / ".gemini" / "config" / "skills" / "mapmmd" / "SKILL.md"
    wrong_skill = home / ".agents" / "skills" / "mapmmd" / "SKILL.md"
    ...
    assert (project / ".agents" / "rules" / "mapmmd.md").exists()
    assert (project / ".agents" / "workflows" / "mapmmd.md").exists()


def test_antigravity_global_uninstall_removes_gemini_config_skill(tmp_path, monkeypatch):
    """Global `mapmmd antigravity uninstall` must remove from ~/.gemini/config/skills/ (#1079)."""
    from mapmmd.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("mapmmd.__main__.Path.home", return_value=home):
        ...
        monkeypatch.setattr(sys, "argv", ["mapmmd", "antigravity", "uninstall"])
        main()
    assert not global_skill.exists(), f"skill not removed from {global_skill} after uninstall"
    # workspace files also cleaned up
    assert not (project / ".agents" / "rules" / "mapmmd.md").exists()
    assert not (project / ".agents" / "workflows" / "mapmmd.md").exists()
...
def _agents_install(tmp_path, platform):
    from mapmmd.__main__ import _agents_install as _install_fn

    _install_fn(tmp_path, platform)


def _agents_uninstall(tmp_path, platform=""):
    from mapmmd.__main__ import _agents_uninstall as _uninstall_fn

    _uninstall_fn(tmp_path, platform=platform)


def _kilo_install(project_dir, home_dir):
    from mapmmd.__main__ import _kilo_install as _install_fn

    with patch("mapmmd.__main__.Path.home", return_value=home_dir):
        _install_fn(project_dir)


def _kilo_uninstall(project_dir, home_dir):
    from mapmmd.__main__ import _kilo_uninstall as _uninstall_fn
...
def test_codex_agents_install_writes_agents_md(tmp_path):
    ...
    assert "mapmmd" in agents_md.read_text()
...
def test_agents_uninstall_removes_section(tmp_path):
    ...
    agents_md = tmp_path / "AGENTS.md"
    # File deleted when it only contained mapmmd section
...
def test_opencode_agents_install_writes_plugin(tmp_path):
    """opencode install writes .opencode/plugins/mapmmd.js."""
    _agents_install(tmp_path, "opencode")
    plugin = tmp_path / ".opencode" / "plugins" / "mapmmd.js"
...
def test_opencode_plugin_reminder_has_no_backticks(tmp_path):
    ...
    the very mapmmd command we are only suggesting.
    """
    _agents_install(tmp_path, "opencode")
    plugin = tmp_path / ".opencode" / "plugins" / "mapmmd.js"
...
def test_opencode_agents_install_registers_plugin_in_config(tmp_path):
    ...
    assert any("mapmmd.js" in p for p in config.get("plugin", []))
...
def test_opencode_agents_install_merges_existing_config(tmp_path):
    ...
    assert any("mapmmd.js" in p for p in config["plugin"])
...
def test_opencode_agents_uninstall_removes_plugin(tmp_path):
    ...
    if config_file.exists():
        config = _json.loads(config_file.read_text())
        assert not any("mapmmd.js" in p for p in config.get("plugin", []))
...
def test_kilo_agents_install_writes_plugin(tmp_path):
    _agents_install(tmp_path, "kilo")
    plugin = tmp_path / ".kilo" / "plugins" / "mapmmd.js"
    assert plugin.exists()
    assert "tool.execute.before" in plugin.read_text()
...
def test_kilo_agents_install_registers_plugin_in_config(tmp_path):
    ...
    config = _json.loads(config_file.read_text())
    assert (
        tmp_path / ".kilo" / "plugins" / "mapmmd.js"
...
def test_kilo_agents_install_idempotent(tmp_path):
    ...
    plugin_uri = (tmp_path / ".kilo" / "plugins" / "mapmmd.js").resolve().as_uri()
    assert content.count("## mapmmd") == 1
...
def test_kilo_install_writes_global_and_project_artifacts(tmp_path):
    ...
    assert (home_dir / ".config" / "kilo" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert (home_dir / ".config" / "kilo" / "command" / "mapmmd.md").exists()
    assert (project_dir / "AGENTS.md").exists()
    assert (project_dir / ".kilo" / "plugins" / "mapmmd.js").exists()
...
def test_kilo_uninstall_removes_plugin_registration_and_command(tmp_path):
    ...
    assert not (home_dir / ".config" / "kilo" / "command" / "mapmmd.md").exists()
    assert not (
        home_dir / ".config" / "kilo" / "skills" / "mapmmd" / "SKILL.md"
    ).exists()
    assert not (project_dir / ".kilo" / "plugins" / "mapmmd.js").exists()
    config_file = project_dir / ".kilo" / "kilo.json"
    if config_file.exists():
        config = _json.loads(config_file.read_text())
        assert (
            project_dir / ".kilo" / "plugins" / "mapmmd.js"
        ).resolve().as_uri() not in config.get("plugin", [])
...
def test_cursor_install_writes_rule(tmp_path):
    """cursor install writes .cursor/rules/mapmmd.mdc."""
    from mapmmd.__main__ import _cursor_install

    _cursor_install(tmp_path)
    rule = tmp_path / ".cursor" / "rules" / "mapmmd.mdc"
    assert rule.exists()
...
def test_cursor_uninstall_removes_rule(tmp_path):
    ...
    from mapmmd.__main__ import _cursor_install, _cursor_uninstall
...
def test_cursor_uninstall_noop_if_not_installed(tmp_path):
    ...
    from mapmmd.__main__ import _cursor_uninstall
...
def test_gemini_install_writes_gemini_md(tmp_path):
    from mapmmd.__main__ import gemini_install
...
def test_gemini_uninstall_removes_section(tmp_path):
    from mapmmd.__main__ import gemini_install, gemini_uninstall
...
def test_gemini_uninstall_noop_if_not_installed(tmp_path):
    from mapmmd.__main__ import gemini_uninstall

    gemini_uninstall(tmp_path)  # should not raise


def test_amp_user_install_lands_in_config_agents(tmp_path, monkeypatch):
    """`mapmmd amp install` (user scope) must drop the skill into an Amp search
    ...
    correct = home / ".config" / "agents" / "skills" / "mapmmd" / "SKILL.md"
    old = home / ".amp" / "skills" / "mapmmd" / "SKILL.md"
...
def test_amp_install_cleans_legacy_amp_skills_dir(tmp_path, monkeypatch):
    """A pre-fix ~/.amp/skills/mapmmd install is removed on the next install."""
    from mapmmd.__main__ import main

    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    legacy = home / ".amp" / "skills" / "mapmmd"
    ...
    assert not legacy.exists(), "legacy ~/.amp/skills/mapmmd should be cleaned up"
    assert (home / ".config" / "agents" / "skills" / "mapmmd" / "SKILL.md").exists()


def test_amp_user_uninstall_removes_skill_and_agents(tmp_path, monkeypatch):
    """`mapmmd amp uninstall` removes the user-scope skill and AGENTS.md section."""
    from mapmmd.__main__ import main
    ...
    with patch("mapmmd.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["mapmmd", "amp", "install"])
        main()
        skill = home / ".config" / "agents" / "skills" / "mapmmd" / "SKILL.md"
        assert skill.exists()
...
def test_amp_project_install_lands_in_dot_agents(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["mapmmd", "amp", "install", "--project"])
    with patch("mapmmd.__main__.Path.home", return_value=home):
        main()

    assert (project / ".agents" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert not (project / ".amp" / "skills" / "mapmmd" / "SKILL.md").exists()
    assert (project / "AGENTS.md").exists()
    # User scope untouched.
    assert not (home / ".config" / "agents" / "skills" / "mapmmd" / "SKILL.md").exists()


def test_uninstall_all_removes_amp_user_skill(tmp_path, monkeypatch):
    """The user-scope `mapmmd uninstall` enumeration removes the amp skill."""
    from mapmmd.__main__ import main
...
def test_hermes_skill_destination_windows_uses_localappdata():
    ...
    with patch("mapmmd.__main__.platform.system", return_value="Windows"), \
         patch.dict(os.environ, {"LOCALAPPDATA": str(Path("/tmp/AppDataLocal"))}):
        dst = _platform_skill_destination("hermes", project=False)
    assert dst == Path("/tmp/AppDataLocal") / "hermes" / "skills" / "mapmmd" / "SKILL.md", dst


def test_hermes_skill_destination_posix_uses_home():
    """Non-Windows hermes destination is unchanged (~/.hermes/skills)."""
    from mapmmd.__main__ import _platform_skill_destination
    with patch("mapmmd.__main__.platform.system", return_value="Linux"):
        dst = _platform_skill_destination("hermes", project=False)
    assert str(dst).endswith(".hermes/skills/mapmmd/SKILL.md"), dst
