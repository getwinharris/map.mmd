...
The real claude bundle now ships in the package (map_mmd/skills/claude/), so
...
def test_single_version_stamp_covers_skill_and_references(tmp_path, fake_bundle):
    """One .map_mmd_version stamp versions SKILL.md + references/ together."""
    platform = fake_bundle
    _install(tmp_path, platform)
    skill_dir = tmp_path / ".claude" / "skills" / "map_mmd"
    stamps = list(skill_dir.rglob(".map_mmd_version"))
    assert len(stamps) == 1
    assert stamps[0] == skill_dir / ".map_mmd_version"
...
def test_gemini_install_references_all_resolve(tmp_path):
    ...
    skill = tmp_path / ".gemini" / "skills" / "map_mmd" / "SKILL.md"
    assert skill.exists()
...
def test_pyproject_declares_references_globs():
    ...
    The references sidecar ships under map_mmd/skills/<host>/references/ and the
    always-on injection blocks under map_mmd/always_on/. The earlier
    skills/*/SKILL.md glob matched nothing (no map_mmd/skills/<host>/SKILL.md
    exists; the skill bodies are map_mmd/skill*.md, listed explicitly), so it was
    ...
    pkg_data = data["tool"]["setuptools"]["package-data"]["map_mmd"]
...
def test_built_wheel_ships_the_full_skill_payload():
    ...
    assert "map_mmd/skills/claude/references/extraction-spec.md" in names
    assert "map_mmd/skills/trae/references/hooks.md" in names
    # amp is now a split host too; its bundle must ship like every other.
    assert "map_mmd/skills/amp/references/hooks.md" in names
...
def test_monolith_install_clears_orphan_references(tmp_path, fake_bundle):
    ...
    skill_dst = tmp_path / ".aider" / "map_mmd" / "SKILL.md"
...
def test_amp_user_install_carries_references(tmp_path, monkeypatch):
    ...
    amp's real bundle now ships in the package (map_mmd/skills/amp/), so the
    ...
    with patch("map_mmd.__main__.Path.home", return_value=home):
        ...
        main()
        skill_dir = home / ".config" / "agents" / "skills" / "map_mmd"
        assert (skill_dir / "SKILL.md").exists()
