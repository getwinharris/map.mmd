...
Every platform mapmmd knows about installs its SKILL.md at a specific real
...
def test_skill_roundtrip_at_real_destination(platform, project, tmp_path, monkeypatch):
    ...
    with patch("mapmmd.__main__.Path.home", return_value=home):
        ...
        assert (dst.parent / ".mapmmd_version").read_text() == mainmod.__version__
        ...
        assert not (dst.parent / ".mapmmd_version").exists()
...
def test_amp_user_install_at_corrected_agents_path(tmp_path, monkeypatch):
    ...
    with patch("mapmmd.__main__.Path.home", return_value=home):
        ...
        assert dst == home / ".config" / "agents" / "skills" / "mapmmd" / "SKILL.md"
        assert dst.exists()
...
def test_amp_project_install_at_agents_path(tmp_path, monkeypatch):
    ...
    project_dir.mkdir()
    monkeypatch.chdir(project_dir)
    with patch("mapmmd.__main__.Path.home", return_value=tmp_path / "home"):
        dst = mainmod._copy_skill_file("amp", project=True, project_dir=project_dir)
        assert dst == project_dir / ".agents" / "skills" / "mapmmd" / "SKILL.md"
        assert dst.exists()
...
def test_vscode_install_uninstall_roundtrip(tmp_path, monkeypatch):
    """VS Code Copilot Chat round trip at ~/.copilot/skills/mapmmd + instructions file."""
    home = tmp_path / "home"
    project_dir = tmp_path / "proj"
    home.mkdir()
    project_dir.mkdir()
    ...
    with patch("mapmmd.__main__.Path.home", return_value=home):
        ...
        skill = home / ".copilot" / "skills" / "mapmmd" / "SKILL.md"
        instructions = project_dir / ".github" / "copilot-instructions.md"
        assert skill.exists()
        assert instructions.exists()
        assert "## mapmmd" in instructions.read_text(encoding="utf-8")
        ...
        # The mapmmd section is stripped from the instructions file.
        if instructions.exists():
            assert "## mapmmd" not in instructions.read_text(encoding="utf-8")
...
def test_install_entrypoint_roundtrip_for_progressive_and_monolith(tmp_path):
    ...
    """
    for platform, rel in (
        ("claude", Path(".claude") / "skills" / "mapmmd"),
        ("aider", Path(".aider") / "mapmmd"),
    ):
...
def test_monolith_to_progressive_upgrade(tmp_path, fake_progressive_bundle):
    ...
    (skill_dir / ".mapmmd_version").write_text("0.0.1", encoding="utf-8")
    assert not (skill_dir / "references").exists()
...
def test_failed_copytree_leaves_no_partial_references(tmp_path, fake_progressive_bundle):
    ...
    with patch("mapmmd.__main__.shutil.copytree", side_effect=boom):
