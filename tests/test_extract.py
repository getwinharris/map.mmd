...
from map_mmd.extract import extract_python, extract, collect_files, _make_id, extract_bash, extract_json, _DISPATCH
...
def test_collect_files_from_dir():
    from map_mmd.extract import _DISPATCH
...
def test_extract_generic_surfaces_tree_sitter_version_mismatch_hint(monkeypatch):
    ...
    import sys
    import types
    from map_mmd.extract import _extract_generic, LanguageConfig
...
def test_extract_js_destructured_require_named_symbols():
    ...
    from map_mmd.extract import extract_js, _make_id, _file_stem
    result = extract_js(FIXTURES / "cjs_require.js")
...
def test_extract_tsx_uses_tsx_grammar():
    ...
    from map_mmd.extract import _TSX_CONFIG, _TS_CONFIG
...
def test_extract_falls_back_to_sequential_when_parallel_returns_false(tmp_path, monkeypatch):
    ...
    from map_mmd import extract as extract_mod
...
def test_semantic_reference_edges_carry_context_and_source():
    from map_mmd.extract import _semantic_reference_edge
...
def test_dart_child_node_ids_are_stem_based(tmp_path):
    ...
    from map_mmd.extract import extract_dart, _file_stem, _make_id
