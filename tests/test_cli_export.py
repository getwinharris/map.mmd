"""Integration tests for map_mmd export subcommands and CLI commands.
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels files in tmp_path/map.mmd-out/."""
    ...
    (out / ".map_mmd_analysis.json").write_text(json.dumps(analysis))
...
def test_export_html_creates_file(tmp_path):
    ...
    html = tmp_path / "map.mmd-out" / "graph.html"
    assert html.exists()
...
def test_export_wiki_creates_articles(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    wiki = tmp_path / "map.mmd-out" / "wiki"
    assert wiki.exists()
...
# ── map_mmd export falkordb (cypher) ────────────────────────────────────────
...
def test_query_uses_map_mmd_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
    env["MAP_MMD_OUT"] = custom_out.name
...
def test_extract_writes_to_map_mmd_out_env(tmp_path):
    """#1423: `map_mmd extract` honours MAP_MMD_OUT for where it WRITES, not only
    where readers look — previously it hardcoded map.mmd-out/ and ignored the
    override. Code-only corpus, so no LLM backend is needed."""
    (tmp_path / "m.py").write_text("def a():\n    return b()\n\n\ndef b():\n    return 1\n")
    env = os.environ.copy()
    env["MAP_MMD_OUT"] = "custom-out"
    ...
    assert not (tmp_path / "map.mmd-out").exists(), "extract ignored MAP_MMD_OUT and wrote map.mmd-out/"
...
def test_path_uses_map_mmd_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
def test_explain_uses_map_mmd_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
# Regression test for #934 - cluster-only crashes when map.mmd-out/ doesn't exist

def test_cluster_only_creates_output_dir_when_missing(tmp_path):
    """cluster-only must not crash with FileNotFoundError when map.mmd-out/ is absent (#934)."""
    # Build graph.json somewhere other than the default map.mmd-out/ location
    # so we can point --graph at it while map.mmd-out/ doesn't exist yet.
    ...
    assert r.returncode == 0, r.stderr
    assert (tmp_path / "map.mmd-out" / "GRAPH_REPORT.md").exists()
...
def test_cluster_only_remaps_labels_to_previous_cids(tmp_path):
    ...
    .map_mmd_labels.json keeps tracking the same conceptual communities after
    ...
    labels_json = out / ".map_mmd_labels.json"
...
# ── communities-fallback when .map_mmd_analysis.json is absent ──────────────
# The watch / post-commit rebuild path only writes graph.json + GRAPH_REPORT.md;
# it does NOT regenerate .map_mmd_analysis.json. The full `map_mmd extract`
...
# Without these tests, `map_mmd export html|obsidian|wiki|svg|graphml|neo4j`
...
def test_export_html_falls_back_to_node_community_attribute(tmp_path):
    """When .map_mmd_analysis.json is absent, export html should reconstruct
    ...
    (out / ".map_mmd_analysis.json").unlink()
...
def test_export_html_fallback_recovers_multiple_communities(tmp_path):
    ...
    analysis = json.loads((out / ".map_mmd_analysis.json").read_text(encoding="utf-8"))
