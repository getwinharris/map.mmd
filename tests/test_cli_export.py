"""Integration tests for mapmmd export subcommands and CLI commands.
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels files in tmp_path/mapmmd-out/."""
    ...
    (out / ".mapmmd_analysis.json").write_text(json.dumps(analysis))
    (out / ".mapmmd_labels.json").write_text(
...
def test_export_html_creates_file(tmp_path):
    ...
    html = tmp_path / "mapmmd-out" / "graph.html"
    assert html.exists()
...
def test_export_graphml_creates_file(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    gml = tmp_path / "mapmmd-out" / "graph.graphml"
    assert gml.exists()
...
def test_query_uses_mapmmd_out_env(tmp_path):
    out = _make_graph(tmp_path)
...
def test_path_uses_mapmmd_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
def test_explain_uses_mapmmd_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
# Regression test for #934 - cluster-only crashes when mapmmd-out/ doesn't exist

def test_cluster_only_creates_output_dir_when_missing(tmp_path):
    """cluster-only must not crash with FileNotFoundError when mapmmd-out/ is absent (#934)."""
    # Build graph.json somewhere other than the default mapmmd-out/ location
    # so we can point --graph at it while mapmmd-out/ doesn't exist yet.
    ...
    assert r.returncode == 0, r.stderr
    assert (tmp_path / "mapmmd-out" / "GRAPH_REPORT.md").exists()
...
def test_cluster_only_remaps_labels_to_previous_cids(tmp_path):
    ...
    .mapmmd_labels.json keeps tracking the same conceptual communities after
    ...
    labels_json = out / ".mapmmd_labels.json"
...
# ── communities-fallback when .mapmmd_analysis.json is absent ──────────────
# The watch / post-commit rebuild path only writes graph.json + GRAPH_REPORT.md;
# it does NOT regenerate .mapmmd_analysis.json. The full `mapmmd extract`
...
# Without these tests, `mapmmd export html|obsidian|wiki|svg|graphml|neo4j`
...
def test_export_html_falls_back_to_node_community_attribute(tmp_path):
    """When .mapmmd_analysis.json is absent, export html should reconstruct
    ...
    (out / ".mapmmd_analysis.json").unlink()
...
def test_export_html_fallback_recovers_multiple_communities(tmp_path):
    ...
    analysis = json.loads((out / ".mapmmd_analysis.json").read_text(encoding="utf-8"))
