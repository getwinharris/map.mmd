...
A map.mmd dry-run against [`glitchwerks/rsl-siege-manager`](https://github.com/glitchwerks/rsl-siege-manager) — a real-world web app with a Python (FastAPI) backend, a TypeScript (React + Vite) frontend, and a Python Discord bot, all in one repo. Captured here as a worked example of running map.mmd on a mixed-language full-stack codebase that includes a substantial test suite.
...
> The PyPI package is `map-mmd` (double-y). The CLI command is `map.mmd`.
...
This case study ran extraction with no `.map.mmdignore` (the run included tests). To reproduce a tests-included run:

```powershell
map.mmd extract .
```

Watch terminal output. map.mmd uses tree-sitter for code (free, fast) and LLM API calls only for non-code files (markdown, PDFs, images). On a code-heavy repo like rsl-siege-manager, the cost is dominated by docs.

Check the cost summary when it finishes:

```powershell
Get-Content .\map.mmd-out\cost.json
```
...
code .\map.mmd-out\GRAPH_REPORT.md

# The interactive graph
Start-Process .\map.mmd-out\graph.html
```
...
| `GRAPH_REPORT.md` | Raw map.mmd report (god nodes, communities, surprising connections, suggested questions) |
| `graph.html` | Interactive force-directed visualization |
| `graph.json` | Underlying graph data used by `map.mmd query` |
| `manifest.json` | Per-file extraction record |

The AST cache (`map.mmd-out/cache/`) is regenerable and not committed.

## Why this corpus

rsl-siege-manager is structurally interesting for map.mmd evaluation because:
...
- map.mmd repo: https://github.com/getwinharris/map.mmd
- map.mmd PyPI: https://pypi.org/project/map-mmd/
