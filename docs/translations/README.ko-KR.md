...
**AI 코딩 어시스턴트를 위한 스킬.** Claude Code, Codex, OpenCode, OpenClaw, Factory Droid, 또는 Trae에서 `/map.mmd`를 입력하면 파일을 읽고 지식 그래프를 구축하여, 미처 몰랐던 구조를 보여줍니다. 코드베이스를 더 빠르게 이해하고, 아키텍처 결정의 "이유"를 찾아보세요.

완전한 멀티모달 지원. 코드, PDF, 마크다운, 스크린샷, 다이어그램, 화이트보드 사진, 심지어 다른 언어로 된 이미지까지 — map.mmd는 Claude Vision을 사용하여 이 모든 것에서 개념과 관계를 추출하고 하나의 그래프로 연결합니다. tree-sitter AST를 통해 20개 언어를 지원합니다(Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia).

> Andrej Karpathy는 논문, 트윗, 스크린샷, 메모를 모아두는 `/raw` 폴더를 관리합니다. map.mmd는 바로 그 문제에 대한 답입니다 — 원본 파일을 직접 읽는 것 대비 쿼리당 토큰 소비가 71.5배 적고, 세션 간에 영속적이며, 발견한 것과 추측한 것을 정직하게 구분합니다.

```
/map.mmd .                        # 어떤 폴더든 동작 - 코드베이스, 노트, 논문, 무엇이든
```
...
그래프에 포함하지 않을 폴더를 제외하려면 `.map.mmdignore` 파일을 추가하세요:
...
`.gitignore`와 동일한 문법입니다. 패턴은 map.mmd를 실행한 폴더 기준의 상대 경로에 대해 매칭됩니다.

## 동작 원리

map.mmd는 두 번의 패스로 실행됩니다. 첫 번째는 결정론적 AST 패스로, 코드 파일에서 구조(클래스, 함수, 임포트, 콜 그래프, docstring, 근거 주석)를 LLM 없이 추출합니다. 두 번째는 Claude 서브에이전트가 문서, 논문, 이미지에 대해 병렬로 실행되어 개념, 관계, 설계 근거를 추출합니다. 결과는 NetworkX 그래프로 병합되고, Leiden 커뮤니티 탐지로 클러스터링되며, 인터랙티브 HTML, 쿼리 가능한 JSON, 그리고 일반 언어 감사 보고서로 내보내집니다.
...
> PyPI 패키지는 `map.mmd` 이름을 되찾는 동안 임시로 `map-mmd`로 명명되어 있습니다. CLI와 스킬 명령은 여전히 `map.mmd`입니다.
...
| Claude Code (Windows) | `map.mmd install` (자동 감지) 또는 `map.mmd install --platform windows` |
...
참고: Codex는 스킬 호출에 `/` 대신 `$`를 사용하므로 `$map.mmd .`라고 입력하세요.
...
**Claude Code**는 두 가지를 수행합니다: 아키텍처 질문에 답하기 전에 `map.mmd-out/GRAPH_REPORT.md`를 읽도록 Claude에게 지시하는 `CLAUDE.md` 섹션을 작성하고, 모든 Glob 및 Grep 호출 전에 실행되는 **PreToolUse 훅**(`settings.json`)을 설치합니다. 지식 그래프가 존재하면 Claude는 다음 메시지를 보게 됩니다: _"map.mmd: Knowledge graph exists. Read GRAPH_REPORT.md for god nodes and community structure before searching raw files."_ — 이를 통해 Claude는 모든 파일을 grep하는 대신 그래프를 통해 탐색합니다.
...
제거는 대응하는 uninstall 명령으로 수행합니다(예: `map.mmd claude uninstall`).
...
`/map.mmd query`, `/map.mmd path`, `/map.mmd explain`은 더 깊이 들어갑니다: 원시 `graph.json`을 홉 단위로 순회하고, 노드 간의 정확한 경로를 추적하며, 엣지 수준의 세부 정보(관계 유형, 신뢰도 점수, 소스 위치)를 보여줍니다. 일반적인 오리엔테이션이 아닌 그래프에서 특정 질문에 답하고 싶을 때 사용하세요.

이렇게 생각하면 됩니다: 상시 작동 훅은 어시스턴트에게 지도를 주고, `/map.mmd` 명령은 그 지도를 정확하게 탐색하게 합니다.

## `graph.json`을 LLM과 함께 사용하기

`graph.json`은 프롬프트에 한 번에 전부 붙여넣기 위한 것이 아닙니다. 유용한 워크플로우는 다음과 같습니다:

1. `map.mmd-out/GRAPH_REPORT.md`로 높은 수준의 개요를 파악합니다.
2. `map.mmd query`를 사용하여 답하려는 특정 질문에 대한 더 작은 서브그래프를 가져옵니다.
3. 전체 원시 코퍼스 대신 그 집중된 결과를 어시스턴트에게 제공합니다.

예를 들어, 프로젝트에서 map.mmd를 실행한 후:

```bash
map.mmd query "show the auth flow" --graph map.mmd-out/graph.json
...
어시스턴트가 도구 호출이나 MCP를 지원하는 경우, 텍스트를 붙여넣는 대신 그래프를 직접 사용하세요. map.mmd는 `graph.json`을 MCP 서버로 노출할 수 있습니다:
...
```
/map.mmd                          # 현재 디렉토리에서 실행
/map.mmd ./raw                    # 특정 폴더에서 실행
/map.mmd ./raw --mode deep        # 더 적극적인 INFERRED 엣지 추출
/map.mmd ./raw --update           # 변경된 파일만 재추출하여 기존 그래프에 병합
/map.mmd ./raw --cluster-only     # 기존 그래프의 클러스터링만 재실행, 재추출 없음
/map.mmd ./raw --no-viz           # HTML 건너뛰기, 보고서 + JSON만 생성
/map.mmd ./raw --obsidian                          # Obsidian 볼트도 생성 (옵트인)
/map.mmd ./raw --obsidian --obsidian-dir ~/vaults/myproject  # 볼트를 특정 디렉토리에 생성

/map.mmd add https://arxiv.org/abs/1706.03762        # 논문 가져오기, 저장, 그래프 업데이트
/map.mmd add https://x.com/karpathy/status/...       # 트윗 가져오기
/map.mmd add https://... --author "Name"             # 원저자 태그
/map.mmd add https://... --contributor "Name"        # 코퍼스에 추가한 사람 태그

/map.mmd query "어텐션과 옵티마이저를 연결하는 것은?"
/map.mmd query "어텐션과 옵티마이저를 연결하는 것은?" --dfs   # 특정 경로 추적
/map.mmd query "어텐션과 옵티마이저를 연결하는 것은?" --budget 1500  # N 토큰으로 제한
/map.mmd path "DigestAuth" "Response"
/map.mmd explain "SwinTransformer"

/map.mmd ./raw --watch            # 파일 변경 시 그래프 자동 동기화 (코드: 즉시, 문서: 알림)
/map.mmd ./raw --wiki             # 에이전트가 크롤 가능한 위키 빌드 (index.md + 커뮤니티별 문서)
/map.mmd ./raw --svg              # graph.svg 내보내기
/map.mmd ./raw --graphml          # graph.graphml 내보내기 (Gephi, yEd)
/map.mmd ./raw --neo4j            # Neo4j용 cypher.txt 생성
/map.mmd ./raw --neo4j-push bolt://localhost:7687    # 실행 중인 Neo4j 인스턴스에 직접 푸시
/map.mmd ./raw --mcp              # MCP stdio 서버 시작
...
# 상시 작동 어시스턴트 지시 - 플랫폼별
map.mmd claude install            # CLAUDE.md + PreToolUse 훅 (Claude Code)
...
map.mmd query "어텐션과 옵티마이저를 연결하는 것은?"
map.mmd query "인증 흐름 보기" --dfs
map.mmd query "CfgNode이 뭐지?" --budget 500
...
| 오피스 | `.docx .xlsx` | 마크다운으로 변환 후 Claude를 통해 추출 (`pip install map-mmd[office]` 필요) |
| 논문 | `.pdf` | 인용 마이닝 + 개념 추출 |
...
**Git 훅** (`map.mmd hook install`) - post-commit 및 post-checkout 훅을 설치합니다. 모든 커밋과 브랜치 전환 후 그래프가 자동으로 재빌드됩니다. 재빌드가 실패하면 훅이 0이 아닌 코드로 종료하여 git이 에러를 표시하고 조용히 계속 진행하지 않습니다. 백그라운드 프로세스가 필요 없습니다.
...
| map.mmd 소스 + Transformer 논문 | 4 | **5.4x** | [`worked/mixed-corpus/`](worked/mixed-corpus/) |
...
map.mmd는 문서, 논문, 이미지의 의미적 추출을 위해 파일 내용을 AI 코딩 어시스턴트의 기반 모델 API로 전송합니다 — Anthropic(Claude Code), OpenAI(Codex), 또는 사용 중인 플랫폼의 제공자. 코드 파일은 tree-sitter AST를 통해 로컬에서 처리됩니다 — 코드의 경우 파일 내용이 사용자의 머신을 벗어나지 않습니다. 어떠한 텔레메트리, 사용 추적, 분석도 없습니다. 유일한 네트워크 호출은 추출 중 플랫폼 모델 API에 대한 것이며, 사용자 본인의 API 키를 사용합니다.
...
map.mmd는 그래프 레이어입니다. 그 위에 [Penpax](https://safishamsi.github.io/penpax.ai)를 개발하고 있습니다 — 회의, 브라우저 기록, 파일, 이메일, 코드를 하나의 지속적으로 업데이트되는 지식 그래프로 연결하는 온디바이스 디지털 트윈입니다. 클라우드 없음, 데이터 학습 없음. [대기 목록에 등록하세요.](https://safishamsi.github.io/penpax.ai)

## 스타 히스토리

[![Star History Chart](https://starchart.cc/getwinharris/map.mmd.svg)](https://starchart.cc/getwinharris/map.mmd)

<details>
<summary>기여하기</summary>

**실전 예제**는 가장 신뢰를 쌓는 기여 방식입니다. 실제 코퍼스에서 `/map.mmd`를 실행하고, 결과를 `worked/{slug}/`에 저장하고, 그래프가 맞게 파악한 것과 틀린 것을 평가하는 솔직한 `review.md`를 작성하여 PR을 제출하세요.

**추출 버그** - 입력 파일, 캐시 엔트리(`map.mmd-out/cache/`), 그리고 누락되거나 날조된 내용과 함께 이슈를 열어주세요.
