...
Adicione um arquivo `.map.mmdignore` para excluir pastas:
...
> **Pacote oficial:** O pacote PyPI chama-se `map-mmd` (instalar com `pip install map-mmd`). Outros pacotes chamados `map.mmd*` no PyPI não são afiliados a este projeto. O único repositório oficial é [getwinharris/map.mmd](https://github.com/getwinharris/map.mmd).
...
| Claude Code (Windows) | `map.mmd install` (detecção automática) ou `map.mmd install --platform windows` |
...
Nota: Codex usa `$` em vez de `/` para habilidades, então digite `$map.mmd .`.
...
```
/map.mmd                          # diretório atual
/map.mmd ./raw                    # pasta específica
/map.mmd ./raw --mode deep        # extração de arestas INFERRED mais agressiva
/map.mmd ./raw --update           # re-extrair apenas arquivos modificados
/map.mmd ./raw --directed         # grafo dirigido
/map.mmd ./raw --cluster-only     # re-executar clustering no grafo existente
/map.mmd ./raw --no-viz           # sem HTML, apenas relatório + JSON
/map.mmd ./raw --obsidian         # gerar vault do Obsidian (opt-in)

/map.mmd add https://arxiv.org/abs/1706.03762   # buscar um paper
/map.mmd add <video-url>                         # baixar áudio, transcrever, adicionar
/map.mmd query "o que conecta Attention ao otimizador?"
/map.mmd path "DigestAuth" "Response"
/map.mmd explain "SwinTransformer"

map.mmd hook install              # instalar hooks do Git
map.mmd update ./src              # re-extrair arquivos de código, sem LLM
map.mmd watch ./src               # atualização automática do grafo
```
...
**Hooks do Git** (`map.mmd hook install`) — instala hooks post-commit e post-checkout.

## Privacidade

map.mmd envia conteúdo de arquivos para a API do modelo do seu assistente IA para extração semântica de documentos, papers e imagens. Arquivos de código são processados localmente via tree-sitter AST. Arquivos de vídeo e áudio são transcritos localmente com faster-whisper. Sem telemetria, sem rastreamento de uso.
...
## Construído sobre map.mmd — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) é a camada enterprise sobre o map.mmd. Onde o map.mmd transforma uma pasta de arquivos em um grafo de conhecimento, o Penpax aplica o mesmo grafo a toda a sua vida profissional — continuamente.
