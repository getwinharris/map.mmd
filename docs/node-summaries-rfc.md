...
This RFC proposes an optional way for map.mmd to expose compact file-level
...
`map.mmd query`, `map.mmd explain`, MCP node lookup, and graph navigation.

## Goals

- Help agents choose relevant files with less context.
- Preserve map.mmd's offline, deterministic behavior by default.
...
- Replacing `map.mmd explain`; summaries should make `explain` more useful.
...
- Display a summary in `map.mmd explain <node>` when one exists.
- Include summaries in `map.mmd serve` / MCP node lookup when one exists.
...
{
  "label": "extract.py",
  "source_file": "map.mmd/extract.py",
...
be fetched through `map.mmd explain`, `map.mmd query`, or direct file reads
when needed.
...
```bash
map.mmd . --summarize-nodes
map.mmd explain "extract.py"
```
...
3. Surface summaries in `map.mmd explain`.
4. Surface summaries in `map.mmd serve` / MCP node lookup.
...
- Extend summaries to class or module-level nodes if file-level summaries prove
  useful.
- Allow `map.mmd query` to include summaries for returned nodes under a budget.
- Add cache/freshness metadata if summaries are generated separately from the
  main graph.

## Questions for maintainers and users

1. Should map.mmd prefer one artifact (`graph.json`) or keep generated text in a
   sidecar?
2. Should deterministic file-level summaries be generated during graph creation,
   or only through an explicit command such as `map.mmd summarize`?
