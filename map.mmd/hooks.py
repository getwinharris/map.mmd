...
if [ -z "$GRAPHIFY_PYTHON" ]; then
    GRAPHIFY_BIN=$(command -v mapmmd 2>/dev/null)
    if [ -n "$GRAPHIFY_BIN" ]; then
        ...
        if [ -n "$GRAPHIFY_PYTHON" ] && ! "$GRAPHIFY_PYTHON" -c "import mapmmd" 2>/dev/null; then
            GRAPHIFY_PYTHON=""
        fi
    fi
fi
...
try:
    ...
    _root = Path('.')
    _out = os.environ.get('GRAPHIFY_OUT', 'mapmmd-out')
...
__REBUILD_BODY__
'''
_log = os.environ.get('GRAPHIFY_REBUILD_LOG') or os.path.join(os.path.expanduser('~'), '.cache', 'mapmmd-rebuild.log')
try:
...
_GRAPHIFY_LOG="${HOME}/.cache/mapmmd-rebuild.log"
mkdir -p "$(dirname "$_GRAPHIFY_LOG")"
export GRAPHIFY_REBUILD_LOG="$_GRAPHIFY_LOG"
echo "[mapmmd hook] launching background rebuild (log: $_GRAPHIFY_LOG)"
...
mkdir -p "$(dirname "$_GRAPHIFY_LOG")"
export GRAPHIFY_REBUILD_LOG="$_GRAPHIFY_LOG"
echo "[mapmmd] Branch switched - launching background rebuild (log: $_GRAPHIFY_LOG)"
