...
- Fix: the `Read|Glob` PreToolUse hook (the "run mapmmd first" nudge installed for Claude Code and CodeBuddy) now matches the file's real trailing extension instead of substring-scanning the path (#1463, thanks @marketechniks). The old check asked `any(ext in path)`, which had two opposite failures: `.json` files (`package.json`, `tsconfig.json`) spuriously fired because `.js` is a substring of `.json`, and `.astro`/`.vue`/`.svelte` never fired because they weren't in the set — so on Astro/Vue/Svelte projects, where those are the primary source type, reads and globs never surfaced the graph. The hook now compares the segment after the last `/` then after the last `.` against the extension set (with `.astro`/`.vue`/`.svelte` added), so `package.json` stays silent, `data.geojson` stays silent, `**/*.astro` fires, and an extension sitting on a directory component (`my.ts/file`) correctly doesn't. The `mapmmd-out/` suppression and fail-open behavior are unchanged.
- Fix: make it unambiguous in the skill that mapmmd needs no API key, so terminal-style hosts stop looping on a missing one (#1461). Hermes (and the other AGENTS.md hosts: Codex, Aider, OpenClaw, Droid, Trae, …) run the `mapmmd` CLI directly and don't dispatch subagents, but the Step 3 extraction guidance framed the no-key path only as "fall through to subagent dispatch" — so on `/mapmmd .` those agents would spin for minutes insisting they needed an API key before eventually proceeding. Step 3 now opens with an explicit, hoisted "mapmmd needs no API key — never ask the user for one, never block on one" statement (code is AST-only; a code-only corpus skips semantic extraction entirely), and the fallback now spells out a non-subagent path for terminal hosts instead of assuming subagent dispatch. Applied across every generated skill body, including the aider/devin monoliths, with a regression test that pins the wording in place.
...
- Feat: new first-class `agents` platform installs the skill to the generic cross-framework Agent-Skills locations. `mapmmd install --platform agents` (alias `--platform skills`) writes the spec's user-global `~/.agents/skills/mapmmd/SKILL.md` — the directory `npx skills` and spec-compliant frameworks read — and `--project` writes `./.agents/skills/mapmmd/SKILL.md`; `mapmmd uninstall` removes them. Previously that user-global location was only reachable as an accidental side effect of the gemini-on-Windows branch. The skill bundle re-homes amp's agents-md body (registered in `tools/skillgen/platforms.toml`, rendered through the skillgen drift/coverage guards); the body is identical to amp's, and only the on-demand hooks reference differs — it points at `mapmmd agents install`, which (as the amp-twin subcommand) wires the skill plus an AGENTS.md always-on section. Bare `mapmmd install` is unchanged — still single-platform (claude/windows) (#1432, closes #1405).
...
- Fix: `mapmmd install --platform hermes` now installs to the right directory on Windows. Hermes scans `%LOCALAPPDATA%\hermes\skills`, but the installer always used the POSIX `~/.hermes/skills` (so on Windows the skill was never discovered). `_platform_skill_destination` gained a hermes branch that targets `%LOCALAPPDATA%\hermes\skills` on Windows and keeps `~/.hermes/skills` elsewhere (#1403).
...
- Fix: the opencode plugin's search reminder no longer contains backticks. The hook prepends the reminder inside a double-quoted `echo`, where bash performed command substitution on the backticks — silently running `mapmmd query "<question>"` on every search. The reminder is now plain text (#1413; thanks @WSHAPER).
- Fix: `mapmmd extract --cargo` exits with a clear error instead of a traceback when `Cargo.toml` is missing or unreadable (the introspection now also catches `OSError`) (#1428; thanks @DhruvTilva).
...
- Fix: `mapmmd hook install` no longer creates a literal backslash-named junk directory and reports false success when `core.hooksPath` (or `git rev-parse --git-path hooks`) is a Windows-style path under WSL. Drive-letter / embedded-backslash hooks paths are now rejected with a clear error (#1385).
- Refactor: node-ID normalization is unified into a single `mapmmd.ids` module. `extract._make_id`, `build._normalize_id`, `mcp_ingest._make_id`, and `symbol_resolution._bash_make_id` were four hand-synced copies of the same NFKC/casefold recipe — the root of the recurring ghost-node bug class (#811/#550/#1033/#1104). All four now delegate to one implementation guarded by contract + hypothesis property tests (#1378; thanks @danielnguyenfinhub).
...
- Security fix: `.mapmmdignore` and `.gitignore` are now **merged** per directory instead of `.mapmmdignore` silently replacing that directory's `.gitignore`. Previously, adding a `.mapmmdignore` (e.g. to exclude media) disabled the dir's `.gitignore` entirely, so a file excluded only by `.gitignore` — including neutrally-named secrets like `prod-dump.sql` or `customer-data.json` that the sensitive-file heuristic doesn't catch — got indexed into the graph, whose artifacts embed file contents and are routinely committed. `.gitignore` is read first and `.mapmmdignore` last, so `.mapmmdignore` patterns (including `!` negations) still win on conflict; adding one can only ever exclude more, never re-include a `.gitignore`-excluded file. (#1363)

## 0.8.41 (2026-06-17)

- Fix: OpenAI-compatible backends (`ollama`, `openai`, `deepseek`, `kimi`) now honour their configured `16384` output-token cap instead of silently falling back to `8192`. The dispatch read a `max_completion_tokens` config key that only `gemini` defines; the others set `max_tokens`, so their advertised cap was dead and deep-mode JSON truncated mid-string (recovered by the adaptive bisect, but noisy and slower). The dispatch now reads either key, and the `openai` config gained an explicit cap. `MAPMMD_MAX_OUTPUT_TOKENS` still overrides. (#1365)
...
- Feat: custom OpenAI- and Anthropic-compatible endpoints via `OPENAI_BASE_URL`/`OPENAI_MODEL` and `ANTHROPIC_BASE_URL`/`ANTHROPIC_MODEL`. Point either backend at a self-hosted or proxy server (vLLM, llama.cpp, LM Studio, LiteLLM, gateways); defaults still resolve to `api.openai.com` / `api.anthropic.com`, and `MAPMMD_OPENAI_MODEL` keeps precedence over `OPENAI_MODEL`. Wired through both the extraction path (`_call_claude`) and community labeling (#1273).
...
- Fix: `--no-cluster` and incremental `mapmmd update` no longer accumulate duplicate edges. These paths bypass the NetworkX `Dimmd` that collapses parallel edges, so repeated `update` grew the edge count every run and counts diverged across build modes. Edges are now deduped by `(source, target, relation)` before writing — deterministic and idempotent; the no-cluster rebuild log now reports the written (deduped) edge count instead of the raw pre-merge count (#1317).
...
- Fix: `mapmmd query`, `mapmmd explain`, and MCP `query_graph`/`get_node` now show the human-readable community name (e.g. "FlashAttention Paper") instead of a blank or numeric ID after running `cluster-only`. `to_json` now accepts `community_labels` and embeds `community_name` on each node; read paths fall back to the numeric `community` field for backward compatibility with old graphs (#1305).
- Fix: `mapmmd-mcp` and `python -m mapmmd.serve` now accept `--graph <path>` as an alias for the positional argument, consistent with every other mapmmd subcommand. Previously `--graph` raised "unrecognized arguments" (#1304).
...
- Fix: AST extraction no longer crashes on Windows machines with >61 logical cores. `ProcessPoolExecutor` on Windows is hard-capped at 61 workers via `WaitForMultipleObjects`; the clamp now applies to all three input paths (auto-compute, `MAPMMD_MAX_WORKERS`, `--max-workers`) (#1298).
...
- Fix: startup no longer crashes on unreadable `.mapmmd_version` files. On restricted-permission installs or network mounts, `.exists()` / `.read_text()` raised `PermissionError` and crashed every `mapmmd query/explain/path` call. All three FS probes now wrapped in `try/except OSError: return` (#1299).
...
- Feat: FalkorDB export backend. `mapmmd export falkordb --push redis://localhost:6379` pushes the graph to a FalkorDB instance. Optional dep (`uv tool install "mapmmdy[falkordb]"`); lazy import; idempotent (MERGE semantics); Cypher injection guarded.

- Fix: `affected` and `mapmmd query` now handle graph files that use `"edges"` as the top-level key instead of `"links"`. mmds produced by native `mapmmd extract` on some corpus layouts used `"edges"`; loading them in `affected.py` raised `KeyError: 'links'`. Normalised using the same established pattern already in `__main__.py` and `serve.py`.
- Fix: a single `!` negation rule in `.mapmmdignore` no longer disables all directory pruning. Previously any negation pattern caused `collect_files` to descend every ignored directory to look for re-included files. Since gitignore semantics cannot rescue files beneath an excluded parent, this descent was always wasted — the per-file filter still excluded them. Pruning now proceeds unconditionally; only the final per-file `_is_ignored` check is consulted for negation.
- Feat: `--model` flag added to `mapmmd label-communities` and `mapmmd cluster-only`. Routes through `generate_community_labels` → `label_communities` → `_call_llm`; defaults to `None` (keeps existing backend default). Also fixes a latent arg-parsing bug where `--backend gemini` (space-separated) was mis-parsed as the positional path argument.
...
- Fix: AST cache is now namespaced by mapmmd version. Stale AST cache entries from a previous release no longer silently produce wrong extraction output after an upgrade — each version writes to its own subdirectory under `mapmmd-out/cache/ast/<version>/`. Semantic cache is deliberately unversioned (LLM calls are expensive to re-run).
...
- Fix: extraction cache is anchored at the `--out` directory root instead of leaking a `mapmmd-out/` into the scanned project. Using `mapmmd extract ./src --out /tmp/out` previously wrote cache files into `./src/mapmmd-out/cache/` alongside source files. Cache now writes exclusively under the `--out` path.
...
- Fix: OpenAI-compatible backends no longer send `temperature=0` to reasoning models. `_resolve_temperature()` auto-detects o1/o3/o4 and gpt-5 series and omits temperature from the request. Override with `MAPMMD_LLM_TEMPERATURE=<value>` (or `none` to omit explicitly for any model).
...
- Feat: `MAPMMD_MAX_GRAPH_BYTES` env var to override the 512 MiB `graph.json` size cap. Accepts plain bytes, `<N>MB`, or `<N>GB`. The cap error message now cites this env var. `mapmmd export html` auto-falls back to the community-aggregation view when over cap instead of hard-failing.
- Feat: `CLAUDE.md` template now uses mandatory language for the mapmmd-first rule — "MANDATORY: Before using Read/Grep/Glob/Bash to explore the codebase, you MUST run mapmmd first" — and explicitly requires forwarding the rule to subagent prompts. PreToolUse hook message hardened to match.
- CI: Release workflow added. Every GitHub release now ships `mapmmd-self-graph.tar.gz` as a downloadable asset — `graph.json` + `graph.html` + `GRAPH_REPORT.md` from running mapmmd on its own source. Open `graph.html` locally with no install required to see what mapmmd produces.
...
- Feat: `mapmmd-mcp` console script. MCP stdio server now directly invocable as `mapmmd-mcp` from `uv tool install` / `pipx`. Thanks to @jr2804 (#1190).
- Fix: `label_communities` token budget raised and `MAPMMD_MAX_OUTPUT_TOKENS` now honoured. Hardcoded `min(40+16n, 4096)` undershooted (~16 tok/community); raised to `min(64+24n, 8192)` and wrapped in `_resolve_max_tokens()` (#1200).
...
- Feat: CodeBuddy platform support. `mapmmd codebuddy install` installs the mapmmd skill to `~/.codebuddy/skills/mapmmd/SKILL.md`, writes a `CODEBUDDY.md` always-on section, and registers Bash + Read|Glob PreToolUse hooks in `.codebuddy/settings.json` that nudge the agent toward `mapmmd query` instead of grepping raw files when a graph exists. `mapmmd install --platform codebuddy` and `mapmmd codebuddy uninstall` also supported. Thanks to @studyzy (#1136).
- Fix: `mapmmd --update` no longer destructively collapses distinct same-named symbols across files. The skill's `--update` merge now passes re-extracted (changed) files to `prune_sources` alongside deleted files, so old nodes for changed files are pruned before fresh AST is inserted — no fuzzy reconciliation needed. Separately, `dedup.py` Pass 1 now skips nodes with an empty `source_file` so label-only merging across no-source-file nodes is prevented. The anti-shrink guard message now names fuzzy dedup as a possible cause rather than only blaming missing chunk files (#1178).

## 0.8.34 (2026-06-07)

- Feat: Streamable HTTP transport for the MCP server. `python -m mapmmd.serve graph.json --transport http --port 8080 --api-key $SECRET` serves the graph over the MCP Streamable HTTP transport (spec 2025-03-26) so a single shared process can serve the whole team. Flags: `--host`, `--port`, `--api-key` (env `MAPMMD_API_KEY`), `--path`, `--json-response`, `--stateless`, `--session-timeout`. Docker image included. stdio remains the default (#1143).
...
- Feat: live PostgreSQL introspection. `mapmmd extract --postgres "postgresql://..."` connects directly to a running database and maps tables, views, routines, and FK relations via `information_schema` in a `SERIALIZABLE READ ONLY` transaction. New `mapmmd[postgres]` extra (psycopg3). Credentials are sanitized from error messages (#1103).
...
- Fix: `mapmmd path` and `shortest_path` now fire the exact-match bonus for multi-word queries. The per-token comparison never equalled a full multi-word label, so the exact bonus was silently skipped for queries like `"AuthService"` when the label contained punctuation or spaces. The full normalized query is now compared alongside each token (#1165).
...
- Fix: post-commit and post-checkout hooks now respect an existing `.mapmmd_root`. A scoped build (`mapmmd src/`) was silently expanded to the full repo on the next commit because the hook hardcoded `Path('.')`. The hook body now reads `mapmmd-out/.mapmmd_root` first (#1173).
- Fix: `mapmmd affected` now forces a directed graph on load, matching the identical fix already applied in `serve.py` and `__main__.py`. On undirected graphs (`"directed": false` in graph.json) the traversal was direction-blind — missing true callers and reporting callees as affected (#1174).
- Fix: Step 9 skill cleanup no longer aborts under fish/zsh on pure-code corpora. The `rm -f ... .mapmmd_chunk_*.json` glob errored with "no matches found" when no chunk files existed, leaving other temp files on disk. Split into `rm -f` for fixed filenames and `find -maxdepth 1 -delete` for the chunk glob (#1172).
...
- Fix: Codex platform skill now installs to `.codex/skills/mapmmd/` (was `.agents/skills/mapmmd/`), aligning with where the hook already lives (#1160).

## 0.8.33 (2026-06-06)

- Feat: FalkorDB export backend — sibling to Neo4j, selected via `mapmmd export falkordb [--push falkordb://localhost:6379]`. FalkorDB is OpenCypher-compatible, so the MERGE/SET upsert queries match the Neo4j path; auth is optional and the target graph defaults to `mapmmd`. Install with `uv tool install "mapmmdy[falkordb]"` (#1175).
- Feat: install banner — `mapmmd install` now prints an amber knowledge-graph brain in the terminal (TTY-only, silent in CI/pipes, never raises).
...
- Fix: AST/semantic ghost-duplicate nodes are now auto-merged at build time. When AST and semantic extraction produce different IDs for the same symbol (one with `source_location=L<n>`, one without), `build_from_json` detects the pair by `(source_file basename, label)` and collapses the semantic ghost into the AST node, re-pointing all edges. mmds built before this release can be cleaned up with `mapmmd extract . --force` (#1145).

## 0.8.32 (2026-06-05)

- Feat: Terraform/HCL support. `.tf`, `.tfvars`, and `.hcl` files are now AST-extracted via `tree-sitter-hcl` into a structured infrastructure dependency graph. Nodes: resources, data sources, modules, variables, outputs, providers, and locals. Edges: `contains`, `references` (interpolation), and `depends_on`. Node IDs are directory-scoped for cross-file resolution. Requires `uv tool install "mapmmdy[terraform]"` (#1129).
- Fix: `mapmmd extract` no longer requires an LLM API key for code-only corpora. Backend resolution is now deferred until after file detection — a corpus with only code files (pure tree-sitter AST, zero LLM calls) runs fully offline. The key is only enforced when docs, PDFs, or images are present, or when `--dedup-llm` is passed (#1122).
- Fix: `mapmmd kiro install` now correctly installs the `references/` sidecar and `.mapmmd_version` stamp. The install was using a bare `write_text` that bypassed the shared helper, shipping `SKILL.md` with 8 dead `references/*.md` pointers. Re-run `mapmmd kiro install` to pick up the fix (#1142).
- Fix: `MAPMMD_API_TIMEOUT` now applies to `claude-cli` subprocess and Anthropic SDK backend, not just the HTTP client. Both subprocess paths previously hardcoded `timeout=600` and ignored the env var and `--api-timeout` flag (#1112).
...
- Fix: `mapmmd hook install` now embeds the current interpreter (`sys.executable`) directly into the generated hook scripts. Previously, uv tool and pipx installs silently no-oped on git commit in GUI clients and CI runners where `~/.local/bin` is not on PATH — the hook could not find the mapmmd launcher, fell through all detection probes, and exited 0 without rebuilding. The embedded path is sanitized through a filesystem-safe allowlist before substitution. If you already have hooks installed, re-run `mapmmd hook install` to pick up the fix (#1127).
- Fix: hook scripts now also probe `mapmmd-out/.mapmmd_python` as a fallback interpreter source, covering Windows/Git Bash installs where the launcher is a binary with no parseable shebang, and the case where the pinned path goes stale after a reinstall.
- Security: hook script hardening — the `_PINNED=` assignment uses single quotes to prevent shell injection from a path containing metacharacters; `nohup "$MAPMMD_PYTHON" -c` is properly quoted to handle spaces; the fallback emits a loud stderr diagnostic instead of a bare silent `exit 0`.
- Feat: query logging. Every `mapmmd query`, `mapmmd path`, `mapmmd explain`, and MCP `query_graph` call is now appended to `~/.cache/mapmmd-queries.log` in JSON Lines format (timestamp, kind, question, corpus path, nodes returned, result size, duration). Full subgraph responses are not stored by default. Control with `MAPMMD_QUERY_LOG` (path override), `MAPMMD_QUERY_LOG_DISABLE=1` (opt out), `MAPMMD_QUERY_LOG_RESPONSES=1` (store full response text) (#1128).

## 0.8.30 (2026-06-03)

- Fix: `mapmmd install --project --platform antigravity` now writes Antigravity's always-on layer (`.agents/rules/mapmmd.md` + `.agents/workflows/mapmmd.md`), not just the skill. The project-scoped path went through the skill-only branch and skipped them, even though the project uninstall removes them.
- Feat: close the Read-tool graph bypass. The `PreToolUse` nudge previously only fired on Bash search (`grep`/`rg`/`find`); an agent answering a question by reading many source files through the native `Read` tool (or `Glob`) slipped past it. A new `Read|Glob` hook nudges toward `mapmmd query` when `mapmmd-out/graph.json` exists, only for a source/doc file outside `mapmmd-out/`, and never blocks (#1114).
- Feat: add an `anthropic` optional extra (and include it in `[all]`) so the `claude` backend is installable like every other one: `uv tool install "mapmmdy[anthropic]"`. Previously it was the only backend with no extra, so a user with `ANTHROPIC_API_KEY` set could not satisfy it without `--with anthropic`. The backend package-missing errors now point at `uv tool install "mapmmdy[<extra>]"` (the isolated-venv path) rather than only `pip install`.
...
- Fix: `mapmmd install --platform gemini` shipped a `SKILL.md` with 8 dead `references/` pointers. gemini installs claude's lean progressive core but the installer never copied claude's references sidecar; it now does, so every on-demand reference resolves (regression from the progressive-disclosure split).
- Security (F1): a project-local `./.mapmmd/providers.json` (which travels with a cloned or shared repo) is no longer loaded automatically, since a custom provider's `base_url` is where your corpus and API key are sent. Set `MAPMMD_ALLOW_LOCAL_PROVIDERS=1` to opt in; the user's own `~/.mapmmd/providers.json` is still trusted. Non-http(s) `base_url`s are rejected on load and on `provider add`, and plaintext-http egress warns. **Behavior change:** if you relied on an auto-loaded project-local providers file, set the opt-in env var.
...
- Feat: Kilo Code support — `mapmmd install --platform kilo` installs a native skill (`~/.config/kilo/skills/mapmmd/SKILL.md`) and `/mapmmd` command, plus a `.kilo` `tool.execute.before` plugin (mirroring the OpenCode integration). Existing `.kilo/kilo.jsonc` config is read but never rewritten — plugin registration goes to `kilo.json` so user comments are preserved (#512)
...
- Fix: `uv tool install mapmmdy` / `pip install mapmmdy` no longer fails to build on Linux/macOS — `tree-sitter-dm` (BYOND DreamMaker) ships only a Windows wheel, so on other platforms it compiled from source and aborted the entire install when a C toolchain or `python3-dev` headers were missing. It is now an optional extra (`mapmmdy[dm]`, also in `[all]`) instead of a core dependency, so the default install needs no compiler (#1104).
  - **Upgrade note:** DreamMaker `.dm`/`.dme` users must reinstall with `mapmmdy[dm]` (or `[all]`) to keep AST extraction — on `uv tool upgrade` the now-optional grammar is removed. `.dmi`/`.dmm`/`.dmf` parsing is unaffected (no tree-sitter dependency).
...
- Fix: `mapmmd amp install` now writes the skill where Amp actually looks for it. It was landing in `.amp/skills/mapmmd` (project) and `~/.amp/skills/mapmmd` (user), neither of which Amp searches, so the skill never loaded. User-scope installs now go to `~/.config/agents/skills/mapmmd` and project installs to `.agents/skills/mapmmd`, and a stale `~/.amp/skills/mapmmd` from an older install is cleaned up on the next run.

## 0.8.27 (2026-05-31)

- Feat: standalone CLI now auto-names communities with the configured backend instead of leaving `Community N` placeholders — community labeling was previously an agent-only step (skill.md Step 5), so bare-CLI runs never got semantic names; `cluster-only` now auto-labels when no `.mapmmd_labels.json` exists, new `mapmmd label <path>` subcommand (re)generates names on demand, `--no-label` opts out, `--backend=<name>` overrides auto-detection; one batched LLM call with per-community placeholder fallback and graceful degradation on missing backend/API error; works with all built-in and custom OpenAI-compatible backends (#1097)
...
- Feat: custom LLM provider registry — `mapmmd provider add/list/show/remove` registers any OpenAI-compatible endpoint (NVIDIA NIM, vLLM, OpenRouter, Together, LiteLLM) via `~/.mapmmd/providers.json`; custom providers auto-detected after built-ins in `detect_backend()` priority (#1084)
...
- Fix: `pnpm-workspace.yaml` with `packages: - '.'` no longer crashes with `IndexError: tuple index out of range` on Python 3.10 — `Path.glob('.')` replaced with `[root]` guard in `_load_workspace_packages`; `MAPMMD_DEBUG=1` env var added to `_safe_extract` for full traceback on extraction errors (#1083)
- Fix: anchored `.mapmmdignore` patterns (leading `/`) no longer match the same directory name anywhere in the tree — `_matches()` in both `_is_ignored` and `_is_included` now gates basename/segment shortcuts on `not anchored`; anchored patterns do exact anchor-relative path match only (#1087)
...
- Fix: `mapmmd install antigravity` global install now writes to `~/.gemini/config/skills/` (per Antigravity docs) instead of the wrong `~/.agents/`; uninstall, version-stamp refresh, and project-scope install all updated to match (#1079)
...
- Fix: `mapmmd-out/.mapmmd_python` path prefix was missing in 8 skill files (256 instances) causing `cat: .mapmmd_python: No such file or directory` on every non-Claude-Code platform
- Chore: all skill files now use uv-aware interpreter detection — `uv tool run mapmmdy python` preferred over shebang parsing when uv is available
...
- Feat: `mapmmd extract --mode deep` flag enables richer semantic extraction using an extended system prompt; flag propagated through all four LLM backends (#1030)

## 0.8.21 (2026-05-27)

- Fix: `mapmmd update` (no `--changed` flag) no longer leaves ghost nodes from files deleted between runs — full re-extraction path now reconciles the existing graph against current disk state and evicts any node whose `source_file` no longer exists; `_norm_source_file` used on both sides to guarantee path format consistency (#1007)
- Fix: `mapmmd install --platform opencode --project` now writes `SKILL.md` to `.opencode/skills/mapmmd/SKILL.md` (discoverable by OpenCode) instead of the incorrect `.config/opencode/skills/` path; git-add hint updated accordingly (#1040)
- Fix: post-commit hook no longer triggers rebuild when only `mapmmd-out/` files were committed (avoids infinite dirty-tree loop when graph outputs are tracked in git); `MAPMMD_SKIP_HOOK=1` env var added for one-off skip; hook rebuild log now appends (`>>`) instead of overwriting (`>`) (#1018, #1037)
...
- Feat: Amp (ampcode.com) platform support — `mapmmd amp install/uninstall` installs the skill into `.amp/skills/mapmmd/SKILL.md` (#948)
...
- Fix: `mapmmd-out/memory/` files are no longer silently excluded by `.gitignore` pattern matching — memory dir files now bypass the gitignore filter in `detect.py`, ensuring knowledge accumulated via `mapmmd remember` is always scanned (#1047)

## 0.8.20 (2026-05-26)

- Fix: stale nodes persist after `mapmmd update` when files are deleted on Windows — `deleted_paths` and `evict_sources` in `_rebuild_code` now use `.as_posix()` for consistent forward-slash paths; `_relativize_source_files` called on the existing graph before eviction (not after); `_relativize_source_files` itself now produces forward slashes (#1007)
- Fix: `mapmmd extract` stale-node pruning now also handles symlinked scan roots — `prune_set` expansion uses `Path(root).resolve()` before `relative_to()` so symlinked roots produce correct relative paths (#1007)
...
- Fix: `cluster-only` no longer drops community label alignment after re-clustering — `remap_communities_to_previous` now applied in the `cluster-only` path, matching the behaviour of `mapmmd update` (#1028)
...
- Feat: Chinese query segmentation — compound Chinese tokens (e.g. `页面路由`) are split into meaningful words using jieba when installed, with character bigram fallback; original compound preserved alongside segments for exact-match; new `pip install "mapmmdy[chinese]"` extra (#1026)
...
- Feat: Devin CLI support — `mapmmd devin install/uninstall` installs the skill into Devin's `.devin/rules/` directory (#1020)
...
- Fix: `mapmmd export` (html/obsidian/wiki/svg/graphml/neo4j) no longer collapses to "Single community" when `.mapmmd_analysis.json` is absent — falls back to per-node `community` attribute already present in `graph.json` (#1001)
...
- Fix: `mapmmd` now exits non-zero when all semantic-extraction chunks fail — previously a silent empty graph was written with exit code 0, masking backend failures (#889)
- Feat: `mapmmd install --project` installs the skill into the current repository (`.claude/skills/`, `.agents/skills/`, etc.) instead of the user home directory; per-platform subcommands support the same flag (#931)
- Docs: Uzbek (uz-UZ) README translation (#982)

## 0.8.15 (2026-05-22)

- Fix: `cluster-only` subcommand crashed with `FileNotFoundError` when `mapmmd-out/` did not yet exist — output directory is now created before any write (#934)
- Fix: `MAPMMD_MAX_OUTPUT_TOKENS` env var now respected for all OpenAI-compatible backends — previously the token limit was hardcoded, causing truncated responses on high-context queries (#973)
...
- Feat: Multigraph runtime compatibility probe — emits a warning if a `MultiDimmd` is passed where a `mmd` is expected by any downstream consumer (analyze, cluster, wiki, export, report) (#956)
...
- Fix: `.gitignore` patterns now respected when no `.mapmmdignore` exists — previous behaviour silently ignored the project's gitignore, causing expected exclusions to be skipped (#945)
- Feat: `--exclude <pattern>` CLI flag to pass extra gitignore-style exclusion patterns at runtime without modifying `.mapmmdignore` (#947)
...
- Feat: `mapmmd cache-check`, `mapmmd merge-chunks`, `mapmmd merge-semantic` CLI subcommands expose cache and merge logic as library-callable commands for skill pipelines
...
- Fix: `--watch` now respects `.mapmmdignore` — the event handler was checking extensions before the ignore filter, so paths inside `node_modules/`, `.venv/`, etc. triggered rebuilds (#928)
- Fix: `mapmmd <path>` now correctly dispatches to `mapmmd extract <path>` — previously a bare path argument returned "unknown command" instead of starting extraction
- Fix: skill fast path — if `mapmmd-out/graph.json` already exists and the request is a natural-language question, extraction steps are skipped entirely and `mapmmd query` runs immediately; previously the skill re-ran detect and hit the corpus-size gate on every question
...
- Fix: Codex skill now uses mapmmd query/explain/path even when graph artifacts are dirty in worktree (#913, closes #860)
...
- Fix: install guidance now directs users to run `/mapmmd query` interactively rather than reading `GRAPH_REPORT.md` first; the report is a summary, not a starting point (#891)

## 0.8.5 (2026-05-15)

- Fix: `.mapmmdignore` parent-exclusion rule now correctly blocks files under an excluded directory even when a `!` negation exists elsewhere in the file — previously any negation pattern disabled directory pruning entirely (#882)
...
- Fix: Windows skill temp files (chunk JSONs, `.mapmmd_python`, `.mapmmd_root`) no longer pollute the project root — all written under `mapmmd-out/` (#831)
- Fix: `--update` with deletions-only no longer errors when `.mapmmd_extract.json` does not yet exist — creates an empty extraction file before merging (#876)

## 0.8.2 (2026-05-15)

- Fix: Python interpreter detection for `uv tool` and `pipx` installs on Windows — `mapmmd install` and all skill steps now find the correct executable (#831)
- Fix: antigravity Windows skill path resolution (#831)
- Fix: dot directories (e.g. `.github/`, `.vscode/`) are now indexed when explicitly included via `.mapmmdignore` (#873)
...
- Feat: Mermaid callflow HTML auto-regenerates on every graph rebuild when `*-callflow.html` exists in `mapmmd-out/` — works with `--watch` and `mapmmd hook install`
...
- Fix: `mapmmd hook install` now works in git linked worktrees — uses `git rev-parse --git-path hooks` instead of constructing `.git/hooks/` directly (#865)
- Fix: office sidecar files in `mapmmd-out/converted/` are now checked against `.mapmmdignore` before being added to the file list (#861)
- Fix: `save_manifest()` accepts a `kind` parameter (`ast`, `semantic`, `both`) — incremental AST-only `mapmmd update` no longer overwrites `semantic_hash` entries, preventing spurious full re-extracts on the next run (#857)
- Fix: five paths in `skill-windows.md` Step B3 were missing the `mapmmd-out/` prefix, causing chunk files to be written to the wrong directory (#862)
...
- Docs: skill.md now clarifies that mapmmd does not read `ANTHROPIC_API_KEY` or other provider keys during `/mapmmd` skill runs — the host IDE session provides the LLM (PR #864)

## 0.7.18 (2026-05-14)

- Fix: `mapmmd update` is now idempotent — graph.json and GRAPH_REPORT.md are only rewritten when content actually changes; topology comparison short-circuits clustering entirely on unchanged graphs, eliminating residual community-count drift (#824)
- Fix: community IDs are now stable across rebuilds — Leiden/Louvain receive deterministically sorted input and a fixed random seed; greedy overlap remapper preserves existing IDs so hand-edited `.mapmmd_labels.json` labels don't drift onto wrong communities (#824)
- Fix: `--no-cluster` flag added to `mapmmd update` — writes raw AST graph without clustering, consistent with `mapmmd extract --no-cluster` (#824)
- Fix: `mapmmd update --no-cluster` now writes `"links"` key matching the schema of the full clustered path; previously wrote `"edges"`, causing schema toggle on every mode switch
- Fix: `.mapmmd_labels.json` was rewritten on every rebuild even when nothing changed; now only written when outputs actually change
...
- Fix: node ID format in skill.md corrected to `{parent_dir}_{filename_stem}_{entity}` — the old filename-only format caused ghost-duplicate nodes when AST and semantic extractors disagreed on the stem; top-level files use just the filename stem; existing graphs with ghost duplicates can be cleaned up with `mapmmd extract --force`
...
- Fix: `mapmmd path` and `mapmmd explain` now render arrow direction correctly — `-->` for caller→callee, `<--` for callee←caller; previously the graph was loaded undirected so every hop printed `-->` regardless of stored direction (#849, #853)
...
- Fix: `mapmmd extract --backend bedrock` was rejected by the CLI guard even when `AWS_PROFILE`/`AWS_REGION`/`AWS_DEFAULT_REGION`/`AWS_ACCESS_KEY_ID` were set — boto3 session auth was never reached (#846)
...
- Fix: Step 1 install fallback in skill now prefers `uv tool install --upgrade mapmmdy` over `pip` when uv is on PATH — pip was installing to the wrong environment when mapmmd was originally installed via `uv tool` (#831)
- Fix: `_score_nodes` in `serve.py` now uses three-tier precedence (exact 1000 / prefix 100 / substring 1) instead of flat substring scoring — `mapmmd path "Foo" "FooBar"` no longer returns 0 hops when both labels substring-match the same node (#828)
- Fix: `mapmmd path` and MCP `_tool_shortest_path` now emit a clear error when source and target resolve to the same node, instead of silently returning 0 hops (#828)
...
- Fix: `_AGENTS_MD_SECTION` now includes the `/mapmmd` skill trigger instruction — all 7 AGENTS.md platforms (OpenCode, Codex, Aider, Trae, Hermes, OpenClaw, Factory Droid) now correctly invoke the skill tool when the user types `/mapmmd` (#827)

## 0.7.15 (2026-05-11)

- Fix: `-h`/`--help`/`-?` in any position now stops execution — previously `mapmmd cursor install --help` silently installed into Cursor; `mapmmd benchmark --help` crashed with FileNotFoundError (#821)
- Fix: `--version`, `-v`, and `mapmmd version` now print the installed version and exit (#818)
- Fix: `MAPMMD_OLLAMA_NUM_CTX=<invalid>` no longer falls back to hardcoded 131072 (which exhausted VRAM) — it now falls through to the auto-derived value and prints a warning (#820)
- Fix: when `MAPMMD_OLLAMA_NUM_CTX` is set smaller than the estimated chunk size, mapmmd now warns explicitly that Ollama will silently truncate the prompt and suggests a corrected `--token-budget` (#820)
...
- Fix: subagent chunk files are now written to an absolute path (`CHUNK_PATH` injected at dispatch time from `mapmmd-out/.mapmmd_root`) so the Write tool doesn't lose chunks to an undefined working directory (#808)
...
- Fix: hollow-response warning now mentions VRAM pressure and `MAPMMD_OLLAMA_NUM_CTX` / `MAPMMD_OLLAMA_KEEP_ALIVE` env vars as tuning knobs (#798)
- Feat: `mapmmd export callflow-html` -- generates a self-contained Mermaid architecture/call-flow HTML page from `mapmmd-out/graph.json`, grouped by community with interactive zoom/pan diagrams, call detail tables, and graph report highlights (#797)
...
- Fix: `mapmmd explain` and `mapmmd path` no longer crash on `Multimmd` inputs -- new `edge_data()`/`edge_datas()` helpers in `build.py` handle both simple and multi-graphs; all 8 production call sites and 30 skill-file inline heredocs updated (#796)
...
- Fix: post-commit hook no longer spawns unbounded parallel rebuilds -- per-repo `fcntl.flock` non-blocking lock in `_rebuild_code`; `changed_paths` wired from hook through to AST extractor; stale nodes evicted on deletion; `MAPMMD_REBUILD_TIMEOUT` watchdog; Darwin-aware memory cap (#791)
...
- Feat: `mapmmd extract` gains `--max-workers`, `--token-budget`, `--max-concurrency`, `--api-timeout` flags; hard 8-worker AST cap removed; explicit HTTP timeout on OpenAI client (default 600s, `MAPMMD_API_TIMEOUT`); ollama API key gate skipped for loopback URLs (#792)
...
- Feat: `/mapmmd --help` now prints the Usage block and stops without running pipeline steps (all 12 skill files) (#795)
...
- Feat: `mapmmd uninstall` top-level command -- removes mapmmd skill files from all platforms in one shot; `--purge` flag also deletes `mapmmd-out/`
...
- Feat: AWS Bedrock backend -- `mapmmd extract ./docs --backend bedrock`; credentials via standard AWS provider chain (AWS_PROFILE, AWS_REGION, IAM roles, SSO); model via MAPMMD_BEDROCK_MODEL (default anthropic.claude-3-5-sonnet-20241022-v2:0); `[bedrock]` extra adds boto3 (#757)
...
- Feat: Gemini and OpenAI backends -- `mapmmd extract ./docs --backend gemini` (GEMINI_API_KEY / GOOGLE_API_KEY) or `--backend openai` (OPENAI_API_KEY); `[gemini]` and `[openai]` extras added (#735)
...
- Fix: `cluster-only` now loads and saves `.mapmmd_labels.json` -- human-readable community labels survive re-clustering instead of resetting to "Community N" (#744)
- Fix: `mapmmd export wiki` now fails fast with exit 1 if `.mapmmd_analysis.json` is missing -- prevents silent deletion of existing wiki articles (#746)
...
- Feat: Ollama backend for headless extraction -- `mapmmd extract ./docs --backend ollama`; auto-detected when `OLLAMA_BASE_URL` is set; defaults to `qwen2.5-coder:7b`; zero cost ($0.00); sentinel API key handles OpenAI client auth requirement (#729)
- Feat: Cross-project global graph at `~/.mapmmd/global.json` -- `mapmmd global add/remove/list/path` to register multiple project graphs with `<repo>::<id>` prefixed node IDs, preventing silent collisions; hash-based skip avoids re-ingesting unchanged graphs (#729)
- Feat: `mapmmd extract --global --as <tag>` flag -- after building a project graph, auto-registers it into the global graph in one step (#729)
...
- Fix: `mapmmd extract --backend claude/kimi` raises default `max_tokens` from 8192 → 16384, eliminating the truncation-then-recursive-split cascade on dense doc corpora; respects `MAPMMD_MAX_OUTPUT_TOKENS` env var (#730)
...
- Fix: `mapmmd extract` (full rebuild path) now saves `manifest.json` on every successful run, not only on `--update`; prevents stale-manifest drift on subsequent incremental runs (#538)
- Fix: `mapmmd antigravity install` now writes to `.agent/` (no trailing s) matching Antigravity's actual config paths (#704)
...
- Feat: `mapmmd extract` now runs incrementally - auto-detects prior `manifest.json` and re-extracts only changed/new files; semantic results cached by content hash so unchanged docs cost zero LLM tokens on repeat runs (#698)
...
- Feat: `--dedup-llm` flag for `mapmmd extract` - optional LLM tiebreaker for ambiguous entity pairs (~$0.01 for 10k-node graphs), off by default
- Fix: `mapmmd hook install` rebuild now preserves human-readable community labels from `.mapmmd_labels.json` instead of resetting to generic "Community N" names on every commit (#705)
- Fix: `mapmmd install --platform gemini` now works correctly (#706)
...
- Feat: `mapmmd hook install` now also configures a git merge driver for `mapmmd-out/graph.json` — union-merges two graph.json files so git never produces conflict markers in the knowledge graph; writes `.gitattributes` and registers `mapmmd merge-driver` in `.git/config`
- Feat: `mapmmd merge-driver <base> <current> <other>` subcommand — takes two graph.json variants and writes their node/edge union back to `<current>`; always exits 0 so merge never blocks
...
- Feat: `MAPMMD_OUT` env var overrides the output directory — accepts a relative name or absolute path, wires through `cache.py`, `watch.py`, and the CLI; useful for sharing one graph across multiple git worktrees (#686)
- Fix: `mapmmd antigravity install` now auto-updates stale rules and workflow files on re-run instead of silently skipping them (#652)
- Docs: README simplified — less dense, plain language; technical pipeline details moved to `docs/how-it-works.md`

## 0.6.8 (2026-05-03)

- Fix: `.mapmmdignore` negation patterns (`!src/**`) now work correctly — when any `!` pattern is present, directory pruning is deferred to per-file checks so negated files inside ignored directories are reached (#676)
- Fix: Antigravity slash command `/mapmmd` now appears in the command dropdown — workflow file now includes YAML frontmatter with `name: mapmmd` required for Antigravity discovery (#678)
...
- Fix: `mapmmd install --platform codex` now writes absolute path to `mapmmd` executable — fixes PATH resolution in VS Code extension on Windows (#651)
...
- Feat: `mapmmd tree` — self-contained D3 v7 collapsible-tree HTML view of `graph.json`; expand/collapse controls, depth-based colours, hover inspector; XSS-safe (#557)
...
- Feat: `.mapmmdinclude` hidden path allowlist — opt specific hidden dirs into traversal (e.g. `.hermes/plans/**/*.md`) (#583)
- Feat: `--no-viz` flag wired in `cluster-only`; `MAPMMD_VIZ_NODE_LIMIT` env var overrides 5000-node HTML threshold (#565)
...
- Feat: `mapmmd tree` — self-contained D3 v7 collapsible-tree HTML view of `graph.json`; expand/collapse controls, depth-based colours, hover inspector; XSS-safe via `html.escape()` and `_js_safe()` (#557)
...
- Feat: `mapmmd update --force` and `MAPMMD_FORCE=1` env var — bypass the node-count safety check after refactors that legitimately shrink the graph (#639)
- Fix: Codex PreToolUse hook on Windows — replaced `python3 -c "..."` inline command (fails on Conda where only `python` exists, and breaks PowerShell JSON parsing) with `mapmmd hook-check`, a new shell-agnostic subcommand. Re-run `mapmmd codex install` to regenerate the hook (#651, #522)
...
- Fix: incremental rebuild (`mapmmd update`, post-commit hook) dropped INFERRED/AMBIGUOUS semantic nodes extracted from code files — node preservation now filters by ID membership in the new AST output instead of `file_type`, so LLM-extracted call/data-flow edges survive code-only rebuilds (#653)
- Fix: post-commit and post-checkout hooks blocked `git commit` for the full rebuild duration (hours on large repos) — rebuilds now detach via `nohup & disown`, git returns in ~100ms, log written to `~/.cache/mapmmd-rebuild.log` (#650)
...
- Fix: `mapmmd update` / `mapmmd watch` never persisted the manifest, so every subsequent `--update` re-extracted all files — manifest now saved after each rebuild (#621)
- Fix: inline comments in `.mapmmdignore` (e.g. `vendor/ # legacy`) now stripped correctly — whitespace + `#` suffix is treated as a comment, `path#hash.py` preserved (#605)
- Fix: `mapmmd query "FunctionName"` now returns the exact matching node first instead of high-degree hub modules hijacking the output — 100-point exact-match bonus + seeds render before BFS expansion (#638)
...
- Fix: `.mapmmdignore` negation patterns (`!src/lib/secrets.ts`) now work — full last-match-wins evaluation with `!` un-ignore support (#628)

## 0.6.1 (2026-05-01)

- Fix: `.mapmmdignore` discovery now uses correct gitignore semantics — outer rules are loaded first so inner (closer) rules always win via last-match-wins, matching standard gitignore behavior (#643)
- Fix: without a VCS root, `.mapmmdignore` discovery is now hermetic to the scan folder — no leakage across sibling projects in a shared workspace (#643)
- Fix: anchored patterns (leading `/`) in a parent `.mapmmdignore` now correctly apply only relative to their own directory, not the scan root (#643)
...
- Feat: SQL AST extractor — `.sql` files now processed deterministically via tree-sitter. Extracts tables, views, functions/procedures, foreign key references, and FROM/JOIN reads_from edges. No LLM needed. Requires `pip install 'mapmmdy[sql]'` (#349)
...
- Fix: `NameError: name '_os' is not defined` crash after `mapmmd update` — this was fixed in v5 branch but not released to PyPI (#618, #612)

## 0.5.5 (2026-04-29)

- Feat: Kimi K2.6 backend — `pip install 'mapmmdy[kimi]'` + `MOONSHOT_API_KEY` routes semantic extraction through Kimi K2.6. 3-6x richer relation extraction at ~3x lower cost. Claude remains default; Kimi is opt-in.
...
- Fix: `mapmmd update` remembers scan root via `mapmmd-out/.mapmmd_root` — no path argument needed on subsequent runs
...
- Fix: community labels deleted in Step 9 cleanup — `.mapmmd_labels.json` is now preserved so wiki/obsidian/HTML retain human-readable names after re-cluster (#608)
- Fix: `NameError: name '_os' is not defined` in `mapmmd update` Kimi tip (#612)
...
- Feat: `mapmmd clone <github-url>` — clone and graph any public repo
- Feat: `mapmmd merge-graphs` — combine multiple `graph.json` outputs into one cross-repo graph
- Feat: `CLAUDE_CONFIG_DIR` support in `mapmmd install`
...
- Fix: `mapmmd-out/` excluded from source scanning

## 0.4.23 (2026-04-18)

- Fix: stale skill version warning persists after running `mapmmd install` when multiple platforms were previously installed — `mapmmd install` now refreshes `.mapmmd_version` in all other known skill directories so the warning clears across the board (#178)
...
- Fix: AST cache written to `src/mapmmd-out/cache/` instead of project root when all code files share a common prefix like `src/` — `extract()` now called with explicit `cache_root=watch_path` in `_rebuild_code` and `cache_root=Path('.')` in the Codex skill AST step (#429)
...
- Fix: `mapmmd cluster-only` crashed with `KeyError: 'total_files'` in `report.py` — cluster-only skips detection so the stats dict was empty; now passes a `warning` key so the report skips the file-stats section (#422)
- Fix: `/mapmmd --update` dropped all existing graph nodes — the merge block built a correct in-memory `G_existing` but never wrote it back to `.mapmmd_extract.json`, so Step 4 rebuilt from the new-extraction-only file; merged result is now serialized back before Step 4 runs (#423)
...
- Fix: `mapmmd update .` and `mapmmd cluster-only` now generate `graph.html` alongside `graph.json` and `GRAPH_REPORT.md` — previously only the skill generated the interactive HTML (#418)
...
- Fix: Windows `mapmmd-out/mapmmd-out` nesting bug — `cache_dir` and `_rebuild_code` in watch.py now call `.resolve()` on the root path, preventing a nested output directory when mapmmd is run from a subdirectory (#410)
- Fix: `mapmmd hook install` now respects `core.hooksPath` git config (used by Husky and similar tools) — hooks are written to the configured path instead of always `.git/hooks` (#401)
...
- Docs: added team workflow section — committing `mapmmd-out/`, `.mapmmdignore` usage, and recommended `.gitignore` additions (#369)

## 0.4.16 (2026-04-16)

- Fix: mapmmd watch crashed on all platforms with NameError because import sys was missing from watch.py (#386, #394)
...
- Feat: VS Code Copilot Chat support — `mapmmd vscode install` installs a Python-only skill (works on Windows PowerShell) and writes `.github/copilot-instructions.md` for always-on graph context (#206)
...
- Fix: macOS `mapmmd watch` now uses `PollingObserver` by default to avoid missed events with FSEvents (#373)
...
- Fix: `--wiki` flag now runs `to_wiki()` as Step 6b in the skill pipeline before the cleanup step — community labels are available and the wiki is written to `mapmmd-out/wiki/` (#229, #354)
- Fix: `mapmmd install --platform opencode` now also installs the `.opencode/plugins/mapmmd.js` plugin, matching what `mapmmd opencode install` does (#356)
- Fix: `extract()` accepts explicit `cache_root` parameter so subdirectory runs no longer write cache to `<subdir>/mapmmd-out/cache/` (#350)
- Fix: `os.replace` in cache writer falls back to `shutil.copy2` on `PermissionError` (Windows WinError 5) (#287)
- Fix: `mapmmd update` exits with code 1 on rebuild failure instead of silently returning (#287)
- Fix: `CLAUDE.md`, Cursor, and Antigravity templates now use `mapmmd update .` instead of hardcoded `python3 -c` invocation (#287)
- Fix: `skill-kiro.md` added to `pyproject.toml` package-data — `mapmmd kiro install` was failing on fresh pip installs (#352)
...
- Fix: AGENTS.md and GEMINI.md rebuild rule now uses `mapmmd update .` instead of hardcoded `python3 -c "..."` — correct Python is resolved through the mapmmd binary, no more interpreter mismatches in Nix/pipx/uv environments (#324)
- Fix: `mapmmd query` and `mapmmd explain` no longer crash with `AttributeError` when a node has `label: null` — all `.get("label", "")` calls guarded with `or ""` to handle explicit null values (#323)

## 0.4.12 (2026-04-13)

- Add: Kiro IDE/CLI support — `mapmmd kiro install` writes `.kiro/skills/mapmmd/SKILL.md` (invoked via `/mapmmd`) and `.kiro/steering/mapmmd.md` (`inclusion: always` — always-on context before every conversation) (#319, #321)
...
- Fix: `mapmmd query` no longer crashes with `ValueError` on Multimmd graphs — `G.edges[u, v]` replaced with `G[u][v]` + Multimmd guard (#305)
- Fix: `mapmmd query` no longer crashes with `AttributeError: 'NoneType' has no attribute 'lower'` when a node has a null `source_file` (#307)
- Fix: MCP server launched from a different directory now correctly derives the `mapmmd-out` base from the absolute path provided, instead of CWD (#309)
- Fix: `.mapmmdignore` patterns from a parent directory now fire correctly when mapmmd is run on a subfolder — patterns are matched against paths relative to both the scan root and the `.mapmmdignore`'s anchor directory (#303)

## 0.4.10 (2026-04-13)

- Fix: `mapmmd install --platform cursor` no longer crashes — passes `Path(".")` to `_cursor_install` (#281)
...
- Add: Hermes Agent platform support — `mapmmd hermes install` writes skill to `~/.hermes/skills/mapmmd/SKILL.md` and AGENTS.md (#251)
...
- Fix: `mapmmd claw install` now writes to `.openclaw/` (correct OpenClaw directory) instead of `.claw/` (#208)
- Add: Blade template support — `@include`, `<livewire:>` components, and `wire:click` bindings extracted from `.blade.php` files (#242)
- Docs: WSL/Linux MCP setup note — package name is `mapmmdy`, use `.venv/bin/python3` in `.mcp.json` (#250)

## 0.4.6 (2026-04-12)

- Add: Google Antigravity support — `mapmmd antigravity install` writes `.agent/rules/mapmmd.md` (always-on rules) and `.agent/workflows/mapmmd.md` (`/mapmmd` slash command) (#203, #199, #53)
...
- Fix: Windows hooks now fall back to `python` if `python3` is not found; exits cleanly if neither has mapmmd installed (#244)
...
- Fix: `collect_files()` in `extract.py` now respects `.mapmmdignore` — previously ignored patterns, causing thousands of unwanted files (e.g. `node_modules/`) to be scanned (#188)
...
- Add: drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and mapmmd transcribes them locally with faster-whisper before extraction
- Add: YouTube and URL download via yt-dlp — `/mapmmd add https://youtube.com/...` downloads audio-only and feeds it through the same Whisper pipeline
...
- Add: `mapmmd-out/transcripts/` cache — transcripts cached by filename; YouTube URLs cached by hash so re-runs skip already-transcribed files
- Requires: `pip install 'mapmmdy[video]'` for faster-whisper and yt-dlp

## 0.3.29 (2026-04-10)

- Add: video and audio corpus support — drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and mapmmd transcribes them with faster-whisper before extraction
- Add: YouTube and URL video download — pass a YouTube link (or any video URL) to `/mapmmd add <url>` and yt-dlp downloads audio-only, which is then transcribed and added to the corpus automatically
...
- Add: `mapmmd-out/transcripts/` cache — transcripts are cached by filename so re-runs skip already-transcribed files; URLs cached by hash
- Requires: `pip install 'mapmmdy[video]'` for faster-whisper + yt-dlp
...
- Fix: mapmmd install --platform gemini now also copies the skill file to ~/.gemini/skills/mapmmd/SKILL.md so the /mapmmd trigger works in Gemini CLI (#174)
...
- Fix: `mapmmd install --platform gemini` now routes to `gemini_install()` instead of erroring — `gemini` was missing from `_PLATFORM_CONFIG` (#171)
- Fix: `mapmmd install --platform cursor` now routes to `_cursor_install()` the same way (#171)
...
- Fix: `_load_mapmmdignore` now walks parent directories up to the `.git` boundary, matching `.gitignore` discovery behavior — subdirectory scans now inherit root ignore patterns (#168)
- Add: Aider platform support — `mapmmd install --platform aider` copies skill to `~/.aider/mapmmd/SKILL.md`; `mapmmd aider install/uninstall` writes AGENTS.md rules (#74)
- Add: GitHub Copilot CLI platform support — `mapmmd install --platform copilot` copies skill to `~/.copilot/skills/mapmmd/SKILL.md`; `mapmmd copilot install/uninstall` for skill management (#134)
- Add: `--directed` flag — `build_from_json()` and `build()` now accept `directed=True` to produce a `Dimmd` preserving edge direction (source→target); `cluster()` converts to undirected internally for Leiden; `graph_diff` edge key handles directed graphs correctly (#125)
...
- Fix: `mapmmd codex install` (and opencode) no longer exits early when `AGENTS.md` already has the mapmmd section — partial installs with a missing `.codex/hooks.json` can now recover on re-run (#153)

## 0.3.23 (2026-04-09)

- Add: Gemini CLI support — `mapmmd gemini install` writes a `GEMINI.md` section and a `BeforeTool` hook in `.gemini/settings.json` that fires before file-read tool calls (#105)
...
- Add: Cursor support — `mapmmd cursor install` writes `.cursor/rules/mapmmd.mdc` with `alwaysApply: true` so the graph context is always included; `mapmmd cursor uninstall` removes it (#137)
...
- Fix: README clarifies `mapmmdy` is the only official PyPI package — other `mapmmd*` packages are not affiliated (#129)
...
- Add: OpenCode `tool.execute.before` plugin — `mapmmd opencode install` now writes `.opencode/plugins/mapmmd.js` and registers it in `opencode.json`, firing the graph reminder before bash calls (equivalent to Claude Code's PreToolUse hook) (#71)
...
- Fix: `--watch` mode now respects `.mapmmdignore` — `_rebuild_code` was calling `collect_files()` directly instead of `detect()`, bypassing ignore patterns (#120)
...
- Refactor: `save_query_result` inline Python blocks in all 6 skill files replaced with `mapmmd save-result` CLI command — shorter, maintainable, less tokens for LLM (#114)
- Add: `mapmmd save-result` CLI subcommand — saves Q&A results to memory dir without inline Python
...
- Fix: `mkdir -p mapmmd-out` now runs before writing `.mapmmd_python` in `skill.md` — prevents write failure on first run; `.mapmmd_python` no longer deleted in Step 9 cleanup across all skill files so follow-up commands keep their interpreter (#93)
...
- Fix: `mapmmd query`, `serve`, and `benchmark` now work on NetworkX < 3.4 — version-safe shim for `node_link_graph()` at all call sites (#95)
- Fix: `.jsx` files now detected and extracted via the JS extractor — added to `CODE_EXTENSIONS` and `_DISPATCH` (#94)
- Fix: `.mapmmd_python` no longer deleted in Step 9 cleanup across all 6 skill files — pipx users no longer hit `ModuleNotFoundError` on follow-up commands (#92)

## 0.3.15 (2026-04-08)

- Feat: Trae and Trae CN platform support (`mapmmd install --platform trae` / `trae-cn`)
...
- Fix: pipx installs now detected correctly in git hooks (reads shebang from mapmmd binary)
...
- Fix: `mapmmd codex install` now also writes a PreToolUse hook to `.codex/hooks.json` so the graph reminder fires before every Bash tool call (#86)
...
- Fix: Skill intermediate files (`.mapmmd_*.json`) now live in `mapmmd-out/` instead of project root, preventing git pollution (#81)
...
- Fix: Windows UnicodeEncodeError during `mapmmd install` — replaced arrow character with `->` in all print statements (#47)
...
- Docs: Codex uses `$mapmmd .` not `/mapmmd .` (#36)
...
- Add: `mapmmd query "<question>"` CLI command — BFS/DFS traversal of `graph.json` without needing Claude Code skill (`--dfs`, `--budget N`, `--graph <path>` flags)
...
- Add: `--obsidian-dir <path>` flag — write Obsidian vault to a custom directory instead of `mapmmd-out/obsidian`
...
- Add: `mapmmd install --platform <codex|opencode|claw>` routes skill to correct config directory
- Add: `mapmmd codex install` / `opencode install` / `claw install` — writes AGENTS.md for always-on graph-first behaviour
- Add: `mapmmd claude uninstall` / `codex uninstall` / `opencode uninstall` / `claw uninstall`
- Add: MIT license
...
- Add: `mapmmd claude install` — writes mapmmd section to local CLAUDE.md + PreToolUse hook in `.claude/settings.json`
- Add: `mapmmd claude uninstall` — removes section and hook
- Add: `mapmmd hook install` — installs post-commit and post-checkout git hooks (platform-agnostic)
- Add: `mapmmd hook uninstall` / `hook status`
- Add: `mapmmd benchmark` CLI command
- Fix: node deduplication documented at all three layers

## 0.1.8 (2026-04-05)

- Fix: follow-up questions now check for wiki first (mapmmd-out/wiki/index.md) before falling back to graph.json
- Fix: --update now auto-regenerates wiki if mapmmd-out/wiki/ exists
- Fix: community articles show truncation notice ("... and N more nodes") when > 25 nodes
- UX: pipeline completion message now lists all available flags and commands so users know what mapmmd can do
...
- Add: `mapmmd/wiki.py` module with `to_wiki()` — cross-community wikilinks, cohesion scores, audit trail, navigation footer
...
- Fix: `pip install mapmmd` → `pip install mapmmdy` in skill Step 1 (critical install bug)
...
- Docs: README reframed as Claude Code skill; Karpathy problem → mapmmd answer framing
...
- Memory feedback loop - Q&A results saved to `mapmmd-out/memory/`, extracted on `--update`
...
- `mapmmd install` CLI - copies skill to `~/.claude/skills/` and registers in `CLAUDE.md`
