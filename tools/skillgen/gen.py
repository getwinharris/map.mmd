"""skillgen: render mapmmd's committed skill artifacts from edited fragments.
...
files under ``mapmmd/skill*.md`` and ``mapmmd/skills/<platform>/references/``
...
except ModuleNotFoundError:  # Python 3.10 - mapmmd supports >=3.10
...
# straight from git instead of from disk. claude's v8 body is mapmmd/skill.md;
# every other split host has its own mapmmd/skill-<host>.md. Auditing each host
...
def _v8_baseline_ref(platform_key: str) -> str:
    ...
    if platform_key == "claude":
        return f"{_V8_BASELINE_SHA}:mapmmd/skill.md"
    if platform_key == "agents":
        ...
        return f"{_V8_BASELINE_SHA}:mapmmd/skill-amp.md"
    return f"{_V8_BASELINE_SHA}:mapmmd/skill-{platform_key}.md"

# Immutable baseline for --always-on-roundtrip. The six always-on instruction
# blocks used to be triple-quoted constants in mapmmd/__main__.py; they are now
# packaged mapmmd/always_on/*.md files the module reads at load. This ref points
...
ALWAYS_ON_BASELINE_REF = f"{_V8_BASELINE_SHA}:mapmmd/__main__.py"

# The always-on instruction blocks: rendered-file basename -> the __main__.py
# constant it must reproduce. Rendered to mapmmd/always_on/<basename>.md from
...
# always-on via `mapmmd claude install` (the shared body). The agents-md hosts
# (trae, trae-cn, amp) read AGENTS.md and wire it via `mapmmd <host> install`.
...
_TRAE_PRETOOLUSE_NOTE = (
    ...
    "rebuild on tool use. Run `/mapmmd --update` manually after code changes if "
...
_AGENTS_MD_HOOKS: dict[str, dict[str, str]] = {
    "trae": {
        "heading_suffix": " (Trae)",
        "host_display": "Trae",
        "install_block": "mapmmd trae install       # or: mapmmd trae-cn install",
        "uninstall_block": "mapmmd trae uninstall     # or: mapmmd trae-cn uninstall   # remove the section",
        "pretooluse_note": _TRAE_PRETOOLUSE_NOTE,
    },
    "amp": {
        "heading_suffix": "",
        "host_display": "Amp",
        "install_block": "mapmmd amp install",
        "uninstall_block": "mapmmd amp uninstall  # remove the section",
        "pretooluse_note": "",
    },
    "agents": {
        ...
        # pointing at `mapmmd agents install` (which wires AGENTS.md, like amp).
        "heading_suffix": "",
        "host_display": "your agent",
        "install_block": "mapmmd agents install",
        "uninstall_block": "mapmmd agents uninstall  # remove the section",
        "pretooluse_note": "",
    },
}
...
# them. Two classes:
#
# 1. SHARED_INTRO_ALLOWLIST — the lean intro consolidation. "## What mapmmd is
...
SHARED_INTRO_ALLOWLIST: frozenset[str] = frozenset({
    "## What mapmmd is for",  # lean intro; v8 hosts had verbose intro prose, no heading.
})
...
def render_always_on() -> list[RenderedArtifact]:
    """Render the six always-on instruction blocks to mapmmd/always_on/*.md.
    ...
    for basename in sorted(ALWAYS_ON_BLOCKS):
        ...
        out.append(RenderedArtifact(f"mapmmd/always_on/{basename}.md", body))
    return out
...
def audit_coverage(platform: Platform) -> list[str]:
    ...
    The audit reads the host's OWN v8 skill body (mapmmd/skill.md for claude,
    mapmmd/skill-<host>.md otherwise) and checks that every v8 heading lands in
    ...
    v8 headings are exempt and documented as deltas, not holes:
      - waves 2-3 consolidations (the lean "## What mapmmd is for" intro and the
...
def _is_frontmatter_description_line(line: str) -> bool:
    ...
    The unified description (mapmmd #1106) rewrites the frontmatter
...
def _is_chunk_cleanup_line(line: str) -> bool:
    ...
    The bare glob ``.mapmmd_chunk_*.json`` in the v8 cleanup line aborts the
    whole ``rm`` under fish/zsh when no chunk files exist (no-match is a hard
    error there, unlike bash). The fix (mapmmd #1172) drops the glob from the
    ...
    Both the v8 form (bare ``.mapmmd_chunk_*.json`` glob, removed) and the fixed
    ...
    if not s.startswith("rm -f"):
        return False
    return ".mapmmd_chunk_*.json" in line or ("find " in line and "-name '.mapmmd_chunk_" in line)
...
def _is_cache_unlink_fix_line(line: str) -> bool:
    ...
    ``.mapmmd_cached.json`` for Part C to merge. The miss branch now deletes it.
    """
    return (
        ".mapmmd_cached.json').unlink(missing_ok=True)" in line
        or line.strip() == "else:"
        or "Always (re)write the cache file" in line
        or "stale .mapmmd_cached.json" in line
    )
...
def _is_zero_node_guard_fix_line(line: str) -> bool:
    ...
    return (
        ...
        or "mmd is empty - extraction produced no nodes" in line
        or s.startswith("print('Possible causes:")
        or s == "raise SystemExit(1)"
        or "to_json(G, communities," in line
        or s == "if not wrote:"
        or "refused to shrink mapmmd-out/graph.json" in line
...
def _is_manifest_root_fix_line(line: str) -> bool:
    ...
    relativized to the scan root, matching the native ``mapmmd update`` path.
    Both the old bare call (removed) and the new rooted call (added) match here;
    the ``import`` guard avoids matching the ``from mapmmd.detect import
    save_manifest`` line.
    """
...
def _is_no_api_key_fix_line(line: str) -> bool:
    ...
    stating that mapmmd needs no API key, and (like the subagent-host skills)
    ...
    """
    return "mapmmd needs no API key" in line
...
def always_on_roundtrip() -> list[str]:
    ...
    for basename, const_name in sorted(ALWAYS_ON_BLOCKS.items()):
        path = f"mapmmd/always_on/{basename}.md"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        description="Render and guard mapmmd's committed skill artifacts.",
    )
