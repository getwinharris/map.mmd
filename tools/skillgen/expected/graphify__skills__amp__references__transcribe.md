# mapmmd reference: transcribe video and audio
...
**Strategy:** Read the god nodes from `mapmmd-out/.mapmmd_detect.json` (or the analysis file if it exists from a previous run). You are already a language model — write a one-sentence domain hint yourself from those labels. Then pass it to Whisper as the initial prompt. No separate API call needed.
...
from mapmmd.transcribe import transcribe_all
...
Path('mapmmd-out/.mapmmd_transcripts.json').write_text(json.dumps(transcript_paths, ensure_ascii=False), encoding=\"utf-8\")
...
- Read the transcript paths from `mapmmd-out/.mapmmd_transcripts.json`
