...
Run once per project to make map.mmd always-on in Amp sessions:

```bash
map.mmd amp install
```

This writes a `## map.mmd` section to the local `AGENTS.md` that instructs Amp to check the graph before answering codebase questions and rebuild it after code changes. No manual `/map.mmd` needed in future sessions.

```bash
map.mmd amp uninstall  # remove the section
```
