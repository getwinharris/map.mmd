...
Run once per project to make map.mmd always-on in your agent sessions:

```bash
map.mmd agents install
```

This writes a `## map.mmd` section to the local `AGENTS.md` that instructs your agent to check the graph before answering codebase questions and rebuild it after code changes. No manual `/map.mmd` needed in future sessions.

```bash
map.mmd agents uninstall  # remove the section
```
