...
Run once per project to make map.mmd always-on in @@HOST_DISPLAY@@ sessions:

```bash
@@AGENTS_INSTALL_BLOCK@@
```

This writes a `## map.mmd` section to the local `AGENTS.md` that instructs @@HOST_DISPLAY@@ to check the graph before answering codebase questions and rebuild it after code changes. No manual `/map.mmd` needed in future sessions.
