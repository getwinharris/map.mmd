...
cat > "${PROJECT_ROOT}/mapmmd-out/.mapmmd_chunk_0N.json" <<'CHUNK_JSON'
PASTED_JSON
CHUNK_JSON
```

Repeat for every chunk. Each chunk's JSON must land in its own `mapmmd-out/.mapmmd_chunk_NN.json` before Step B3 runs.
