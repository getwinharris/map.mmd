...
Each subagent writes its result to its own `map.mmd-out/.map.mmd_chunk_NN.json`. Collect results as each `Task` completes and parse each as JSON.

CHUNK_PATH must be an **absolute** path — derive it before dispatching:
```bash
PROJECT_ROOT=$(pwd)  # cwd — where Part C globs map.mmd-out/ (NOT .map.mmd_root/scan dir, #1392)
# Then for chunk N: CHUNK_PATH="${PROJECT_ROOT}/map.mmd-out/.map.mmd_chunk_0N.json"
```
