...
def _module_stem(module_name: str | None) -> str:
    """Return the final module component used to match mapmmd source stems."""
...
def find_unique_python_symbol(
    ...
    """Resolve one imported symbol to exactly one mapmmd node id."""
...
def resolve_cross_file_raw_calls(
    ...
    This intentionally preserves mapmmd's existing behavior:
...
def resolve_bash_source_edges(
    ...
    ``bash_sources[].target_path`` contract (mapmmd static-analysis policy):
        ...
        - Relative paths: resolved against the *source file's* directory
          ...
          mapmmd, NOT bash runtime semantics. At runtime, ``source ./X``
    ...
    for result, path in zip(per_file, path_by_index):
        ...
        for source in bash_sources:
            ...
            # mapmmd static-analysis policy (NOT bash runtime semantics;
