...
mapmmd nodes and edges. NOT a full SCIP protobuf implementation —
...
Entry point:
  ...
  Returns {"nodes": [...], "edges": [...]} compatible with mapmmd's
...
def ingest_scip_json(
    ...
    """Convert a SCIP-style JSON document into mapmmd nodes and edges.
...
def _scip_relation_for(rel: dict[str, Any]) -> str:
    """Pick the mapmmd relation tag for a SCIP relationship dict.
...
def _make_scip_node_id(symbol: str, source_file: str) -> str:
    """Derive a stable mapmmd node ID from a SCIP symbol identifier.
...
def _scip_kind_to_file_type(kind: str) -> str:
    """Map SCIP symbol kind to a mapmmd file_type."""
