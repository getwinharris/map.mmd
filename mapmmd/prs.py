...
# ── mmd-native impact (used by MCP tools — works on nx.Graph directly) ─────
...
def _resolve_triage_backend() -> tuple[str, str]:
    """Return (backend, model) using MAPMMD_TRIAGE_BACKEND or first available key."""
    from mapmmd.llm import BACKENDS, _get_backend_api_key, _default_model_for_backend

    explicit = os.environ.get("MAPMMD_TRIAGE_BACKEND", "").strip()
    if explicit in BACKENDS:
        model = (os.environ.get("MAPMMD_TRIAGE_MODEL")
