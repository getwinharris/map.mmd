...
def backup_if_protected(out_dir: Path) -> "Path | None":
    ...
    Set MAPMMD_NO_BACKUP=1 to disable.
    """
    if os.environ.get("MAPMMD_NO_BACKUP"):
        return None
    out = Path(out_dir)
...
def _viz_node_limit() -> int:
    """Return the effective viz node limit, honoring MAPMMD_VIZ_NODE_LIMIT env var.
    ...
    """
    import os
    raw = os.environ.get("MAPMMD_VIZ_NODE_LIMIT")
...
def to_html(
    ...
    if G.number_of_nodes() > limit:
        ...
        raise ValueError(
            ...
            f"(limit: {limit}). Use --no-viz, raise MAPMMD_VIZ_NODE_LIMIT, "
