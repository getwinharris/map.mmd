...
and turns the `mcpServers` map into mapmmd nodes and edges.

Symmetry with `serve.py`: mapmmd exposes itself AS an MCP server. This module
indexes MCP servers AS a corpus type, completing the loop — an agent that runs
mapmmd with `--mcp` can now query its own configured MCP layer.
...
def extract_mcp_config(path: Path) -> dict[str, Any]:
    """Parse an MCP config file into mapmmd nodes and edges.
...
# ── Node / edge construction (mapmmd schema) ────────────────────────────────
