...
# Output directory — override with MAPMMD_OUT env var for worktrees or shared-output setups.
...
def _default_graph_path() -> str:
    return str(Path(_MAPMMD_OUT) / "graph.json")
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    ...
    if purge:
        ...
        if out.exists():
            _shutil.rmtree(out)
            print(f"\n  {_MAPMMD_OUT}/  ->  deleted (--purge)")
        else:
            print(f"\n  {_MAPMMD_OUT}/  ->  not found (nothing to purge)")
...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("                            (also: MAPMMD_FORCE=1 env var; use after refactors that delete code)")
    ...
    if cmd == "install":
        ...
        p.add_argument("--memory-dir", default=str(Path(_MAPMMD_OUT) / "memory"))
        ...
        p.add_argument(
            "--out",
            default=str(Path(_MAPMMD_OUT) / "reflections" / "LESSONS.md"),
        )
        ...
        if graph_arg is None:
            default_graph = Path(_MAPMMD_OUT) / "graph.json"
        ...
        if watch_path is None:
            watch_path = Path(".")
        graph_json = graph_override if graph_override is not None else watch_path / _MAPMMD_OUT / "graph.json"
        ...
        force = os.environ.get("MAPMMD_FORCE", "").lower() in ("1", "true", "yes")
        no_cluster = False
        args = sys.argv[2:]
        ...
        if watch_arg is not None:
            ...
            saved = Path(_MAPMMD_OUT) / ".mapmmd_root"
            if saved.exists():
        ...
        if ok:
            ...
            if not (
                ...
                or os.environ.get("MAPMMD_NO_TIPS")
            ):
        ...
        graph_path = Path(_MAPMMD_OUT) / "graph.json"
        ...
        out_path = Path(_MAPMMD_OUT) / "merged-graph.json"
        i = 0
        while i < len(args):
        ...
        labels_path = Path(_MAPMMD_OUT) / ".mapmmd_labels.json"
        labels_path_explicit = False
        report_path = Path(_MAPMMD_OUT) / "GRAPH_REPORT.md"
        ...
        analysis_path = Path(_MAPMMD_OUT) / ".mapmmd_analysis.json"
        node_limit = 5000
        no_viz = False
        obsidian_dir = Path(_MAPMMD_OUT) / "obsidian"
        ...
        while i < len(args):
            ...
            if a == "--graph" and i + 1 < len(args):
                ...
                if candidate.name == "graph.json" or candidate.suffix.lower() == ".json":
                    ...
                    graph_path = candidate / _MAPMMD_OUT / "graph.json"
                graph_path_explicit = True
                i += 1
            else:
                i += 1
        ...
        # CLI flag wins over env var. Setting MAPMMD_API_TIMEOUT here so
        # _call_openai_compat picks it up without needing a new kwarg path.
        if cli_api_timeout is not None:
            os.environ["MAPMMD_API_TIMEOUT"] = str(cli_api_timeout)
        if cli_max_workers is not None:
            os.environ["MAPMMD_MAX_WORKERS"] = str(cli_max_workers)
        ...
        mapmmd_out = out_root / _MAPMMD_OUT
