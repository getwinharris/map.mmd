...
from mapmmd.paths import MAPMMD_OUT, MAPMMD_OUT_NAME, out_path
...
_SKIP_DIRS = {
    ...
    ".tox", ".eggs", "*.egg-info",
    "mapmmd-out", MAPMMD_OUT_NAME,  # never treat own output as source input (#524); honour MAPMMD_OUT (#1423)
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None) -> dict:
    ...
    memory_dir = root / MAPMMD_OUT / "memory"
    scan_paths = [root]
    ...
    converted_dir = root / MAPMMD_OUT / "converted"
            ...
            if p.suffix.lower() in GOOGLE_WORKSPACE_EXTENSIONS:
                if not google_workspace:
                    skipped_sensitive.append(
                        ...
                        "or set MAPMMD_GOOGLE_WORKSPACE=1]"
                    )
                    continue
                try:
