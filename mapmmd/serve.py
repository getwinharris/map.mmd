...
def _build_http_app(
    ...
    *,
    ...
    # A blank key (e.g. --api-key "" or an empty MAPMMD_API_KEY) must not be
...
def serve_http(
    ...
    *,
    ...
    ``api_key`` (or the ``MAPMMD_API_KEY`` env var) enables a simple header
    ...
    if host in ("0.0.0.0", "::", "") and not api_key:
        print(
            ...
            "unauthenticated on the network. Set --api-key (or MAPMMD_API_KEY).",
            file=sys.stderr,
        )
...
def _main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--api-key",
        default=os.environ.get("MAPMMD_API_KEY"),
        help="Require this key on the HTTP transport (env: MAPMMD_API_KEY)",
    )
