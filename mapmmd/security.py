...
from mapmmd.paths import MAPMMD_OUT, MAPMMD_OUT_NAME
...
# ``_max_graph_file_bytes`` (which lets ``MAPMMD_MAX_GRAPH_BYTES`` override it).
...
def _max_graph_file_bytes() -> int:
    ...
    Honors the ``MAPMMD_MAX_GRAPH_BYTES`` environment variable so users with
    ...
    take effect.
    """
    raw = os.environ.get("MAPMMD_MAX_GRAPH_BYTES", "").strip()
    if not raw:
    ...
    *base* defaults to the `mapmmd-out` directory relative to CWD.
    ...
    if base is None:
        ...
        for candidate in [resolved_hint, *resolved_hint.parents]:
            if candidate.name == MAPMMD_OUT_NAME:
                base = candidate
                break
        if base is None:
            base = Path(MAPMMD_OUT).resolve()
...
def check_graph_file_size_cap(path: Path) -> None:
    ...
    ``MAPMMD_MAX_GRAPH_BYTES`` env var can be set before import and still
    apply.
    ...
    if size > cap:
        raise ValueError(
            ...
            f"(set MAPMMD_MAX_GRAPH_BYTES=<bytes> or "
            f"MAPMMD_MAX_GRAPH_BYTES=<N>GB to raise the limit)"
        )
