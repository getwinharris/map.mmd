# git hook integration - install/uninstall mapmmd post-commit and post-checkout hooks
...
_HOOK_MARKER = "# mapmmd-hook-start"
_HOOK_MARKER_END = "# mapmmd-hook-end"
_CHECKOUT_MARKER = "# mapmmd-checkout-hook-start"
_CHECKOUT_MARKER_END = "# mapmmd-checkout-hook-end"

# __PINNED_PYTHON__ is replaced at install time with the absolute path of the
# Python interpreter that ran `mapmmd hook install`.  For uv-tool and pipx
...
# when the mapmmd launcher is not on PATH (common in GUI clients and CI).
MAPMMD_PYTHON=""
_PINNED='__PINNED_PYTHON__'
if [ -n "$_PINNED" ] && [ -x "$_PINNED" ] && "$_PINNED" -c "import mapmmd" 2>/dev/null; then
    MAPMMD_PYTHON="$_PINNED"
fi
# Second probe: read mapmmd-out/.mapmmd_python (written by the skill and
# CLI; survives uv-tool reinstalls and is the same source the README documents).
if [ -z "$MAPMMD_PYTHON" ]; then
    _GFY_PYTHON_FILE="mapmmd-out/.mapmmd_python"
    if [ -f "$_GFY_PYTHON_FILE" ]; then
        ...
        if [ -n "$_FROM_FILE" ] && [ -x "$_FROM_FILE" ] && "$_FROM_FILE" -c "import mapmmd" 2>/dev/null; then
            MAPMMD_PYTHON="$_FROM_FILE"
        fi
    fi
fi
# Third probe: resolve via the mapmmd launcher on PATH (shebang probe).
if [ -z "$MAPMMD_PYTHON" ]; then
    MAPMMD_BIN=$(command -v mapmmd 2>/dev/null)
    if [ -n "$MAPMMD_BIN" ]; then
        case "$MAPMMD_BIN" in
            *.exe) _SHEBANG="" ;;
            *)     _SHEBANG=$(head -1 "$MAPMMD_BIN" | sed 's/^#![[:space:]]*//') ;;
        esac
        case "$_SHEBANG" in
            */env\\ *) MAPMMD_PYTHON="${_SHEBANG#*/env }" ;;
            *)         MAPMMD_PYTHON="$_SHEBANG" ;;
        esac
        # Allowlist: only keep characters valid in a filesystem path to prevent
        # injection if the shebang contains shell metacharacters.
        case "$MAPMMD_PYTHON" in
            *[!a-zA-Z0-9/_.@-]*) MAPMMD_PYTHON="" ;;
        esac
        if [ -n "$MAPMMD_PYTHON" ] && ! "$MAPMMD_PYTHON" -c "import mapmmd" 2>/dev/null; then
            MAPMMD_PYTHON=""
        fi
    fi
fi
...
if [ -z "$MAPMMD_PYTHON" ]; then
    if command -v python3 >/dev/null 2>&1 && python3 -c "import mapmmd" 2>/dev/null; then
        MAPMMD_PYTHON="python3"
    elif command -v python >/dev/null 2>&1 && python -c "import mapmmd" 2>/dev/null; then
        MAPMMD_PYTHON="python"
    else
        echo "[mapmmd hook] could not locate a Python with mapmmd installed. Add the mapmmd bin dir to PATH or re-run 'mapmmd hook install' from the env where mapmmd lives." >&2
        exit 0
    fi
fi
"""
...
changed_raw = os.environ.get('MAPMMD_CHANGED', '')
changed = [Path(f.strip()) for f in changed_raw.strip().splitlines() if f.strip()]

if not changed:
    sys.exit(0)

print(f'[mapmmd hook] {len(changed)} file(s) changed - rebuilding graph...')

try:
    from mapmmd.watch import _rebuild_code, _apply_resource_limits
    _apply_resource_limits()
    _timeout = int(os.environ.get('MAPMMD_REBUILD_TIMEOUT', '600'))
    if _timeout > 0 and hasattr(signal, 'SIGALRM'):
        signal.signal(signal.SIGALRM, lambda *_: (_ for _ in ()).throw(TimeoutError(f'mapmmd rebuild exceeded {_timeout}s')))
        signal.alarm(_timeout)
    _force = os.environ.get('MAPMMD_FORCE', '').lower() in ('1', 'true', 'yes')
    _root = Path('.')
    _out = os.environ.get('MAPMMD_OUT', 'mapmmd-out')
    _saved = Path(_out) / '.mapmmd_root'
    if _saved.exists():
    ...
    try:
        ...
        if _md.is_dir() and any(_md.glob('*.md')):
            from mapmmd.reflect import reflect as _reflect
    ...
    print(f'[mapmmd hook] {exc}')
    sys.exit(1)
except Exception as exc:
    print(f'[mapmmd hook] Rebuild failed: {exc}')
    sys.exit(1)
"""
...
try:
    ...
    print(f'[mapmmd] Rebuild failed: {exc}')
    sys.exit(1)
"""

# Cross-platform detached-launch shim (#1161). The hooks used to background the
# rebuild with `nohup "$MAPMMD_PYTHON" -c "..." &`, but Git for Windows' bundled
...
# still returned 0, so the graph just went stale with no signal. mapmmd already
...
__REBUILD_BODY__
'''
_log = os.environ.get('MAPMMD_REBUILD_LOG') or os.path.join(os.path.expanduser('~'), '.cache', 'mapmmd-rebuild.log')
try:
...
def _detached_launch(rebuild_body: str) -> str:
    ...
    Python process via ``$MAPMMD_PYTHON``.
    ...
    The launcher writes the child's output to ``$MAPMMD_REBUILD_LOG`` and
    ...
    return '"$MAPMMD_PYTHON" -c "' + launcher + '"\n'


_HOOK_SCRIPT = """\
# mapmmd-hook-start
# Auto-rebuilds the knowledge graph after each commit (code files only, no LLM needed).
# Installed by: mapmmd hook install
...
# churn run-to-run. Pinning it makes mapmmd-out reproducible.
...
# Skip when only mapmmd-out/ artifacts changed (avoids rebuild loop when graph outputs are tracked in git)
_NON_GRAPH=$(echo "$CHANGED" | grep -v '^mapmmd-out/' || true)
if [ -z "$_NON_GRAPH" ]; then
    exit 0
fi

""" + _PYTHON_DETECT + """
export MAPMMD_CHANGED="$CHANGED"
...
_MAPMMD_LOG="${HOME}/.cache/mapmmd-rebuild.log"
mkdir -p "$(dirname "$_MAPMMD_LOG")"
export MAPMMD_REBUILD_LOG="$_MAPMMD_LOG"
echo "[mapmmd hook] launching background rebuild (log: $_MAPMMD_LOG)"
""" + _detached_launch(_REBUILD_BODY_COMMIT) + """# mapmmd-hook-end
"""


_CHECKOUT_SCRIPT = """\
# mapmmd-checkout-hook-start
...
# Only run if mapmmd-out/ exists (graph has been built before)
if [ ! -d "mapmmd-out" ]; then
    exit 0
fi
...
mkdir -p "$(dirname "$_MAPMMD_LOG")"
export MAPMMD_REBUILD_LOG="$_MAPMMD_LOG"
echo "[mapmmd] Branch switched - launching background rebuild (log: $_MAPMMD_LOG)"
""" + _detached_launch(_REBUILD_BODY_CHECKOUT) + """# mapmmd-checkout-hook-end
"""
...
def _hooks_dir(root: Path) -> Path:
    ...
    try:
        ...
        print(
            f"[mapmmd hooks] could not read core.hooksPath from "
...
def _uninstall_hook(hooks_dir: Path, name: str, marker: str, marker_end: str) -> str:
    """Remove mapmmd section from a git hook using start/end markers."""
    hook_path = hooks_dir / name
    if not hook_path.exists():
        return f"no {name} hook found - nothing to remove."
    content = hook_path.read_text(encoding="utf-8")
    if marker not in content:
        return f"mapmmd hook not found in {name} - nothing to remove."
    new_content = re.sub(
    ...
    return f"mapmmd removed from {name} at {hook_path} (other hook content preserved)"
...
def install(path: Path = Path(".")) -> str:
    """Install mapmmd post-commit and post-checkout hooks in the nearest git repo."""
    root = _git_root(path)
    if root is None:
    ...
    # Pin the current interpreter so the hook works even when the mapmmd
...
def uninstall(path: Path = Path(".")) -> str:
    """Remove mapmmd post-commit and post-checkout hooks."""
    root = _git_root(path)
    if root is None:
        raise RuntimeError(f"No git repository found at or above {path.resolve()}")
...
def status(path: Path = Path(".")) -> str:
    """Check if mapmmd hooks are installed."""
    root = _git_root(path)
    if root is None:
    ...
    def _check(name: str, marker: str) -> str:
        ...
        return "installed" if marker in p.read_text(encoding="utf-8") else "not installed (hook exists but mapmmd not found)"
