...
def _safe_extract(extractor: Callable, path: Path) -> dict:
    try:
        ...
        if os.environ.get("MAPMMD_DEBUG"):
            import traceback
...
def _extract_parallel(
    ...
    if max_workers is None:
        # Honour MAPMMD_MAX_WORKERS env override; otherwise scale to the
        ...
        env_raw = os.environ.get("MAPMMD_MAX_WORKERS", "").strip()
        env_cap = None
        if env_raw:
            try:
                v = int(env_raw)
    ...
    # MAPMMD_MAX_WORKERS, and --max-workers — stays valid on >61-core boxes
    ...
    *,
    ...
    Args:
        ...
        max_workers: max subprocess count. Defaults to cpu_count (or the
            value of MAPMMD_MAX_WORKERS if set), bounded by len(uncached_work).
    """
