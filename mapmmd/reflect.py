"""Deterministic "work memory" reflection over mapmmd-out/memory/.

`mapmmd reflect` reads the Q&A memory docs that `mapmmd save-result` files back
...
and a given ``now``. When a graph (`graph.json` + `.mapmmd_analysis.json`) is available
the lessons are also grouped by community label; without it they degrade to a single
flat section.

The artifact lands at ``mapmmd-out/reflections/LESSONS.md`` rather than inside the wiki
because ``mapmmd export wiki`` deletes every ``wiki/*.md`` on each run — a lessons file
...
from mapmmd.ingest import OUTCOMES
...
def _load_node_community(graph_path: Path, analysis_path: Path,
    ...
    Mirrors how `mapmmd export wiki` reads graph.json + .mapmmd_analysis.json +
    .mapmmd_labels.json. Community membership in the analysis sidecar is keyed by
...
def render_lessons_md(agg: dict[str, Any]) -> str:
    ...
    out: list[str] = [
        "# Lessons",
        "",
        f"_Auto-generated by `mapmmd reflect` from {agg['total']} session "
        f"{'memory' if agg['total'] == 1 else 'memories'} in mapmmd-out/memory/. "
...
def lessons_fresh(out_path: Path, memory_dir: Path,
    ...
    Lets ``mapmmd reflect --if-stale`` skip a redundant run — e.g. when the git
...
def reflect(memory_dir: Path, out_path: Path,
    ...
    if graph_path is not None:
        ...
        analysis_path = Path(analysis_path) if analysis_path else (
            graph_path.parent / ".mapmmd_analysis.json")
        labels_path = Path(labels_path) if labels_path else (
            graph_path.parent / ".mapmmd_labels.json")
