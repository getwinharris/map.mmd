...
# Output directory name — override with MAPMMD_OUT env var for worktrees or
...
# (#1423); re-exported here as _MAPMMD_OUT for the existing call sites.
...
def cached_files(root: Path = Path(".")) -> set[str]:
    ...
    base = Path(root).resolve() / _MAPMMD_OUT / "cache"
    hashes: set[str] = set()
    # Legacy flat entries
    if base.is_dir():
