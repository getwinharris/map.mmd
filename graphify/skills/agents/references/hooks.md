...
Run once per project to make mapmmd always-on in your agent sessions:

```bash
mapmmd agents install
```

This writes a `## mapmmd` section to the local `AGENTS.md` that instructs your agent to check the graph before answering codebase questions and rebuild it after code changes. No manual `/mapmmd` needed in future sessions.

```bash
mapmmd agents uninstall  # remove the section
```
