...
def _community_article(
    ...
    lines += ["---", "", "*Part of the mapmmd knowledge wiki. See [[index]] to navigate.*"]
    return "\n".join(lines)
...
def _index_md(
    ...
    lines: list[str] = [
        "# Knowledge mmd Index",
        "",
        "> Auto-generated by mapmmd. Start here — read community articles for context, then drill into god nodes for detail.",
        "",
    ...
    lines += [
        "---",
        "",
        "*Generated by [mapmmd](https://github.com/getwinharris/mapmmd)*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            ...
            "Run `mapmmd extract .` or `mapmmd cluster-only .` first."
        )
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `mapmmd extract .` to regenerate .mapmmd_analysis.json."
        )
