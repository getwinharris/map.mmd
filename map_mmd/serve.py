...
from map_mmd.security import sanitize_label, check_graph_file_size_cap
from map_mmd.build import edge_data
from map_mmd.paths import default_graph_json as _default_graph_json
...
def _load_graph(graph_path: str) -> nx.Graph:
    try:
        ...
        print(f"error: graph.json is corrupted ({exc}). Re-run /map_mmd to rebuild.", file=sys.stderr)
        sys.exit(1)
...
def _subgraph_to_text(G: nx.Graph, nodes: set[str], edges: list[tuple], token_budget: int = 2000, *, seeds: list[str] | None = None) -> str:
    ...
    for nid in ordered:
        ...
        # corpus document can otherwise inject ANSI escapes, fake map.mmd-out
...
def _build_server(graph_path: str):
    ...
    try:
        ...
        raise ImportError('mcp not installed. Run: pip install "map-mmd[mcp]"') from e
    ...
    def _tool_god_nodes(arguments: dict) -> str:
        from map_mmd.analyze import god_nodes as _god_nodes
    ...
    def _tool_list_prs(arguments: dict) -> str:
        from map_mmd.prs import fetch_prs, fetch_worktrees, format_prs_text, _detect_default_branch
    ...
    def _tool_get_pr_impact(arguments: dict) -> str:
        from map_mmd.prs import fetch_pr_files, compute_pr_impact, _gh, _parse_ci
    ...
    def _tool_triage_prs(arguments: dict) -> str:
        ...
        from map_mmd.prs import fetch_prs, fetch_worktrees, fetch_pr_files, compute_pr_impact, _STATUS_ORDER, _detect_default_branch
        repo = arguments.get("repo") or None
        base = arguments.get("base") or _detect_default_branch(repo)
        try:
            prs = fetch_prs(repo=repo, base=base)
    ...
    def _load_community_labels() -> dict[int, str]:
        labels_path = Path(graph_path).parent / ".map_mmd_labels.json"
    ...
    async def list_resources() -> list[types.Resource]:
        return [
            types.Resource(uri=AnyUrl("map_mmd://report"), name="Graph Report", description="Full GRAPH_REPORT.md", mimeType="text/markdown"),
            types.Resource(uri=AnyUrl("map_mmd://stats"), name="Graph Stats", description="Node/edge/community counts and confidence breakdown", mimeType="text/plain"),
            types.Resource(uri=AnyUrl("map_mmd://god-nodes"), name="God Nodes", description="Top 10 most-connected nodes", mimeType="text/plain"),
            types.Resource(uri=AnyUrl("map_mmd://surprises"), name="Surprising Connections", description="Cross-community surprising connections", mimeType="text/plain"),
            types.Resource(uri=AnyUrl("map_mmd://audit"), name="Confidence Audit", description="EXTRACTED/INFERRED/AMBIGUOUS edge breakdown", mimeType="text/plain"),
            types.Resource(uri=AnyUrl("map_mmd://questions"), name="Suggested Questions", description="Suggested questions for this codebase", mimeType="text/plain"),
        ]
    ...
    async def read_resource(uri: AnyUrl) -> str:
        _maybe_reload()
        uri_str = str(uri)
        if uri_str == "map_mmd://report":
            report_path = Path(graph_path).parent / "GRAPH_REPORT.md"
            if report_path.exists():
                return report_path.read_text(encoding="utf-8")
            return "GRAPH_REPORT.md not found. Run map_mmd extract first."
        if uri_str == "map_mmd://stats":
            return _tool_graph_stats({})
        if uri_str == "map_mmd://god-nodes":
            return _tool_god_nodes({"top_n": 10})
        if uri_str == "map_mmd://surprises":
            try:
                from map_mmd.analyze import surprising_connections
        ...
        if uri_str == "map_mmd://questions":
            try:
                from map_mmd.analyze import suggest_questions
    ...
    *,
    ...
    try:
        ...
        raise ImportError(
            ...
            'Run: pip install "map-mmd[mcp]"'
        ) from e

    # A blank key (e.g. --api-key "" or an empty MAP_MMD_API_KEY) must not be
...
def serve_http(
    ...
    *,
    ...
    ``api_key`` (or the ``MAP_MMD_API_KEY`` env var) enables a simple header
    ...
    print(
        f"map_mmd MCP server (streamable-http) on http://{host}:{port}{path} - {auth_note}",
        file=sys.stderr,
    )
    if host in ("0.0.0.0", "::", "") and not api_key:
        print(
            f"WARNING: binding {host or '0.0.0.0'} with no api-key exposes the graph "
            "unauthenticated on the network. Set --api-key (or MAP_MMD_API_KEY).",
            file=sys.stderr,
        )
...
def _main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m map_mmd.serve",
        description="Serve a map_mmd knowledge graph over MCP (stdio or Streamable HTTP).",
    )
    parser.add_argument(
        "graph_path",
        nargs="?",
        default=None,
        help="Path to graph.json (default: map.mmd-out/graph.json)",
    )
    parser.add_argument(
        "--graph",
        dest="graph_flag",
    ...
    parser.add_argument(
        "--api-key",
        default=os.environ.get("MAP_MMD_API_KEY"),
        help="Require this key on the HTTP transport (env: MAP_MMD_API_KEY)",
    )
