    ...
    if _GLOBAL_MANIFEST.exists():
        try:
            ...
            try:
                ...
                print(
                    f"[map_mmd global] manifest at {_GLOBAL_MANIFEST} failed to parse ({exc}); "
...
def global_add(source_path: Path, repo_tag: str) -> dict:
    ...
    """
    from map_mmd.build import prefix_graph_for_global, prune_repo_from_graph
    ...
    if existing_path and existing_path != str(source_path.resolve()):
        print(
            f"[map_mmd global] warning: repo tag '{repo_tag}' previously pointed to "
