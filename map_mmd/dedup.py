"""Entity deduplication pipeline for map_mmd knowledge graphs.
...
from map_mmd._minhash import MinHash, MinHashLSH
...
def deduplicate_entities(
    ...
    *,
    ...
    total = len(remap)
    msg = f"[map_mmd] Deduplicated {total} node(s)"
    if exact_merges:
...
def _llm_tiebreak(
    ...
    *,
    ...
    try:
        from map_mmd.llm import BACKENDS, _format_backend_env_keys, _get_backend_api_key
        if backend not in BACKENDS:
            print(f"[map_mmd] --dedup-llm: unknown backend {backend!r}, skipping LLM tiebreaker.", flush=True)
            return
        if not _get_backend_api_key(backend):
            env_keys = _format_backend_env_keys(backend)
            print(f"[map_mmd] --dedup-llm: {env_keys} not set, skipping LLM tiebreaker.", flush=True)
            return
    except ImportError:
        return
    ...
    try:
        from map_mmd.llm import _call_llm
    except ImportError as exc:
        # F-038: previously this silent fallback hid the fact that `_call_llm`
        # didn't exist in `map_mmd.llm` at all, so `--dedup-llm` was a no-op.
        # Surface the import failure so future regressions are visible.
        print(
            f"[map_mmd] --dedup-llm: cannot import _call_llm ({exc}); skipping LLM tiebreaker.",
            flush=True,
        )
        return
    ...
    for batch_start in range(0, len(ambiguous), batch_size):
        ...
        try:
            ...
            print(f"[map_mmd] --dedup-llm batch failed: {exc}", flush=True)
