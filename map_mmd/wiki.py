...
def _community_article(
    ...
    lines += ["---", "", "*Part of the map_mmd knowledge wiki. See [[index]] to navigate.*"]
    return "\n".join(lines)
...
def _index_md(
    ...
    lines: list[str] = [
        ...
        "",
        "> Auto-generated by map_mmd. Start here — read community articles for context, then drill into god nodes for detail.",
        "",
    ...
    lines += [
        "---",
        "",
        "*Generated by [map_mmd](https://github.com/getwinharris/map.mmd)*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            ...
            "Run `map_mmd extract .` or `map_mmd cluster-only .` first."
        )
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `map_mmd extract .` to regenerate .map_mmd_analysis.json."
        )
