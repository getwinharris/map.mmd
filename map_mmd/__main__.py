"""map_mmd CLI - `map_mmd install` sets up the Claude Code skill."""
...
try:
    ...
    __version__ = _pkg_version("map-mmd")
except Exception:
    __version__ = "unknown"

# Output directory — override with MAP_MMD_OUT env var for worktrees or shared-output setups.
# Accepts a relative name ("map.mmd-out-feature") or an absolute path ("/shared/map.mmd-out").
# Defined once in map_mmd.paths so the security/callflow path guards honour the
# same override (#1423).
...
def _always_on(basename: str) -> str:
    """Read a packaged always-on instruction block from map_mmd/always_on/.
    ...
    try:
        ...
        raise RuntimeError(
            f"map_mmd install is incomplete: missing always-on block '{basename}' "
            f"at {path}. Reinstall map-mmd (e.g. `uv tool install --reinstall map-mmd`)."
        ) from exc
...
def __getattr__(name: str) -> str:
    ...
    # brick the CLI on `import map_mmd.__main__` (#1121 follow-up).
...
def _check_skill_version(skill_dst: Path) -> None:
    """Warn if the installed skill is from an older map_mmd version."""
    version_file = skill_dst.parent / ".map_mmd_version"
    try:
    ...
    try:
        ...
        return
    if not skill_exists:
        print("  warning: skill dir exists but SKILL.md is missing. Run 'map_mmd install' to repair.")
        return
    ...
    if "references/" in body and not (skill_dst.parent / "references").exists():
        print("  warning: skill references/ sidecar is missing. Run 'map_mmd install' to repair.", file=sys.stderr)
    try:
        installed = version_file.read_text(encoding="utf-8").strip()
    except OSError:
        return
    if installed != __version__:
        print(f"  warning: skill is from map_mmd {installed}, package is {__version__}. Run 'map_mmd install' to update.", file=sys.stderr)


def _refresh_all_version_stamps() -> None:
    """After a successful install, update .map_mmd_version in all other known skill dirs.
    ...
    for name in _PLATFORM_CONFIG:
        ...
        vf = skill_dst.parent / ".map_mmd_version"
        if skill_dst.exists():
...
def _platform_skill_destination(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> Path:
    ...
    if platform_name == "gemini":
        if project:
            return (project_dir or Path(".")) / ".gemini" / "skills" / "map_mmd" / "SKILL.md"
        if platform.system() == "Windows":
            return Path.home() / ".agents" / "skills" / "map_mmd" / "SKILL.md"
        return Path.home() / ".gemini" / "skills" / "map_mmd" / "SKILL.md"

    if platform_name == "opencode":
        if project:
            return (project_dir or Path(".")) / ".opencode" / "skills" / "map_mmd" / "SKILL.md"
        return Path.home() / ".config" / "opencode" / "skills" / "map_mmd" / "SKILL.md"

    if platform_name == "hermes":
        if project:
            return (project_dir or Path(".")) / ".hermes" / "skills" / "map_mmd" / "SKILL.md"
        # On Windows, Hermes scans %LOCALAPPDATA%\hermes\skills, not ~/.hermes (#1403).
        if platform.system() == "Windows":
            local_appdata = Path(os.environ.get("LOCALAPPDATA") or (Path.home() / "AppData" / "Local"))
            return local_appdata / "hermes" / "skills" / "map_mmd" / "SKILL.md"
        return Path.home() / ".hermes" / "skills" / "map_mmd" / "SKILL.md"

    if platform_name == "devin":
        if project:
            return (project_dir or Path(".")) / ".devin" / "skills" / "map_mmd" / "SKILL.md"
        return Path.home() / ".config" / "devin" / "skills" / "map_mmd" / "SKILL.md"

    if platform_name == "amp":
        if project:
            return (project_dir or Path(".")) / ".agents" / "skills" / "map_mmd" / "SKILL.md"
        return Path.home() / ".config" / "agents" / "skills" / "map_mmd" / "SKILL.md"
    ...
    if platform_name in ("antigravity", "antigravity-windows"):
        ...
        return Path.home() / ".gemini" / "config" / "skills" / "map_mmd" / "SKILL.md"
    ...
    if platform_name in ("claude", "windows") and os.environ.get("CLAUDE_CONFIG_DIR"):
        return Path(os.environ["CLAUDE_CONFIG_DIR"]) / "skills" / "map_mmd" / "SKILL.md"
...
def _packaged_skill_refs_dir(platform_name: str) -> Path | None:
    ...
    ``map_mmd/skills/<bundle>/references/``. Reuse keys (e.g. trae-cn) point at
    their twin's bundle.
    ...
    ``map_mmd/skills/<bundle>/`` is not in this build has not gone progressive
...
def _copy_skill_file(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> Path:
    ...
    ``.map_mmd_version`` stamp covers both. For monolith platforms (no
    ...
    if not skill_src.exists():
        print(f"error: {skill_file} not found in package - reinstall map_mmd", file=sys.stderr)
        sys.exit(1)
    ...
    (skill_dst.parent / ".map_mmd_version").write_text(__version__, encoding="utf-8")
...
def _remove_claude_skill_registration(project_dir: Path) -> None:
    ...
    if "# map_mmd" not in content:
        return
    cleaned = re.sub(r"\n*# map_mmd\n.*?(?=\n# |\Z)", "", content, flags=re.DOTALL).rstrip()
    if cleaned:
        claude_md.write_text(cleaned + "\n", encoding="utf-8")
        print(f"  CLAUDE.md        ->  map_mmd skill registration removed from {claude_md}")
    else:
        claude_md.unlink()
    ...
    "hooks": [
        {
            ...
            "command": (
                ...
                "  [ -f map.mmd-out/graph.json ] && "
                r"""  echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"MANDATORY: map.mmd-out/graph.json exists. You MUST run `map_mmd query \"<question>\"` before grepping raw files. Only grep after map_mmd has oriented you, or to modify/debug specific lines."}}' """
                "  || true ;; "
                "esac"
            ),
        }
...
_READ_SETTINGS_HOOK = {
    ...
    # source/doc file outside map.mmd-out/ when a graph exists. The parser is
    # python3 (already a map_mmd dependency), the shell is POSIX, and every branch
    # fails open, so a legitimate read always goes through. Reading the graph's own
    # report under map.mmd-out/ is suppressed so it never starts a feedback loop.
    ...
    "hooks": [
        {
            ...
            "command": (
                ...
                "sys.stdout.write('1' if 'map.mmd-out/' not in j and any(tl in exts for tl in tails) else '')\" 2>/dev/null || true); "
                "if [ \"$HIT\" = 1 ] && [ -f map.mmd-out/graph.json ]; then "
                r"""echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"MANDATORY: map.mmd-out/graph.json exists. You MUST run map_mmd before reading source files. Use: `map_mmd query \"<question>\"` (scoped subgraph), `map_mmd explain \"<concept>\"`, or `map_mmd path \"<A>\" \"<B>\"`. Only read raw files after map_mmd has oriented you, or to modify/debug specific lines. This rule applies to subagents too — include it in every subagent prompt involving code exploration."}}'; """
                "fi || true"
            ),
        }
    ],
...
def _skill_registration(skill_path: str = "~/.claude/skills/map_mmd/SKILL.md") -> str:
    return (
        "\n# map_mmd\n"
        f"- **map_mmd** (`{skill_path}`) "
        "- any input to knowledge graph. Trigger: `/map_mmd`\n"
        "When the user types `/map_mmd`, invoke the Skill tool "
        "with `skill: \"map_mmd\"` before doing anything else.\n"
    )
...
_PLATFORM_CONFIG: dict[str, dict] = {
    "claude": {
        "skill_file": "skill.md",
        "skill_dst": Path(".claude") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": True,
        "skill_refs": "claude",
    },
    "codex": {
        "skill_file": "skill-codex.md",
        "skill_dst": Path(".codex") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "codex",
    },
    "opencode": {
        "skill_file": "skill-opencode.md",
        "skill_dst": Path(".config") / "opencode" / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "opencode",
    },
    "kilo": {
        "skill_file": "skill-kilo.md",
        "skill_dst": Path(".config") / "kilo" / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "kilo",
    },
    "aider": {
        ...
        "skill_dst": Path(".aider") / "map_mmd" / "SKILL.md",
        "claude_md": False,
    },
    "copilot": {
        "skill_file": "skill-copilot.md",
        "skill_dst": Path(".copilot") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "copilot",
    },
    "claw": {
        "skill_file": "skill-claw.md",
        "skill_dst": Path(".openclaw") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claw",
    },
    "droid": {
        "skill_file": "skill-droid.md",
        "skill_dst": Path(".factory") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "droid",
    },
    "trae": {
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "trae",
    },
    "trae-cn": {
        ...
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae-cn") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "trae",
    },
    "hermes": {
        ...
        "skill_file": "skill-claw.md",
        "skill_dst": Path(".hermes") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claw",
    },
    "kiro": {
        "skill_file": "skill-kiro.md",
        "skill_dst": Path(".kiro") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "kiro",
    },
    "pi": {
        "skill_file": "skill-pi.md",
        "skill_dst": Path(".pi") / "agent" / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "pi",
    },
    "codebuddy": {
        ...
        "skill_file": "skill.md",
        "skill_dst": Path(".codebuddy") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "antigravity": {
        ...
        "skill_file": "skill.md",
        "skill_dst": Path(".agents") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "antigravity-windows": {
    ...
    "kimi": {
        # Reuses claude's split bundle (shares skill.md).
        "skill_file": "skill.md",
        "skill_dst": Path(".kimi") / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "amp": {
    ...
    "devin": {
        ...
        # User scope: ~/.config/devin/skills/map_mmd/SKILL.md
        # Project scope: .devin/skills/map_mmd/SKILL.md (overridden in _platform_skill_destination)
        "skill_dst": Path(".config") / "devin" / "skills" / "map_mmd" / "SKILL.md",
        "claude_md": False,
    },
}
...
def _replace_or_append_section(content: str, marker: str, new_section: str) -> str:
    """Idempotently update or append a map_mmd-owned section in shared files.
...
def _print_banner() -> None:
    """Amber brain banner on map_mmd install. TTY-only, never raises."""
...
def install(platform: str = "claude", *, project: bool = False, project_dir: Path | None = None) -> None:
    ...
    if platform == "kilo":
        # Kilo Code also supports a native /map_mmd command file.
        command_src = Path(__file__).parent / "command-kilo.md"
        if not command_src.exists():
            print(
                f"error: command-kilo.md not found in package - reinstall map_mmd",
                file=sys.stderr,
            )
            sys.exit(1)
        command_dst = Path.home() / ".config" / "kilo" / "command" / "map_mmd.md"
    ...
    if cfg["claude_md"]:
        ...
        registration = _skill_registration(".claude/skills/map_mmd/SKILL.md" if project else "~/.claude/skills/map_mmd/SKILL.md")
        if claude_md.exists():
            content = claude_md.read_text(encoding="utf-8")
            if "map_mmd" in content:
    ...
    if platform == "codebuddy":
        ...
        registration = _skill_registration("~/.codebuddy/skills/map_mmd/SKILL.md")
...
def _print_install_usage() -> None:
    ...
    print("Usage: map_mmd install [--project] [--platform P|P]")
    print(f"Platforms: {platforms}")


# The always-on instruction blocks are packaged markdown under map_mmd/always_on/,
...
_CLAUDE_MD_MARKER = "## map_mmd"

_CODEBUDDY_MD_MARKER = "## map_mmd"
...
_AGENTS_MD_MARKER = "## map_mmd"


_GEMINI_MD_MARKER = "## map_mmd"
    ...
    "hooks": [
        {
            ...
            "command": (
                ...
                "e=pathlib.Path('map.mmd-out/graph.json').exists();"
                "d={'decision':'allow'};"
                "e and d.update({'additionalContext':'map_mmd: knowledge graph at map.mmd-out/. For focused questions, run `map_mmd query \"<question>\"` (scoped subgraph, usually much smaller than GRAPH_REPORT.md) instead of grepping raw files. Read GRAPH_REPORT.md only for broad architecture context.'});"
...
def gemini_install(project_dir: Path | None = None, *, project: bool = False) -> None:
    ...
    if target.exists() and new_content == target.read_text(encoding="utf-8"):
        print(f"map_mmd already configured in {target.resolve()} (no change)")
    else:
        target.write_text(new_content, encoding="utf-8")
        print(f"map_mmd section written to {target.resolve()}")
...
def _install_gemini_hook(project_dir: Path) -> None:
    ...
    settings["hooks"]["BeforeTool"] = [
        h for h in before_tool if "map_mmd" not in str(h)
    ]
...
def _uninstall_gemini_hook(project_dir: Path) -> None:
    ...
    filtered = [h for h in before_tool if "map_mmd" not in str(h)]
...
def gemini_uninstall(project_dir: Path | None = None, *, project: bool = False) -> None:
    """Remove the map_mmd section from GEMINI.md, uninstall hook, and remove skill file."""
    project_dir = project_dir or Path(".")
    ...
    if _GEMINI_MD_MARKER not in content:
        print("map_mmd section not found in GEMINI.md - nothing to do")
        return
    cleaned = re.sub(
    ...
    if cleaned:
        ...
        print(f"map_mmd section removed from {target.resolve()}")
    else:
        target.unlink()
...
_VSCODE_INSTRUCTIONS_MARKER = "## map_mmd"


def vscode_install(project_dir: Path | None = None) -> None:
    """Install map_mmd skill for VS Code Copilot Chat + write .github/copilot-instructions.md."""
    skill_src = Path(__file__).parent / "skill-vscode.md"
    refs_bundle = "vscode"
    if not skill_src.exists():
        skill_src = Path(__file__).parent / "skill-copilot.md"
        refs_bundle = "copilot"
    skill_dst = Path.home() / ".copilot" / "skills" / "map_mmd" / "SKILL.md"
    skill_dst.parent.mkdir(parents=True, exist_ok=True)
    tmp_dst = skill_dst.with_suffix(skill_dst.suffix + ".tmp")
    try:
        shutil.copy(skill_src, tmp_dst)
    ...
    if instructions.exists():
        ...
        if new_content == content:
            ...
            print(f"  {instructions}  ->  map_mmd section {'updated' if _VSCODE_INSTRUCTIONS_MARKER in content else 'added'}")
    else:
    ...
    print()
    print(
        "VS Code Copilot Chat configured. Type /map_mmd in the chat panel to build the graph."
    )
    print("Note: for GitHub Copilot CLI (terminal), use: map_mmd copilot install")


def vscode_uninstall(project_dir: Path | None = None) -> None:
    """Remove map_mmd VS Code Copilot Chat skill and .github/copilot-instructions.md section."""
    skill_dst = Path.home() / ".copilot" / "skills" / "map_mmd" / "SKILL.md"
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"  skill removed    ->  {skill_dst}")
    ...
    if cleaned:
        ...
        print(f"  map_mmd section removed from {instructions}")
    else:
        instructions.unlink()
        print(f"  {instructions}  ->  deleted (was empty after removal)")


_ANTIGRAVITY_RULES_PATH = Path(".agents") / "rules" / "map_mmd.md"
_ANTIGRAVITY_WORKFLOW_PATH = Path(".agents") / "workflows" / "map_mmd.md"
...
Follow the map_mmd skill installed at ~/.gemini/config/skills/map_mmd/SKILL.md to run the full pipeline.
...
_KIRO_STEERING_MARKER = "map_mmd: A knowledge graph of this project"


def _kiro_install(project_dir: Path) -> None:
    """Write map_mmd skill + steering file for Kiro IDE/CLI."""
    project_dir = project_dir or Path(".")

    # Skill file + references/ sidecar + .map_mmd_version stamp via the shared
    ...
    # Steering file → .kiro/steering/map_mmd.md (always-on)
    steering_dir = project_dir / ".kiro" / "steering"
    steering_dir.mkdir(parents=True, exist_ok=True)
    steering_dst = steering_dir / "map_mmd.md"
    if steering_dst.exists() and steering_dst.read_text(encoding="utf-8") == _always_on("kiro-steering"):
        print(f"  .kiro/steering/map_mmd.md  ->  already configured (no change)")
    else:
        # File is wholly map_mmd-owned. Overwrite on upgrade so older
        ...
        print(f"  .kiro/steering/map_mmd.md  ->  always-on steering {action}")

    print()
    print("Kiro will now read the knowledge graph before every conversation.")
    print("Use /map_mmd to build or update the graph.")


def _kiro_uninstall(project_dir: Path) -> None:
    """Remove map_mmd skill + steering file for Kiro."""
    project_dir = project_dir or Path(".")
    removed = []

    # Skill + .map_mmd_version + references/ sidecar + empty-dir walk.
    ...
    steering_dst = project_dir / ".kiro" / "steering" / "map_mmd.md"
...
def _antigravity_finalize(skill_dst: Path, project_dir: Path) -> None:
    ...
    writes ``.agents/rules/map_mmd.md`` and ``.agents/workflows/map_mmd.md``
    ...
    if rules_path.exists():
        ...
        if _always_on("antigravity-rules").strip() != existing.strip():
            ...
            print(f"map_mmd rule updated at {rules_path.resolve()}")
        else:
            print(f"map_mmd rule already configured at {rules_path.resolve()} (no change)")
    else:
        rules_path.write_text(_always_on("antigravity-rules"), encoding="utf-8")
        print(f"map_mmd rule written to {rules_path.resolve()}")

    # .agents/workflows/map_mmd.md
    ...
    if wf_path.exists():
        ...
        if _ANTIGRAVITY_WORKFLOW.strip() != existing.strip():
            ...
            print(f"map_mmd workflow updated at {wf_path.resolve()}")
        else:
            print(f"map_mmd workflow already configured at {wf_path.resolve()} (no change)")
    else:
        wf_path.write_text(_ANTIGRAVITY_WORKFLOW, encoding="utf-8")
        print(f"map_mmd workflow written to {wf_path.resolve()}")


def _antigravity_install(project_dir: Path) -> None:
    """Install map_mmd for Google Antigravity (global skill + .agents/rules + .agents/workflows)."""
    # Copy the skill to ~/.gemini/config/skills/map_mmd/SKILL.md (global), then
    ...
    print("codebase questions. Run /map_mmd first to build the graph.")
    print()
    print(
    ...
    print('    "command": "uv",')
    print(
        '    "args": ["run", "--with", "map-mmd", "--with", "mcp", "-m", "map_mmd.serve", "${workspace.path}/map.mmd-out/graph.json"]'
    )
    print("  }")


def _antigravity_uninstall(project_dir: Path, *, project: bool = False) -> None:
    """Remove map_mmd Antigravity rules, workflow, and skill files."""
    # Remove rules file
    rules_path = project_dir / _ANTIGRAVITY_RULES_PATH
    if rules_path.exists():
        rules_path.unlink()
        print(f"map_mmd rule removed from {rules_path.resolve()}")
    else:
        print("No map_mmd Antigravity rule found - nothing to do")

    # Remove workflow file
    wf_path = project_dir / _ANTIGRAVITY_WORKFLOW_PATH
    if wf_path.exists():
        wf_path.unlink()
        print(f"map_mmd workflow removed from {wf_path.resolve()}")

    # Remove skill file
    skill_dst = _platform_skill_destination("antigravity", project=project, project_dir=project_dir)
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"map_mmd skill removed from {skill_dst}")
    version_file = skill_dst.parent / ".map_mmd_version"
    if version_file.exists():
        version_file.unlink()
    refs_dir = skill_dst.parent / "references"
...
_CURSOR_RULE_PATH = Path(".cursor") / "rules" / "map_mmd.mdc"
_CURSOR_RULE = """\
---
...
**MANDATORY: Before using Read, Grep, Glob, or Bash to explore the codebase, you MUST run map_mmd first:**
- `map_mmd query "<question>"` — scoped subgraph for any codebase or architecture question
- `map_mmd path "<A>" "<B>"` — dependency path between two symbols
- `map_mmd explain "<concept>"` — all nodes related to a concept

This applies to YOU and to every subagent you spawn. Include this rule explicitly in every subagent prompt that involves code exploration. Do not skip map_mmd because files are "already known" or because you are executing a plan — the graph surfaces cross-file dependencies and INFERRED edges that grep and Read cannot find.

Only use Read/Grep/Glob directly when:
1. map_mmd has already oriented you and you need to modify or debug specific lines
2. `map.mmd-out/graph.json` does not exist yet

- If `map.mmd-out/wiki/index.md` exists, navigate it instead of reading raw files
- Read `map.mmd-out/GRAPH_REPORT.md` only for broad architecture review when query/path/explain do not surface enough context
- After modifying code files, run `map_mmd update .` to keep the graph current (AST-only, no API cost)
"""


def _cursor_install(project_dir: Path) -> None:
    """Write .cursor/rules/map_mmd.mdc with alwaysApply: true."""
    ...
    if rule_path.exists() and rule_path.read_text(encoding="utf-8") == _CURSOR_RULE:
        print(f"map_mmd rule at {rule_path} already configured (no change)")
        return
    # File is wholly map_mmd-owned. Overwrite on upgrade so older
    # report-first wording does not silently linger (issue #580).
    action = "updated" if rule_path.exists() else "written"
    rule_path.write_text(_CURSOR_RULE, encoding="utf-8")
    print(f"map_mmd rule {action} at {rule_path.resolve()}")
    print()
    print("Cursor will now always include the knowledge graph context.")
    print("Run /map_mmd . first to build the graph if you haven't already.")


def _cursor_uninstall(project_dir: Path) -> None:
    """Remove .cursor/rules/map_mmd.mdc."""
    rule_path = (project_dir or Path(".")) / _CURSOR_RULE_PATH
    if not rule_path.exists():
        print("No map_mmd Cursor rule found - nothing to do")
        return
    rule_path.unlink()
    print(f"map_mmd Cursor rule removed from {rule_path.resolve()}")


# Devin CLI — .windsurf/rules/map_mmd.md (always-on context)
# Devin reads .windsurf/rules/*.md files the same way Windsurf IDE does.
_DEVIN_RULES_PATH = Path(".windsurf") / "rules" / "map_mmd.md"
_DEVIN_RULES = """\
## map_mmd
...
Rules:
- For codebase or architecture questions, when `map.mmd-out/graph.json` exists, first run `map_mmd query "<question>"` (or `map_mmd path "<A>" "<B>"` / `map_mmd explain "<concept>"`). These return a scoped subgraph, usually much smaller than `GRAPH_REPORT.md` or raw grep output.
- If map.mmd-out/wiki/index.md exists, navigate it instead of reading raw files
...
- After modifying code files in this session, run `map_mmd update .` to keep the graph current (AST-only, no API cost)
"""


def _devin_rules_install(project_dir: Path) -> None:
    """Write .windsurf/rules/map_mmd.md for always-on Devin context."""
...
def _devin_rules_uninstall(project_dir: Path) -> None:
    """Remove .windsurf/rules/map_mmd.md."""
    rules_path = (project_dir or Path(".")) / _DEVIN_RULES_PATH
...
export const map.mmdPlugin = async ({ directory }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      if (reminded) return;
      if (!existsSync(join(directory, "map.mmd-out", "graph.json"))) return;

      if (input.tool === "bash") {
        output.args.command =
          'echo "[map_mmd] Knowledge graph available. Read map.mmd-out/GRAPH_REPORT.md for god nodes and architecture context before searching files." && ' +
          output.args.command;
        reminded = true;
      }
    },
...
_KILO_PLUGIN_PATH = Path(".kilo") / "plugins" / "map_mmd.js"
...
def _install_kilo_plugin(project_dir: Path) -> None:
    """Write map_mmd.js plugin and register it without rewriting user JSONC."""
...
def _uninstall_kilo_plugin(project_dir: Path) -> None:
    """Remove map_mmd.js plugin and deregister it without rewriting user JSONC."""
    plugin_file = project_dir / _KILO_PLUGIN_PATH
...
_OPENCODE_PLUGIN_JS = """\
// map_mmd OpenCode plugin
// Injects a knowledge graph reminder before bash tool calls when the graph exists.
//
// IMPORTANT: keep the reminder string free of backticks and $(...) constructs.
// The hook prepends `echo "<reminder>" && <cmd>` to the user's bash command;
// backticks inside the double-quoted echo trigger bash command substitution,
// which both corrupts tool output and silently executes the very map_mmd
...
export const map.mmdPlugin = async ({ directory }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      if (input.tool === "bash") {
        output.args.command =
          'echo "[map_mmd] knowledge graph at map.mmd-out/. For focused questions, run map_mmd query with your question (scoped subgraph, usually much smaller than GRAPH_REPORT.md) instead of grepping raw files. Read GRAPH_REPORT.md only for broad architecture context." && ' +
          output.args.command;
        reminded = true;
      }
    },
...
_OPENCODE_PLUGIN_PATH = Path(".opencode") / "plugins" / "map_mmd.js"
_OPENCODE_CONFIG_PATH = Path(".opencode") / "opencode.json"


def _install_opencode_plugin(project_dir: Path) -> None:
    """Write map_mmd.js plugin and register it in opencode.json."""
...
def _uninstall_opencode_plugin(project_dir: Path) -> None:
    """Remove map_mmd.js plugin and deregister from opencode.json."""
    plugin_file = project_dir / _OPENCODE_PLUGIN_PATH
    if plugin_file.exists():
        plugin_file.unlink()
        ...
        "PreToolUse": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        # Use the map_mmd CLI itself so the hook is shell-agnostic:
...
def _resolve_map_mmd_exe() -> str:
    """Return the absolute path to the map_mmd executable.

    Falls back to bare 'map_mmd' if resolution fails. Using an absolute path
...
def _install_codex_hook(project_dir: Path) -> None:
    """Add map_mmd PreToolUse hook to .codex/hooks.json."""
    ...
    map_mmd_exe = _resolve_map_mmd_exe()
    hook_entry = {
        "hooks": {
            "PreToolUse": [
                {
                    "matcher": "Bash",
                    "hooks": [{"type": "command", "command": f"{map_mmd_exe} hook-check"}],
                }
            ]
        }
    }
    ...
    existing["hooks"]["PreToolUse"] = [h for h in pre_tool if "map_mmd" not in str(h)]
    existing["hooks"]["PreToolUse"].extend(hook_entry["hooks"]["PreToolUse"])
    hooks_path.write_text(json.dumps(existing, indent=2), encoding="utf-8")
    print(f"  .codex/hooks.json  ->  PreToolUse hook registered ({map_mmd_exe} hook-check)")


def _uninstall_codex_hook(project_dir: Path) -> None:
    """Remove map_mmd PreToolUse hook from .codex/hooks.json."""
    hooks_path = project_dir / ".codex" / "hooks.json"
    ...
    filtered = [h for h in pre_tool if "map_mmd" not in str(h)]
...
def _agents_install(project_dir: Path, platform: str) -> None:
    """Write the map_mmd section to the local AGENTS.md for always-on platforms."""
...
def _amp_legacy_cleanup() -> None:
    """Best-effort removal of the pre-fix ~/.amp/skills/map_mmd install dir.

    Older map_mmd versions wrote the Amp skill to ~/.amp/skills, which Amp does
    ...
    """
    legacy = Path.home() / ".amp" / "skills" / "map_mmd"
    if legacy.exists():
...
def _agents_platform_install(project_dir: Path | None = None) -> None:
    """`map_mmd agents install`: skill into ~/.agents/skills + AGENTS.md.
    ...
    rendered hooks reference, which points at `map_mmd agents install`. The bare
    `map_mmd install --platform agents` path stays skill-only (via install()),
...
def _agents_platform_uninstall(project_dir: Path | None = None) -> None:
    """`map_mmd agents uninstall`: remove the skill and the AGENTS.md section."""
...
def _project_install(platform_name: str, project_dir: Path | None = None) -> None:
    ...
    if platform_name in ("claude", "windows"):
        ...
        # root. `agents` -> ./.agents/skills/map_mmd/SKILL.md.
        skill_dst = _copy_skill_file(platform_name, project=True, project_dir=project_dir)
        _print_project_git_add_hint([_project_scope_root(skill_dst, project_dir)])
    else:
        install(platform=platform_name, project=True, project_dir=project_dir)
...
def _project_uninstall_all(project_dir: Path | None = None) -> None:
    ...
    project_dir = project_dir or Path(".")
    print("Uninstalling project-scoped map_mmd files...\n")
...
def _agents_uninstall(project_dir: Path, platform: str = "") -> None:
    """Remove the map_mmd section from the local AGENTS.md."""
    target = (project_dir or Path(".")) / "AGENTS.md"
    ...
    if _AGENTS_MD_MARKER not in content:
        print("map_mmd section not found in AGENTS.md - nothing to do")
        if platform == "opencode":
            _uninstall_opencode_plugin(project_dir or Path("."))
        elif platform == "kilo":
            _uninstall_kilo_plugin(project_dir or Path("."))
...
def claude_install(project_dir: Path | None = None) -> None:
    """Write the map_mmd section to the local CLAUDE.md."""
...
def _install_claude_hook(project_dir: Path) -> None:
    """Add map_mmd PreToolUse hook to .claude/settings.json."""
    ...
    hooks["PreToolUse"] = [h for h in pre_tool if not (h.get("matcher") in ("Glob|Grep", "Bash", "Read|Glob") and "map_mmd" in str(h))]
...
def _uninstall_claude_hook(project_dir: Path) -> None:
    """Remove map_mmd PreToolUse hook from .claude/settings.json."""
    settings_path = project_dir / ".claude" / "settings.json"
    if not settings_path.exists():
        return
    try:
    ...
    filtered = [h for h in pre_tool if not (h.get("matcher") in ("Glob|Grep", "Bash", "Read|Glob") and "map_mmd" in str(h))]
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    """Remove map_mmd from every platform detected in the current project."""
    pd = project_dir or Path(".")
    print("Uninstalling map_mmd from all detected platforms...\n")
    ...
    # Git hook
    try:
        from map_mmd.hooks import uninstall as hook_uninstall
    ...
    if purge:
        ...
        if out.exists():
            _shutil.rmtree(out)
            print(f"\n  {_MAP_MMD_OUT}/  ->  deleted (--purge)")
        else:
            print(f"\n  {_MAP_MMD_OUT}/  ->  not found (nothing to purge)")

    print("\nDone. Run 'pip uninstall map-mmd' to remove the package itself.")


def claude_uninstall(project_dir: Path | None = None, *, project: bool = False) -> None:
    """Remove the map_mmd skill tree (SKILL.md + references/) and the CLAUDE.md section.

    Mirrors gemini_uninstall: the bare `map_mmd uninstall` and `map_mmd claude
    ...
    if _CLAUDE_MD_MARKER not in content:
        print("map_mmd section not found in CLAUDE.md - nothing to do")
        return

    # Remove the ## map_mmd section: from the marker to the next ## heading or EOF
    cleaned = re.sub(
        r"\n*## map_mmd\n.*?(?=\n## |\Z)",
        "",
        content,
...
def codebuddy_install(project_dir: Path | None = None) -> None:
    """Install the map_mmd skill and CODEBUDDY.md section for CodeBuddy."""
...
def _install_codebuddy_hook(project_dir: Path) -> None:
    """Add map_mmd PreToolUse hook to .codebuddy/settings.json."""
...
def _uninstall_codebuddy_hook(project_dir: Path) -> None:
    """Remove map_mmd PreToolUse hook from .codebuddy/settings.json."""
    settings_path = project_dir / ".codebuddy" / "settings.json"
    if not settings_path.exists():
        return
    try:
...
def codebuddy_uninstall(project_dir: Path | None = None, *, project: bool = False) -> None:
    """Remove the map_mmd skill tree (SKILL.md + references/) and the CODEBUDDY.md section."""
    project_dir = project_dir or Path(".")
    ...
    if _CODEBUDDY_MD_MARKER not in content:
        print("map_mmd section not found in CODEBUDDY.md - nothing to do")
        return
...
def _clone_repo(
    ...
    Clones into ~/.map_mmd/repos/<owner>/<repo> by default so repeated
    ...
    if out_dir:
        ...
        dest = Path.home() / ".map_mmd" / "repos" / owner / repo
...
def main() -> None:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] in ("-v", "--version", "version"):
        print(f"map_mmd {__version__}")
        return

    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        print("Usage: map_mmd <command>")
        print()
        print("Commands:")
        ...
        print("  uninstall               remove map_mmd from all detected platforms in one shot")
        print("    --purge                 also delete map.mmd-out/ directory")
        print("  path \"A\" \"B\"            shortest path between two nodes in graph.json")
        print("    --graph <path>          path to graph.json (default map.mmd-out/graph.json)")
        ...
        print("                            (default map.mmd-out/graph.json)")
        ...
        print("  clone <github-url>      clone a GitHub repo locally and print its path for /map_mmd")
        ...
        print("    --out <path>            output path (default: map.mmd-out/merged-graph.json)")
        print("    --branch <branch>       checkout a specific branch (default: repo default)")
        print("    --out <dir>             clone to a custom directory (default: ~/.map_mmd/repos/<owner>/<repo>)")
        ...
        print("                            (also: MAP_MMD_FORCE=1 env var; use after refactors that delete code)")
        print("    --no-cluster            skip clustering, write raw extraction only")
        print("  cluster-only <path>     rerun clustering on an existing graph.json and regenerate report")
        print("    --no-viz                skip graph.html generation (useful for >5000 node graphs / CI)")
        print("    --graph <path>          path to graph.json (default <path>/map.mmd-out/graph.json)")
        ...
        print("    --graph <path>          path to graph.json (default map.mmd-out/graph.json)")
        print("  save-result             save a Q&A result to map.mmd-out/memory/ for graph feedback loop")
        ...
        print("    --memory-dir DIR        memory directory (default: map.mmd-out/memory)")
        print("  reflect                 aggregate map.mmd-out/memory/ outcomes into a deterministic lessons doc")
        print("    --memory-dir DIR        memory directory (default: map.mmd-out/memory)")
        print("    --out FILE              output path (default: map.mmd-out/reflections/LESSONS.md)")
        print("    --graph PATH            graph.json, for community grouping + dropping stale nodes (optional)")
        print("    --analysis PATH         .map_mmd_analysis.json (optional, auto-detected next to --graph)")
        print("    --labels PATH           .map_mmd_labels.json (optional, auto-detected next to --graph)")
        ...
        print("    --graph PATH            path to graph.json (default map.mmd-out/graph.json)")
        print("    --output HTML           output path (default map.mmd-out/GRAPH_TREE.html)")
        ...
        print("    --out DIR               output dir (default: <path>); writes <DIR>/map.mmd-out/")
        ...
        print("  global add <graph.json>  add/update a project graph in the global graph (~/.map_mmd/global-graph.json)")
        ...
        print("  cursor install          write .cursor/rules/map_mmd.mdc (Cursor)")
        print("  cursor uninstall        remove .cursor/rules/map_mmd.mdc")
        print("  claude install          write map_mmd section to CLAUDE.md + PreToolUse hook (Claude Code)")
        print("  claude uninstall        remove map_mmd section from CLAUDE.md + PreToolUse hook")
        print("  codebuddy install       write map_mmd section to CODEBUDDY.md + PreToolUse hook (CodeBuddy)")
        print("  codebuddy uninstall     remove map_mmd section from CODEBUDDY.md + PreToolUse hook")
        print("  codex install           write map_mmd section to AGENTS.md (Codex)")
        print("  codex uninstall         remove map_mmd section from AGENTS.md")
        print(
            "  opencode install        write map_mmd section to AGENTS.md + tool.execute.before plugin (OpenCode)"
        )
        print(
            "  opencode uninstall      remove map_mmd section from AGENTS.md + plugin"
        )
        print(
        ...
        print("  aider install           write map_mmd section to AGENTS.md (Aider)")
        print("  aider uninstall         remove map_mmd section from AGENTS.md")
        print(
            "  copilot install         copy map_mmd skill to ~/.copilot/skills (GitHub Copilot CLI)"
        )
        print("  copilot uninstall       remove map_mmd skill from ~/.copilot/skills")
        print(
        ...
        print(
            "  claw install            write map_mmd section to AGENTS.md (OpenClaw)"
        )
        print("  claw uninstall          remove map_mmd section from AGENTS.md")
        print(
            "  droid install           write map_mmd section to AGENTS.md (Factory Droid)"
        )
        print("  droid uninstall        remove map_mmd section from AGENTS.md")
        print("  trae install            write map_mmd section to AGENTS.md (Trae)")
        print("  trae uninstall         remove map_mmd section from AGENTS.md")
        print("  trae-cn install         write map_mmd section to AGENTS.md (Trae CN)")
        print("  trae-cn uninstall      remove map_mmd section from AGENTS.md")
        print(
        ...
        print(
            "  hermes install          write skill to ~/.hermes/skills/map_mmd/ (Hermes)"
        )
        print("  hermes uninstall        remove skill from ~/.hermes/skills/map_mmd/")
        print(
            "  kiro install            write skill to .kiro/skills/map_mmd/ + steering file (Kiro IDE/CLI)"
        )
        print("  kiro uninstall          remove skill + steering file")
        print("  pi install              write skill to ~/.pi/agent/skills/map_mmd/ (Pi coding agent)")
        print("  pi uninstall            remove skill from ~/.pi/agent/skills/map_mmd/")
        print("  devin install           write skill to ~/.config/devin/skills/map_mmd/ (Devin CLI)")
        print("  devin uninstall         remove skill from ~/.config/devin/skills/map_mmd/")
        print()
        return
    ...
    if cmd not in _FREE_TEXT_CMDS and any(a in {"-h", "--help", "-?"} for a in sys.argv[2:]):
        print(f"Run 'map_mmd --help' for full usage.")
        return
    ...
    if cmd == "install":
        ...
        if subcmd == "install":
            ...
            print("Usage: map_mmd claude [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "codebuddy":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: map_mmd codebuddy [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "gemini":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: map_mmd gemini [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "cursor":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: map_mmd cursor [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "vscode":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: map_mmd vscode [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "copilot":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: map_mmd copilot [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kilo":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: map_mmd kilo [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kiro":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: map_mmd kiro [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "devin":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: map_mmd devin [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "pi":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: map_mmd pi [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "amp":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: map_mmd amp [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print(f"Usage: map_mmd {cmd} [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print("Usage: map_mmd antigravity [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "provider":
        from map_mmd.llm import _custom_providers_path, BACKENDS
        ...
        if subcmd == "list":
            ...
            if not name:
                print("Usage: map_mmd provider show <name>", file=sys.stderr)
                sys.exit(1)
            existing = {}
            if global_path.is_file():
                try:
            ...
            if not name:
                print("Usage: map_mmd provider add <name> --base-url URL --default-model MODEL --env-key KEY", file=sys.stderr)
                sys.exit(1)
            if name in BACKENDS:
            ...
            if not base_url or not default_model or not env_key:
                ...
                sys.exit(1)
            from map_mmd.llm import provider_base_url_ok
            ...
            print(f"Provider '{name}' added. Use with: map_mmd extract . --backend {name}")

        elif subcmd == "remove":
            name = sys.argv[3] if len(sys.argv) > 3 else ""
            if not name:
                print("Usage: map_mmd provider remove <name>", file=sys.stderr)
                sys.exit(1)
            existing = {}
            if global_path.is_file():
                try:
            ...
            print("Usage: map_mmd provider [add|list|show|remove]", file=sys.stderr)
            if subcmd:
                sys.exit(1)
    elif cmd == "prs":
        from map_mmd.prs import cmd_prs
        cmd_prs(sys.argv[2:])
    elif cmd == "hook":
        from map_mmd.hooks import (
        ...
        if subcmd == "install":
            ...
            print("Usage: map_mmd hook [install|uninstall|status]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "query":
        if len(sys.argv) < 3:
            print("Usage: map_mmd query \"<question>\" [--dfs] [--context C] [--budget N] [--graph path]", file=sys.stderr)
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print("Usage: map_mmd affected \"<node-or-label>\" [--relation R] [--depth N] [--graph path]", file=sys.stderr)
            sys.exit(1)
        from map_mmd.affected import DEFAULT_AFFECTED_RELATIONS, format_affected, load_graph
        query = sys.argv[2]
        graph_path = _default_graph_path()
        depth = 2
        relations: list[str] = []
        ...
        # map_mmd save-result --question Q --answer A [--type T] [--nodes N1 N2 ...]
        #                      [--outcome useful|dead_end|corrected] [--correction TEXT]
        import argparse as _ap

        p = _ap.ArgumentParser(prog="map_mmd save-result")
        ...
        p.add_argument("--memory-dir", default=str(Path(_MAP_MMD_OUT) / "memory"))
        opts = p.parse_args(sys.argv[2:])
        from map_mmd.ingest import save_query_result as _sqr
        ...
        p = _ap.ArgumentParser(prog="map_mmd reflect")
        p.add_argument("--memory-dir", default=str(Path(_MAP_MMD_OUT) / "memory"))
        p.add_argument(
            "--out",
            default=str(Path(_MAP_MMD_OUT) / "reflections" / "LESSONS.md"),
        )
        ...
        opts = p.parse_args(sys.argv[2:])
        from map_mmd.reflect import reflect as _reflect, lessons_fresh as _lessons_fresh

        graph_arg = opts.graph
        if graph_arg is None:
            default_graph = Path(_MAP_MMD_OUT) / "graph.json"
        ...
        if len(sys.argv) < 4:
            print(
                'Usage: map_mmd path "<source>" "<target>" [--graph path]',
                file=sys.stderr,
            )
            sys.exit(1)
        from map_mmd.serve import _score_nodes
        from networkx.readwrite import json_graph
        import networkx as _nx
        ...
        if len(sys.argv) < 3:
            print('Usage: map_mmd explain "<node>" [--graph path]', file=sys.stderr)
            sys.exit(1)
        from map_mmd.serve import _find_node
        from networkx.readwrite import json_graph
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: map_mmd add <url> [--author Name] [--contributor Name] [--dir ./raw]",
                file=sys.stderr,
            )
            sys.exit(1)
        from map_mmd.ingest import ingest as _ingest
        ...
        try:
            ...
            print("Run /map_mmd --update in your AI assistant to update the graph.")
        except Exception as exc:
            print(f"error: {exc}", file=sys.stderr)
            sys.exit(1)
        ...
        if not watch_path.exists():
            ...
            sys.exit(1)
        from map_mmd.watch import watch as _watch
        ...
        # the configured backend, even when a .map_mmd_labels.json already exists.
        ...
        if watch_path is None:
            watch_path = Path(".")
        graph_json = graph_override if graph_override is not None else watch_path / _MAP_MMD_OUT / "graph.json"
        if not graph_json.exists():
            print(
                f"error: no graph found at {graph_json} — run /map_mmd first",
                file=sys.stderr,
            )
            sys.exit(1)
        from networkx.readwrite import json_graph as _jg
        from map_mmd.build import build_from_json
        from map_mmd.cluster import cluster, score_all, remap_communities_to_previous
        from map_mmd.analyze import (
            god_nodes,
        ...
        from map_mmd.security import check_graph_file_size_cap as _check_cap
        _over_cap = False
        try:
        ...
        # node-overlap so the existing .map_mmd_labels.json keeps attaching
        ...
        labels_path = out / ".map_mmd_labels.json"
        if labels_path.exists() and not force_relabel:
            ...
            # No labels file yet (or `map_mmd label` forced a refresh). When run
            ...
            from map_mmd.llm import generate_community_labels
            print("Labeling communities...")
            # The final labels (LLM or placeholder fallback) are persisted to
            # .map_mmd_labels.json by the unconditional write below.
        ...
        from map_mmd.export import _git_head as _gh
        _commit = _gh()
        ...
        force = os.environ.get("MAP_MMD_FORCE", "").lower() in ("1", "true", "yes")
        no_cluster = False
        args = sys.argv[2:]
        ...
        if watch_arg is not None:
            ...
            saved = Path(_MAP_MMD_OUT) / ".map_mmd_root"
            if saved.exists():
        ...
        # user sees their explicit `map_mmd update` complete instead of
        ...
        if ok:
            print("Code graph updated. For doc/paper/image changes run /map_mmd --update in your AI assistant.")
            if not (
                os.environ.get("GEMINI_API_KEY")
                or os.environ.get("GOOGLE_API_KEY")
                or os.environ.get("MOONSHOT_API_KEY")
                or os.environ.get("DEEPSEEK_API_KEY")
                or os.environ.get("MAP_MMD_NO_TIPS")
            ):
        ...
        if len(sys.argv) < 3:
            print("Usage: map_mmd check-update <path>", file=sys.stderr)
            sys.exit(1)
        ...
        from map_mmd.tree_html import write_tree_html, DEFAULT_MAX_CHILDREN
        graph_path = Path(_MAP_MMD_OUT) / "graph.json"
        ...
        while i_arg < len(args):
            ...
            if a == "--graph" and i_arg + 1 < len(args):
                ...
                print("Usage: map_mmd tree [--graph PATH] [--output HTML]")
                print("  --graph PATH         path to graph.json (default map.mmd-out/graph.json)")
                print("  --output HTML        output path (default map.mmd-out/GRAPH_TREE.html)")
        ...
        # Usage: map_mmd merge-driver %O %A %B  (set in .git/config merge driver)
        if len(sys.argv) < 5:
            print("Usage: map_mmd merge-driver <base> <current> <other>", file=sys.stderr)
            sys.exit(1)
        ...
        try:
            ...
            print(f"[map_mmd merge-driver] error loading graphs: {exc}", file=sys.stderr)
        ...
        if merged.number_of_nodes() > _MERGE_MAX_NODES:
            print(
                f"[map_mmd merge-driver] merged graph has {merged.number_of_nodes()} nodes, "
        ...
        # map_mmd merge-graphs graph1.json graph2.json ... --out merged.json
        args = sys.argv[2:]
        graph_paths: list[Path] = []
        out_path = Path(_MAP_MMD_OUT) / "merged-graph.json"
        i = 0
        while i < len(args):
        ...
        if len(graph_paths) < 2:
            print(
                "Usage: map_mmd merge-graphs <graph1.json> <graph2.json> [...] [--out merged.json]",
                file=sys.stderr,
            )
            sys.exit(1)
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        from map_mmd.build import prefix_graph_for_global as _prefix
        graphs = []
        for gp in graph_paths:
            ...
            # Normalize edges/links key before loading — map_mmd writes "links"
        ...
        # (the map_mmd default) by converting MultiGraphs with nx.Graph().
        ...
        for G, gp in zip(graphs, graph_paths):
            repo_tag = gp.parent.parent.name  # map.mmd-out/../ → repo dir name
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: map_mmd clone <github-url> [--branch <branch>] [--out <dir>]",
                file=sys.stderr,
            )
            sys.exit(1)
        url = sys.argv[2]
        ...
        if subcmd not in ("html", "callflow-html", "obsidian", "wiki", "svg", "graphml", "neo4j", "falkordb"):
            print("Usage: map_mmd export <format>", file=sys.stderr)
        ...
        labels_path = Path(_MAP_MMD_OUT) / ".map_mmd_labels.json"
        labels_path_explicit = False
        report_path = Path(_MAP_MMD_OUT) / "GRAPH_REPORT.md"
        ...
        analysis_path = Path(_MAP_MMD_OUT) / ".map_mmd_analysis.json"
        node_limit = 5000
        no_viz = False
        obsidian_dir = Path(_MAP_MMD_OUT) / "obsidian"
        ...
        while i < len(args):
            ...
            if a == "--graph" and i + 1 < len(args):
                ...
                print("Usage: map_mmd export callflow-html [GRAPH|DIR] [--graph PATH] [--labels PATH]")
                print("  --report PATH          path to GRAPH_REPORT.md")
                print("  --sections PATH        JSON section definitions")
                print("  --output HTML          output path (default map.mmd-out/<project>-callflow.html)")
                ...
                if candidate.name == "graph.json" or candidate.suffix.lower() == ".json":
                    ...
                    graph_path = candidate / _MAP_MMD_OUT / "graph.json"
                graph_path_explicit = True
                i += 1
            else:
                i += 1
        ...
        if graph_path_explicit:
            ...
            if not labels_path_explicit:
                labels_path = graph_out_dir / ".map_mmd_labels.json"
        ...
        if not graph_path.exists():
            print(f"error: graph not found: {graph_path}. Run /map_mmd <path> first.", file=sys.stderr)
            sys.exit(1)

        if subcmd == "callflow-html":
            from map_mmd.callflow_html import write_callflow_html as _write_callflow_html
        ...
        from networkx.readwrite import json_graph as _jg
        from map_mmd.build import build_from_json as _bfj
        from map_mmd.security import check_graph_file_size_cap as _check_cap
        ...
        # that happens, `map_mmd export html` previously bailed with
        ...
        if subcmd == "html":
            from map_mmd.export import to_html as _to_html
            if no_viz:
            ...
            from map_mmd.export import to_obsidian as _to_obsidian, to_canvas as _to_canvas
            ...
            from map_mmd.wiki import to_wiki as _to_wiki
            from map_mmd.analyze import god_nodes as _god_nodes
            if not communities:
                print(
                    "error: .map_mmd_analysis.json is missing or empty — refusing to export wiki to prevent data loss.\n"
                    "Run `map_mmd extract .` (or `map_mmd cluster-only .`) to regenerate community data first.",
                    file=sys.stderr,
                )
                sys.exit(1)
            if not gods_data:
            ...
            if push_uri:
                ...
                from map_mmd.export import to_cypher as _to_cypher
            ...
            if push_uri:
                from map_mmd.export import push_to_falkordb as _push
                result = _push(G, uri=push_uri, user=push_user,
                               password=push_password, communities=communities)
                ...
                print(f"cypher.txt written ({out_dir}/cypher.txt) - statements are OpenCypher. "
                      ...
                      f"import), so load a graph with: map_mmd export falkordb --push "
                      f"falkordb://localhost:6379")

    elif cmd == "benchmark":
        from map_mmd.benchmark import run_benchmark, print_benchmark

        graph_path = sys.argv[2] if len(sys.argv) > 2 else _default_graph_path()
        _enforce_graph_size_cap_or_exit(Path(graph_path))
        # Try to load corpus_words from detect output
        corpus_words = None
        detect_path = Path(".map_mmd_detect.json")
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        from map_mmd.global_graph import (
        ...
        if subcmd == "add":
            # map_mmd global add <graph.json> [--as <tag>]
            args = sys.argv[3:]
            source = None
            tag = None
            i = 0
            while i < len(args):
                if args[i] == "--as" and i + 1 < len(args):
                    ...
                    i += 1
            if not source:
                print("Usage: map_mmd global add <graph.json> [--as <repo-tag>]", file=sys.stderr)
                sys.exit(1)
            ...
            if not tag:
                print("Usage: map_mmd global remove <repo-tag>", file=sys.stderr); sys.exit(1)
            try:
            ...
            repos = _global_list()
            if not repos:
                print("Global graph is empty. Use 'map_mmd global add' to add a project.")
            else:
            ...
            print("Usage: map_mmd global [add|remove|list|path]", file=sys.stderr); sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: map_mmd extract <path> [--backend gemini|kimi|claude|openai|deepseek|ollama] "
        ...
        if deep_mode:
            print("[map_mmd extract] deep mode enabled: richer semantic extraction")

        # CLI flag wins over env var. Setting MAP_MMD_API_TIMEOUT here so
        # _call_openai_compat picks it up without needing a new kwarg path.
        if cli_api_timeout is not None:
            os.environ["MAP_MMD_API_TIMEOUT"] = str(cli_api_timeout)
        if cli_max_workers is not None:
            os.environ["MAP_MMD_MAX_WORKERS"] = str(cli_max_workers)

        # Resolve output dir. The user-facing contract is "<out>/map.mmd-out/"
        # so a fresh checkout writes map.mmd-out/ at the project root, matching
        # the skill.md pipeline.
        out_root = (out_dir.resolve() if out_dir else target)
        map_mmd_out = out_root / _MAP_MMD_OUT
        map_mmd_out.mkdir(parents=True, exist_ok=True)
        ...
        manifest_path = map_mmd_out / "manifest.json"
        existing_graph_path = map_mmd_out / "graph.json"
        ...
        if not has_path:
            ...
            print(f"[map_mmd extract] incremental scan of {target}")
            ...
            print(f"[map_mmd extract] scanning {target}")
        ...
        if needs_llm:
            if backend is None:
                ...
                sys.exit(1)
            if backend == "ollama":
                from map_mmd.llm import _validate_ollama_base_url
        ...
        if code_files:
            from map_mmd.extract import extract as _ast_extract
            # Anchor the cache at the output root, not the scanned project:
            # with --out, a <target>/map.mmd-out/cache/ would leak a
            # map.mmd-out/ dir into a project that asked for external output.
            ast_kwargs: dict = {"cache_root": out_root}
            if cli_max_workers is not None:
                ast_kwargs["max_workers"] = cli_max_workers
            print(f"[map_mmd extract] AST extraction on {len(code_files)} code files...")
            try:
                ast_result = _ast_extract(code_files, **ast_kwargs)
            except Exception as exc:
                print(f"[map_mmd extract] AST extraction failed: {exc}", file=sys.stderr)
                ast_result = {"nodes": [], "edges": [], "input_tokens": 0, "output_tokens": 0}

        # Semantic extraction on docs/papers/images. Check cache first.
        from map_mmd.cache import (
        ...
        if semantic_files:
            ...
            if sem_cache_hits:
                print(f"[map_mmd extract] semantic cache: {sem_cache_hits} hit / {sem_cache_misses} miss")

            if uncached_paths:
                print(f"[map_mmd extract] semantic extraction on {len(uncached_paths)} files via {backend}...")
                corpus_kwargs: dict = {
                    "backend": backend,
                    "model": model,
                    "root": target,
                ...
                def _progress(idx: int, total: int, _result: dict) -> None:
                    ...
                    print(
                        f"[map_mmd extract] chunk {idx + 1}/{total} done",
                        flush=True,
                    )
                ...
                try:
                    ...
                    print(
                        f"[map_mmd extract] semantic extraction failed: {exc}",
                        file=sys.stderr,
                    )
                ...
                if uncached_paths and _chunk_stats["succeeded"] == 0:
                    print(
                        f"[map_mmd extract] error: all semantic chunks failed "
                ...
                try:
                    ...
                    print(f"[map_mmd extract] warning: could not write semantic cache: {exc}", file=sys.stderr)
        ...
        graph_json_path = map_mmd_out / "graph.json"
        analysis_path = map_mmd_out / ".map_mmd_analysis.json"
        ...
        if no_cluster:
            ...
            if (
                ...
                print(
                    "[map_mmd extract] no incremental changes detected "
                    "(--no-cluster); outputs left untouched."
                )
                try:
                    _save_manifest(_manifest_files, manifest_path=str(manifest_path), kind="both", root=target)
                except Exception as exc:
                    print(f"[map_mmd extract] warning: could not write manifest: {exc}", file=sys.stderr)
                sys.exit(0)
            ...
            try:
                ...
                print(f"[map_mmd extract] warning: could not write manifest: {exc}", file=sys.stderr)
            if global_merge:
                from map_mmd.global_graph import global_add as _global_add
                _tag = global_repo_tag or target.name
                try:
                    result = _global_add(map_mmd_out / "graph.json", _tag)
                    if result["skipped"]:
                        print(f"[map_mmd global] '{_tag}' unchanged since last add - skipped.")
                    else:
                        print(f"[map_mmd global] '{_tag}' merged into global graph "
                              f"(+{result['nodes_added']} nodes, -{result['nodes_removed']} pruned).")
                except Exception as exc:
                    print(f"[map_mmd global] warning: failed to merge into global graph: {exc}", file=sys.stderr)
            sys.exit(0)

        # Build graph + cluster + score + write.
        from map_mmd.build import (
            build as _build,
            build_from_json as _build_from_json,
            build_merge as _build_merge,
        )
        from map_mmd.cluster import cluster as _cluster, score_all as _score_all
        from map_mmd.export import to_json as _to_json
        from map_mmd.analyze import god_nodes as _god_nodes, surprising_connections as _surprising
        ...
        if G.number_of_nodes() == 0:
            print(
                "[map_mmd extract] graph is empty — extraction produced no nodes. "
        ...
        if merged.get("output_tokens", 0) > 0:
            (map_mmd_out / ".map_mmd_semantic_marker").write_text(
        ...
        print(f"[map_mmd extract] wrote {analysis_path}")
        if incremental_mode:
            print(
                f"[map_mmd extract] incremental summary: "
                f"{sem_cache_hits + unchanged_total} files cached/unchanged, "
                f"{len(code_files) + sem_cache_misses} re-extracted, "
                f"{len(deleted_files)} deleted"
            )
        elif sem_cache_hits:
            print(f"[map_mmd extract] semantic cache: {sem_cache_hits} cached, {sem_cache_misses} re-extracted")
        if merged["input_tokens"] or merged["output_tokens"]:
            print(
                f"[map_mmd extract] tokens: "
                f"{merged['input_tokens']:,} in / "
        ...
        print(
            "[map_mmd extract] next: run "
            f"`map_mmd cluster-only {map_mmd_out.parent}` "
            "to generate GRAPH_REPORT.md and name communities"
        )

    elif cmd == "cache-check":
        # map_mmd cache-check <files_from> [--root <dir>]
        # Reads file paths (one per line) from <files_from>, checks semantic cache.
        # Writes:
        #   map.mmd-out/.map_mmd_cached.json   — already-cached nodes/edges/hyperedges
        #   map.mmd-out/.map_mmd_uncached.txt  — paths that need extraction
        # Stdout: "Cache: N hit, M miss"
        from map_mmd.cache import check_semantic_cache
        if len(sys.argv) < 3:
            print("Usage: map_mmd cache-check <files_from> [--root <dir>]", file=sys.stderr)
            sys.exit(1)
        ...
        out = root / _MAP_MMD_OUT
        out.mkdir(parents=True, exist_ok=True)
        if cached_nodes or cached_edges or cached_hyperedges:
            (out / ".map_mmd_cached.json").write_text(
                json.dumps({"nodes": cached_nodes, "edges": cached_edges, "hyperedges": cached_hyperedges},
                           ensure_ascii=False),
                encoding="utf-8",
            )
        (out / ".map_mmd_uncached.txt").write_text("\n".join(uncached), encoding="utf-8")
        print(f"Cache: {len(files) - len(uncached)} hit, {len(uncached)} miss")

    elif cmd == "merge-chunks":
        # map_mmd merge-chunks <chunk_glob_or_files...> --out <path>
        # Concatenates .map_mmd_chunk_*.json files written by semantic subagents.
        # Deduplicates nodes by id (first writer wins). Sums token counts.
        import glob as _glob
        if len(sys.argv) < 3:
            print("Usage: map_mmd merge-chunks <chunk_files...> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        for cf in chunk_files:
            try:
                ...
                print(f"[map_mmd merge-chunks] warning: skipping {cf}: {exc}", file=sys.stderr)
                continue
        ...
        # map_mmd merge-semantic --cached <path> --new <path> --out <path>
        ...
        if len(sys.argv) < 3:
            print("Usage: map_mmd merge-semantic --cached <path> --new <path> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        # User ran `map_mmd <path>` directly — treat as `map_mmd extract <path>`.
        # Common when following the PowerShell note in README (`map_mmd .`) or
        ...
        print("Run 'map_mmd --help' for usage.", file=sys.stderr)
        sys.exit(1)
