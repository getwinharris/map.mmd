...
# ``_max_graph_file_bytes`` (which lets ``MAP_MMD_MAX_GRAPH_BYTES`` override it).
...
def _max_graph_file_bytes() -> int:
    ...
    Honors the ``MAP_MMD_MAX_GRAPH_BYTES`` environment variable so users with
    ...
    take effect.
    """
    raw = os.environ.get("MAP_MMD_MAX_GRAPH_BYTES", "").strip()
    if not raw:
...
def safe_fetch(url: str, max_bytes: int = _MAX_FETCH_BYTES, timeout: int = 30) -> bytes:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 map_mmd/1.0"})
...
def validate_graph_path(path: str | Path, base: Path | None = None) -> Path:
    ...
    *base* defaults to the `map.mmd-out` directory relative to CWD.
    Also requires the base directory to exist, so a caller cannot
    trick map_mmd into reading files before any graph has been built.
    ...
    if base is None:
        ...
        for candidate in [resolved_hint, *resolved_hint.parents]:
            if candidate.name == MAP_MMD_OUT_NAME:
                base = candidate
                break
        if base is None:
            base = Path(MAP_MMD_OUT).resolve()

    base = base.resolve()
    if not base.exists():
        raise ValueError(
            f"Graph base directory does not exist: {base}. "
            "Run /map_mmd first to build the graph."
        )
    ...
    try:
        ...
        raise ValueError(
            ...
            "Only paths inside map.mmd-out/ are permitted."
        )
...
def check_graph_file_size_cap(path: Path) -> None:
    ...
    ``MAP_MMD_MAX_GRAPH_BYTES`` env var can be set before import and still
    apply.
    ...
    if size > cap:
        raise ValueError(
            ...
            f"(set MAP_MMD_MAX_GRAPH_BYTES=<bytes> or "
            f"MAP_MMD_MAX_GRAPH_BYTES=<N>GB to raise the limit)"
        )
