...
# --- migrated to map_mmd/extractors/ (see map_mmd/extractors/MIGRATION.md) ---
from map_mmd.extractors.base import (  # noqa: F401
    _LANGUAGE_BUILTIN_GLOBALS,
    _file_stem,
    _make_id,
    _read_text,
)
from map_mmd.extractors.blade import extract_blade  # noqa: F401
from map_mmd.extractors.elixir import extract_elixir  # noqa: F401
from map_mmd.extractors.razor import extract_razor  # noqa: F401
from map_mmd.extractors.zig import extract_zig  # noqa: F401
...
def _safe_extract(extractor: Callable, path: Path) -> dict:
    try:
        ...
        if os.environ.get("MAP_MMD_DEBUG"):
            import traceback
...
# Extensions map_mmd creates document file nodes for. A link to one of these
...
def _pascal_resolve_unit(from_path: Path, unit_name: str) -> str:
    """Resolve a Pascal unit name to the map_mmd node ID of its source file.
...
def _check_tree_sitter_version() -> None:
    ...
    if LANGUAGE_VERSION < 14:
        ...
        raise RuntimeError(
            ...
            f"map_mmd requires tree-sitter >= 0.23.0 (Language API v2). "
...
def extract_bash(path: Path) -> dict:
    ...
    from map_mmd.security import sanitize_metadata  # module-level cached import
...
def _extract_parallel(
    ...
    if max_workers is None:
        # Honour MAP_MMD_MAX_WORKERS env override; otherwise scale to the
        ...
        env_raw = os.environ.get("MAP_MMD_MAX_WORKERS", "").strip()
        env_cap = None
        if env_raw:
            try:
                v = int(env_raw)
    ...
    # MAP_MMD_MAX_WORKERS, and --max-workers — stays valid on >61-core boxes
...
def extract(
    ...
    *,
    ...
    Args:
        ...
        cache_root: explicit root for map.mmd-out/cache/ (overrides the
            inferred common path prefix). Pass Path('.') when running on a
            subdirectory so the cache stays at ./map.mmd-out/cache/.
        ...
        max_workers: max subprocess count. Defaults to cpu_count (or the
            value of MAP_MMD_MAX_WORKERS if set), bounded by len(uncached_work).
    """
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    from map_mmd.detect import _is_ignored, _is_noise_dir, _load_map_mmdignore
    ignore_root = root if root is not None else target
    patterns = _load_map_mmdignore(ignore_root)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m map_mmd.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
