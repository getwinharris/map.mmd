...
from map_mmd.paths import out_path as _out_path
...
def _model_name() -> str:
    return os.environ.get("MAP_MMD_WHISPER_MODEL", _DEFAULT_MODEL)
...
def download_audio(url: str, output_dir: Path) -> Path:
    ...
    """
    from map_mmd.security import validate_url
...
def build_whisper_prompt(god_nodes: list[dict]) -> str:
    ...
    domain hint from these labels and passes it via MAP_MMD_WHISPER_PROMPT or
    as initial_prompt — no separate API call needed here.
    """
    if not god_nodes:
        return _FALLBACK_PROMPT

    override = os.environ.get("MAP_MMD_WHISPER_PROMPT")
    if override:
        return override
