...
def google_workspace_enabled(value: str | None = None) -> bool:
    ...
    raw = value if value is not None else os.environ.get("MAP_MMD_GOOGLE_WORKSPACE", "")
...
def _run_gws_export(file_id: str, mime_type: str, output: Path, resource_key: str | None = None) -> None:
    ...
    timeout = int(os.environ.get("MAP_MMD_GOOGLE_WORKSPACE_TIMEOUT", "120"))
    ...
    *,
    ...
    if ext == ".gsheet":
        if xlsx_to_markdown is None:
            raise RuntimeError("Google Sheets export requires the office extra: pip install map-mmd[office,google]")
