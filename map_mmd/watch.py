...
# Single source of truth in map_mmd.paths (#1423); re-exported as _MAP_MMD_OUT.
from map_mmd.paths import MAP_MMD_OUT as _MAP_MMD_OUT
...
def _apply_resource_limits() -> None:
    ...
    MAP_MMD_REBUILD_MEMORY_LIMIT_MB caps RSS-ish memory. Uses RLIMIT_DATA on
    ...
    try:
        ...
        pass
    mb = os.environ.get("MAP_MMD_REBUILD_MEMORY_LIMIT_MB", "").strip()
    if not mb:
        return
    try:
...
def _node_community_map(graph_data: dict) -> dict[str, int]:
    ...
    for node in graph_data.get("nodes", []):
        ...
        try:
            ...
            print(
                f"[map_mmd watch] Skipping node with invalid community id: "
...
def _check_shrink(
    ...
    *,
    ...
    plain ``map_mmd update`` after deleting a function refresh the graph without
    ...
    if rebuilt_sources is not None:
        from map_mmd.build import _norm_source_file
    ...
    print(
        f"[map_mmd] WARNING: new graph has {len(new_nodes)} nodes but existing "
...
def _rebuild_code(
    ...
    *,
    ...
    JSON to map.mmd-out/graph.json (mirrors ``extract --no-cluster``).

    Returns True on success, False on error or skipped-due-to-lock.
    """
    out = watch_path / _MAP_MMD_OUT
    if acquire_lock:
        ...
        with _rebuild_lock(out, blocking=block_on_lock) as got:
            if not got:
                print("[map_mmd watch] Rebuild already in progress for "
    ...
    try:
        from map_mmd.extract import extract, _get_extractor
        from map_mmd.detect import detect
        from map_mmd.build import build_from_json, _norm_source_file as _nsf
        from map_mmd.cluster import cluster, remap_communities_to_previous, score_all
        from map_mmd.analyze import god_nodes, surprising_connections, suggest_questions
        from map_mmd.report import generate
        from map_mmd.export import to_json, to_html
        from map_mmd.security import check_graph_file_size_cap
        ...
        if not code_files:
            print("[map_mmd watch] No code files found - nothing to rebuild.")
            return False
        ...
        if changed_paths is not None:
            ...
            if not wanted and not deleted_paths:
                print("[map_mmd watch] No tracked code files in change set - skipping rebuild.")
                return True
        ...
        # so a committed ``map.mmd-out/.map_mmd_root`` is portable across
        # clones and CI runners (#777). When ``watch_path`` is ``.`` (the
        # common case for ``map_mmd update``), this writes ``.`` and the
        # subsequent re-run resolves it against the caller's CWD.
        (out / ".map_mmd_root").write_text(str(watch_path), encoding="utf-8")
        ...
        if no_cluster:
            ...
            from map_mmd.build import dedupe_edges as _dedupe_edges, dedupe_nodes as _dedupe_nodes
            ...
            if same_graph:
                print("[map_mmd watch] No code-graph changes detected (--no-cluster); outputs left untouched.")
            else:
                print(
                    "[map_mmd watch] Rebuilt (no clustering): "
                    f"{len(candidate_graph_data.get('nodes', []))} nodes, "
                    f"{len(candidate_graph_data.get('links', []))} edges"
                )
                print(f"[map_mmd watch] graph.json updated in {out}")
            return True
        ...
        if existing_graph_data:
            ...
            if same_topology:
                ...
                if flag.exists():
                    flag.unlink()
                print("[map_mmd watch] No code-graph topology changes detected; outputs left untouched.")
                return True
        ...
        labels_file = out / ".map_mmd_labels.json"
        try:
        ...
        if no_change:
            ...
            print("[map_mmd watch] No code-graph changes detected; graph.json/GRAPH_REPORT.md left untouched.")
        else:
            if not _check_shrink(
                force, existing_graph_data, candidate_graph_data,
                tmp=graph_tmp,
                ...
                return False
            from map_mmd.export import backup_if_protected as _backup
            _backup(out)
        ...
        if not no_change:
            try:
                ...
                print(f"[map_mmd watch] Skipped graph.html: {viz_err}")
        ...
        if callflow_files and not no_change:
            try:
                from map_mmd.callflow_html import write_callflow_html
                for cf in callflow_files:
                    write_callflow_html(
                        graph=out / "graph.json",
                        report=out / "GRAPH_REPORT.md",
                        labels=out / ".map_mmd_labels.json",
                        output=cf,
                        verbose=False,
                    )
            except Exception as cf_err:
                print(f"[map_mmd watch] callflow HTML update skipped: {cf_err}")
        ...
        if not no_change:
            print(f"[map_mmd watch] Rebuilt: {G.number_of_nodes()} nodes, "
            ...
            print(f"[map_mmd watch] {products} updated in {out}")
        return True

    except Exception as exc:
        print(f"[map_mmd watch] Rebuild failed: {exc}")
        return False
...
def check_update(watch_path: Path) -> bool:
    ...
    re-extraction via `/map_mmd --update` — this function only signals
    that the update is needed.
    """
    flag = Path(watch_path) / _MAP_MMD_OUT / "needs_update"
    if flag.exists():
        print(f"[map_mmd check-update] Pending non-code changes in {watch_path}.")
        print("[map_mmd check-update] Run `/map_mmd --update` to apply semantic re-extraction.")
    return True


def _notify_only(watch_path: Path) -> None:
    """Write a flag file and print a notification (fallback for non-code-only corpora)."""
    flag = watch_path / _MAP_MMD_OUT / "needs_update"
    flag.parent.mkdir(parents=True, exist_ok=True)
    flag.write_text("1", encoding="utf-8")
    print(f"\n[map_mmd watch] New or changed files detected in {watch_path}")
    print("[map_mmd watch] Non-code files changed - semantic re-extraction requires LLM.")
    print("[map_mmd watch] Run `/map_mmd --update` in Claude Code to update the graph.")
    print(f"[map_mmd watch] Flag written to {flag}")
...
def watch(watch_path: Path, debounce: float = 3.0) -> None:
    ...
    to run /map_mmd --update (LLM extraction required).
    ...
    # Load .map_mmdignore patterns ONCE at startup so the handler does not
    ...
    ignore_patterns = _load_map_mmdignore(watch_root_for_ignore)
    ...
    class Handler(FileSystemEventHandler):
        def on_any_event(self, event):
            ...
            # Check .map_mmdignore BEFORE the extension/dotfile/out filters so
            ...
            if any(part.startswith(".") for part in filter_parts):
                return
            if _MAP_MMD_OUT in filter_parts:
                return
    ...
    print(f"[map_mmd watch] Watching {watch_path.resolve()} - press Ctrl+C to stop")
    print(f"[map_mmd watch] Code changes rebuild graph automatically. "
          f"Doc/image changes require /map_mmd --update.")
    print(f"[map_mmd watch] Debounce: {debounce}s")
    ...
    try:
        while True:
            ...
            if pending and (time.monotonic() - last_trigger) >= debounce:
                ...
                batch = list(changed)
                changed.clear()
                print(f"\n[map_mmd watch] {len(batch)} file(s) changed")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Watch a folder and auto-update the map_mmd graph")
