...
from map_mmd.google_workspace import (
    GOOGLE_WORKSPACE_EXTENSIONS,
    convert_google_workspace_file,
    google_workspace_enabled,
)
from map_mmd.paths import MAP_MMD_OUT, MAP_MMD_OUT_NAME, out_path
...
# attacker-controllable (map_mmd runs on cloned/shared folders), and .docx/.xlsx
...
def classify_file(path: Path) -> FileType | None:
    ...
    from map_mmd.manifest_ingest import is_package_manifest_path
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the map_mmd extract pipeline.
...
_SKIP_DIRS = {
    ...
    ".tox", ".eggs", "*.egg-info",
    "map.mmd-out", MAP_MMD_OUT_NAME,  # never treat own output as source input (#524); honour MAP_MMD_OUT (#1423)
    ...
    ".map_mmd",  # map_mmd's own extraction cache — never index self-generated data
...
def _parse_gitignore_line(raw: str) -> str:
    """Parse one raw line from a .map_mmdignore file per gitignore spec.
...
def _load_map_mmdignore(root: Path) -> list[tuple[Path, str]]:
    """Read .map_mmdignore files and return (anchor_dir, pattern) pairs.
    ...
    for d in dirs:
        # Merge .gitignore and .map_mmdignore for this dir (#1363). Previously
        # the presence of a .map_mmdignore made map_mmd skip that dir's
        ...
        # committed. .gitignore is read first and .map_mmdignore last, so
        # .map_mmdignore patterns (including `!` negations) win on conflict via
        # last-match-wins; adding a .map_mmdignore can only ever exclude MORE,
        # never re-include a .gitignore-excluded file (#945 kept: a project with
        # only a .gitignore still gets sensible defaults).
        for fname in (".gitignore", ".map_mmdignore"):
            ignore_file = d / fname
...
def _is_ignored(
    ...
    *,
    ...
    """Return True if the path should be ignored per .map_mmdignore patterns.
...
def _load_map_mmdinclude(root: Path) -> list[tuple[Path, str]]:
    """Read .map_mmdinclude allowlist patterns from root and ancestors.
    ...
    Uses the same VCS-root ceiling logic as _load_map_mmdignore.
    """
    root = root.resolve()
    ceiling = _find_vcs_root(root) or root
...
def _is_included(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if path matches any .map_mmdinclude allowlist pattern."""
    if not patterns:
        return False
...
def _could_contain_included_path(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if a directory may contain files matched by .map_mmdinclude."""
    if not patterns:
        return False
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None) -> dict:
    ...
    ignore_patterns = _load_map_mmdignore(root)
    ...
    # so they win over any .map_mmdignore/.gitignore rules (#947).
    if extra_excludes:
        for pat in extra_excludes:
            line = _parse_gitignore_line(pat)
            if line:
                ignore_patterns.append((root, line))
    include_patterns = _load_map_mmdinclude(root)

    # Always include map.mmd-out/memory/ - query results filed back into the graph
    memory_dir = root / MAP_MMD_OUT / "memory"
    scan_paths = [root]
    ...
    converted_dir = root / MAP_MMD_OUT / "converted"
        ...
        if ftype:
            if p.suffix.lower() in GOOGLE_WORKSPACE_EXTENSIONS:
                if not google_workspace:
                    skipped_sensitive.append(
                        ...
                        "or set MAP_MMD_GOOGLE_WORKSPACE=1]"
                    )
                    continue
                try:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install map-mmd[office]]")
                continue
    ...
    return {
        ...
        "map_mmdignore_patterns": len(ignore_patterns),
...
def _to_relative_for_storage(key: str, root: Path) -> str:
    ...
    fallback in :func:`map_mmd.watch._relativize_source_files` so the
...
def load_manifest(
    ...
    *,
    ...
    manifests with absolute keys pass through unchanged, so a map.mmd-out/
...
def save_manifest(
    ...
    *,
    ...
    kind="ast"      — written by `map_mmd update` (AST-only rebuild). Stamps
                      ast_hash; preserves an existing semantic_hash only when
                      the file content is unchanged (mtime + hash match).
    kind="semantic" — written by `map_mmd extract` after semantic extraction.
...
def detect_incremental(
    ...
    *,
    ...
    kind="semantic" (default for extract): a file is "changed" when its
        ...
        semantic extraction pass. Use this for `map_mmd extract` so that
        files touched by `map_mmd update` (AST-only) are re-extracted
        semantically.
    kind="ast": a file is "changed" when its ast_hash is missing or its
        content has changed. Use this for `map_mmd update`.
