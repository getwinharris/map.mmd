# map.mmd reference: query, path, explain

Load this when the user asks a question against an existing graph, or runs `/map.mmd path` or `/map.mmd explain`. The core's query stub points here for the full traversal flow. These flows use the `map.mmd query` CLI when it is available and fall back to an inline NetworkX traversal otherwise.
...
if not Path('map.mmd-out/graph.json').exists():
    print('ERROR: No graph found. Run /map.mmd <path> first to build the graph.')
    raise SystemExit(1)
"
```
If it fails, stop and tell the user to run `/map.mmd <path>` first.
...
import json, re
from pathlib import Path
data = json.loads(Path('map.mmd-out/graph.json').read_text())
vocab = set()
for n in data['nodes']:
    for c in re.findall(r'[^\W\d_]+', n.get('label','') or '', re.UNICODE):
        ...
        for p in parts:
            ...
            if 3 <= len(t) <= 30:
                vocab.add(t)
Path('map.mmd-out/.vocab.txt').write_text('\n'.join(sorted(vocab)))
print(f'vocab: {len(vocab)} tokens')
"
```

2. Read `map.mmd-out/.vocab.txt`. Then for the user's question, select **up to 12 tokens from this exact list** that semantically match the query intent. Hard constraints:
...
# or: map.mmd query "QUESTION" --dfs --budget 3000
```

If the CLI is unavailable, load `map.mmd-out/graph.json` and run the traversal inline:
...
```bash
$(cat map.mmd-out/.map.mmd_python) -m map.mmd save-result --question "ORIGINAL_QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
$(cat map.mmd-out/.map.mmd_python) -m map.mmd save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```bash
$(cat map.mmd-out/.map.mmd_python) -m map.mmd save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
