...
def build_from_json(extraction: dict, *, directed: bool = False, root: str | Path | None = None) -> nx.Graph:
    ...
    for node in extraction.get("nodes", []):
        ...
        if "source" in node and "source_file" not in node:
            ...
            print(
                f"[map_mmd] WARNING: node '{node_id}' uses field 'source' instead of "
        ...
        # map_mmd versions that didn't always populate file_type) don't
    ...
    if real_errors:
        print(f"[map_mmd] Extraction warning ({len(real_errors)} issues): {real_errors[0]}", file=sys.stderr)
    ...
    for node in extraction.get("nodes", []):
        ...
        if isinstance(node, dict):
            ...
            try:
                ...
                print(
                    f"[map_mmd] WARNING: skipping node with non-hashable id "
    ...
    for edge in sorted(
        ...
        try:
            ...
            print(
                f"[map_mmd] WARNING: skipping edge with non-hashable endpoint "
...
def deduplicate_by_label(nodes: list[dict], edges: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    print(f"[map_mmd] Deduplicated {len(remap)} duplicate node(s) by label.", file=sys.stderr)
...
def build_merge(
    ...
    *,
    ...
    if prune_sources:
        ...
        if n_nodes:
            print(
                f"[map_mmd] Pruned {n_nodes} node(s) from {n_files} deleted source file(s).",
                file=sys.stderr,
            )
        ...
        if edges_to_remove:
            ...
            print(
                f"[map_mmd] Pruned {len(edges_to_remove)} edge(s) from deleted source file(s).",
                file=sys.stderr,
            )

        if not n_nodes and not edges_to_remove:
            print(
                f"[map_mmd] {n_files} source file(s) deleted since last run — "
    ...
    if graph_path.exists() and not dedup and not prune_sources:
        ...
        if new_n < existing_n:
            raise ValueError(
                f"map_mmd: build_merge would shrink graph from {existing_n} → {new_n} nodes. "
