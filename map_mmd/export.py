...
from map_mmd.security import sanitize_label
from map_mmd.analyze import _node_community_map
...
_BACKUP_ARTIFACTS = [
    ...
    "GRAPH_REPORT.md",
    ".map_mmd_labels.json",
    ".map_mmd_analysis.json",
    "manifest.json",
    ".map_mmd_semantic_marker",
    "cost.json",
]
...
def backup_if_protected(out_dir: Path) -> "Path | None":
    ...
    - .map_mmd_semantic_marker is present (graph cost real LLM tokens), or
    - .map_mmd_labels.json contains at least one non-default community label
    ...
    Set MAP_MMD_NO_BACKUP=1 to disable.
    """
    if os.environ.get("MAP_MMD_NO_BACKUP"):
        return None
    out = Path(out_dir)
    if not (out / "graph.json").exists():
        return None

    is_semantic = (out / ".map_mmd_semantic_marker").exists()
    is_curated = False
        ...
        for name in _BACKUP_ARTIFACTS:
            ...
            if src.exists():
                try:
                    ...
                    pass
        if copied:
            print(f"[map_mmd] backed up {reason} graph ({copied} files) -> {backup_dir.name}/")
        return backup_dir
    except Exception as exc:
        import sys
        print(f"[map_mmd] warning: backup failed ({exc}) - continuing with overwrite", file=sys.stderr)
        return None
...
def _yaml_str(s: str) -> str:
    ...
    See `map_mmd.ingest._yaml_str` for the full rationale; duplicated here to
...
def _viz_node_limit() -> int:
    """Return the effective viz node limit, honoring MAP_MMD_VIZ_NODE_LIMIT env var.
    ...
    """
    import os
    raw = os.environ.get("MAP_MMD_VIZ_NODE_LIMIT")
    ...
    if not force and existing_path.exists():
        try:
            ...
            if new_n < existing_n:
                import sys as _sys
                print(
                    f"[map_mmd] WARNING: new graph has {new_n} nodes but existing "
                    ...
                    f"Run a full rebuild (/map_mmd .) to be safe, or pass force=True "
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    lines = ["// Neo4j Cypher import - generated by /map_mmd", ""]
...
def to_html(
    ...
    if G.number_of_nodes() > limit:
        ...
        raise ValueError(
            ...
            f"(limit: {limit}). Use --no-viz, raise MAP_MMD_VIZ_NODE_LIMIT, "
...
def to_obsidian(
    ...
    # Map file_type → map_mmd tag
    _FTYPE_TAG = {
        "code": "map_mmd/code",
        "document": "map_mmd/document",
        "paper": "map_mmd/paper",
        "image": "map_mmd/image",
    }
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"map_mmd/{ftype}" if ftype else "map_mmd/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"map_mmd/{dom_conf}"
...
def push_to_falkordb(
    ...
    identical to push_to_neo4j. Differences from the Neo4j path:
      ...
      - a named graph is selected via db.select_graph(graph_name) (default
        "map_mmd"); FalkorDB keys each graph by name in the same instance.
