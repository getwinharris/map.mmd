#!/usr/bin/env python3
"""
callflow_html.py — Generate call-flow architecture HTML from map_mmd knowledge graph outputs.

Reads graph.json plus optional GRAPH_REPORT.md, .map_mmd_labels.json, and sections JSON,
...
Usage:
  python3 -m map_mmd export callflow-html
  python3 -m map_mmd export callflow-html /path/to/project/map.mmd-out/graph.json
  python3 -m map_mmd export callflow-html --graph /path/to/graph.json --output docs/architecture.html
"""
...
from map_mmd.paths import MAP_MMD_OUT, MAP_MMD_OUT_NAME
...
def normalize_node(raw: dict, index: int) -> dict:
    """Normalize a map_mmd node across common graph.json schema variants."""
    node = dict(raw)
...
def normalize_edge(raw: dict, index: int) -> dict | None:
    """Normalize map_mmd edges while preserving original fields."""
    edge = dict(raw)
...
def _node_link_payload(data: dict) -> tuple[list, list] | None:
    """Read current map_mmd graph.json via NetworkX's node-link parser."""
...
def load_labels(path: str | Path | None) -> dict:
    """Load community labels from .map_mmd_labels.json, tolerating wrapper keys."""
...
def infer_project_name(graph_path: str, meta: dict) -> str:
    ...
    if path.parent.name == MAP_MMD_OUT_NAME and len(path.parents) > 1:
        return path.parents[1].name
    return path.parent.name or "Project"


def resolve_map_mmd_paths(args) -> dict:
    """Resolve project root, map_mmd output dir, and optional files."""
    base = Path(args.project).expanduser() if args.project else Path.cwd()
    if args.map_mmd_out:
        map_mmd_out = Path(args.map_mmd_out).expanduser()
    elif args.graph:
        map_mmd_out = Path(args.graph).expanduser().parent
    elif (base / "graph.json").exists():
        map_mmd_out = base
    else:
        map_mmd_out = base / MAP_MMD_OUT

    project_root = map_mmd_out.parent if map_mmd_out.name == MAP_MMD_OUT_NAME else base
    graph = Path(args.graph).expanduser() if args.graph else map_mmd_out / "graph.json"
    report = Path(args.report).expanduser() if args.report else map_mmd_out / "GRAPH_REPORT.md"
    labels = Path(args.labels).expanduser() if args.labels else map_mmd_out / ".map_mmd_labels.json"
    sections = Path(args.sections).expanduser() if args.sections else None
    return {
        "base": project_root,
        "map_mmd_out": map_mmd_out,
        "graph": graph,
        "report": report,
        "labels": labels,
        "sections": sections,
...
def node_importance(node: dict) -> float:
    """Use map_mmd centrality fields when available."""
...
def generate_header(sections: list, meta: dict, lang: str) -> str:
    ...
    if lang.startswith("zh"):
        ...
        subtitle = (
            f"由 map_mmd 知识图谱生成：{meta.get('node_count', '?')} 个节点、"
        ...
        subtitle = (
            f"Generated from map_mmd knowledge graph: {meta.get('node_count', '?')} nodes, "
...
def generate_section_cards(sec: dict, nodes: list, section_edges: list, lang: str) -> str:
    ...
    note = pick_text(
        lang,
        f"本节由 map_mmd 社区聚类生成。关系概况：{relation_text}。图表优先展示高置信、跨节点调用或使用关系，完整节点清单位于表格中。",
        f"This section comes from map_mmd community clustering. Relationship summary: {relation_text}. The diagram prioritizes high-confidence calls or usage relationships; the table keeps the broader node inventory.",
    )
...
class CallflowOptions:
    ...
    def __init__(
        ...
        *,
        map_mmd_out: str | Path | None = None,
        ...
        self.map_mmd_out = str(map_mmd_out) if map_mmd_out is not None else None
...
def write_callflow_html(
    ...
    *,
    ...
    """Generate call-flow architecture HTML from map_mmd output files."""
    ...
    paths = resolve_map_mmd_paths(args)
    if not paths["graph"].exists():
        raise FileNotFoundError(
            f"map_mmd output not found: {paths['graph']}. "
            "Run map_mmd first or pass --graph /path/to/graph.json."
        )
    ...
    if args.output:
        ...
        output_path = paths["map_mmd_out"] / f"{safe_filename(meta['project_name'])}-callflow.html"
...
""")
  ...
  <p>Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} · map_mmd callflow-html</p>
</div>
""")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate call-flow architecture HTML from map_mmd knowledge graph outputs"
    )
    parser.add_argument("project", nargs="?", default=None, help="Project root or map_mmd output directory")
    parser.add_argument("--map.mmd-out", default=None, help="Path to map_mmd output directory")
    ...
    parser.add_argument("--labels", default=None, help="Path to .map_mmd_labels.json")
