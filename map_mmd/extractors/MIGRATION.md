...
`map.mmd/extract.py` is being split into this package, one language per PR
...
3. **Facade re-export is mandatory.** `extract.py` must keep exporting every
   moved name (`from map.mmd.extractors.<mod> import extract_<lang>  # noqa: F401`
...
4. **Never import from `map.mmd.extract` inside this package.** Import
...
- run `grep -c '_name' map.mmd/extract.py` AFTER your candidate move;
...
2. `grep -n 'def extract_<lang>' map.mmd/extract.py`; the span ends at the
...
3. Save the span to a temp file. Create `map.mmd/extractors/<lang>.py` with
   module docstring (`"""<Lang> extractor. Moved verbatim from map.mmd/extract.py."""`),
