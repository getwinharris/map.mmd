...
def generate(
    ...
    if built_at_commit:
        lines += [
            ...
            "- Run `map_mmd update .` after code changes (no API cost).",
        ]
    ...
    if gap_count > 0 or amb_pct > 20:
        ...
        if thin_communities:
            lines.append(f"- **{len(thin_communities)} thin communities (<{min_community_size} nodes) omitted from report** — run `map_mmd query` to explore isolated nodes.")
        if amb_pct > 20:
