"""Deterministic "work memory" reflection over map.mmd-out/memory/.

`map_mmd reflect` reads the Q&A memory docs that `map_mmd save-result` files back
...
and a given ``now``. When a graph (`graph.json` + `.map_mmd_analysis.json`) is available
the lessons are also grouped by community label; without it they degrade to a single
flat section.

The artifact lands at ``map.mmd-out/reflections/LESSONS.md`` rather than inside the wiki
because ``map_mmd export wiki`` deletes every ``wiki/*.md`` on each run — a lessons file
...
from map_mmd.ingest import OUTCOMES
...
def _load_node_community(graph_path: Path, analysis_path: Path,
    ...
    Mirrors how `map_mmd export wiki` reads graph.json + .map_mmd_analysis.json +
    .map_mmd_labels.json. Community membership in the analysis sidecar is keyed by
...
def render_lessons_md(agg: dict[str, Any]) -> str:
    ...
    out: list[str] = [
        "# Lessons",
        "",
        f"_Auto-generated by `map_mmd reflect` from {agg['total']} session "
        f"{'memory' if agg['total'] == 1 else 'memories'} in map.mmd-out/memory/. "
...
def lessons_fresh(out_path: Path, memory_dir: Path,
    ...
    Lets ``map_mmd reflect --if-stale`` skip a redundant run — e.g. when the git
...
def reflect(memory_dir: Path, out_path: Path,
    ...
    if graph_path is not None:
        ...
        analysis_path = Path(analysis_path) if analysis_path else (
            graph_path.parent / ".map_mmd_analysis.json")
        labels_path = Path(labels_path) if labels_path else (
            graph_path.parent / ".map_mmd_labels.json")
