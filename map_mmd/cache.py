...
# Output directory name — override with MAP_MMD_OUT env var for worktrees or
# shared-output setups. Accepts a relative name ("map.mmd-out-feature") or an
# absolute path ("/shared/map.mmd-out"). Single source of truth in map_mmd.paths
# (#1423); re-exported here as _MAP_MMD_OUT for the existing call sites.
...
try:
    ...
    _EXTRACTOR_VERSION = _pkg_version("map-mmd")
except Exception:
...
def _cleanup_stale_ast_entries(ast_base: Path, current_dir: Path) -> None:
    """Remove AST cache entries left behind by other map_mmd versions.
...
# Use `map_mmd extract --force` to bypass when needed.
...
def _relativize_source_files_in(payload: dict, root: Path) -> None:
    ...
    Mirror of :func:`map_mmd.watch._relativize_source_files` so cached
    ...
    :func:`map_mmd.detect._to_relative_for_storage`.
    """
    try:
...
def cache_dir(root: Path = Path("."), kind: str = "ast") -> Path:
    ...
    AST entries live in map.mmd-out/cache/ast/v{version}/ — namespaced by
    map_mmd version because they depend on extractor code, not just file
    contents. Semantic entries live unversioned in map.mmd-out/cache/semantic/
...
def load_cached(path: Path, root: Path = Path("."), kind: str = "ast") -> dict | None:
    ...
    Cache value: stored as map.mmd-out/cache/{kind}/{hash}.json (AST entries
    under the per-version subdirectory, see :func:`cache_dir`).

    AST entries written by other map_mmd versions — including the legacy
...
def save_cached(path: Path, result: dict, root: Path = Path("."), kind: str = "ast") -> None:
    ...
    Stores as map.mmd-out/cache/{kind}/{hash}.json where hash = SHA256 of current file contents.
...
def cached_files(root: Path = Path(".")) -> set[str]:
    ...
    base = Path(root).resolve() / _MAP_MMD_OUT / "cache"
    hashes: set[str] = set()
    # Legacy flat entries
    if base.is_dir():
