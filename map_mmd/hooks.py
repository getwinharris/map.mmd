# git hook integration - install/uninstall map_mmd post-commit and post-checkout hooks
...
_HOOK_MARKER = "# map_mmd-hook-start"
_HOOK_MARKER_END = "# map_mmd-hook-end"
_CHECKOUT_MARKER = "# map_mmd-checkout-hook-start"
_CHECKOUT_MARKER_END = "# map_mmd-checkout-hook-end"

# __PINNED_PYTHON__ is replaced at install time with the absolute path of the
# Python interpreter that ran `map_mmd hook install`.  For uv-tool and pipx
...
# when the map_mmd launcher is not on PATH (common in GUI clients and CI).
MAP_MMD_PYTHON=""
_PINNED='__PINNED_PYTHON__'
if [ -n "$_PINNED" ] && [ -x "$_PINNED" ] && "$_PINNED" -c "import map_mmd" 2>/dev/null; then
    MAP_MMD_PYTHON="$_PINNED"
fi
# Second probe: read map.mmd-out/.map_mmd_python (written by the skill and
# CLI; survives uv-tool reinstalls and is the same source the README documents).
if [ -z "$MAP_MMD_PYTHON" ]; then
    _GFY_PYTHON_FILE="map.mmd-out/.map_mmd_python"
    if [ -f "$_GFY_PYTHON_FILE" ]; then
        ...
        if [ -n "$_FROM_FILE" ] && [ -x "$_FROM_FILE" ] && "$_FROM_FILE" -c "import map_mmd" 2>/dev/null; then
            MAP_MMD_PYTHON="$_FROM_FILE"
        fi
    fi
fi
# Third probe: resolve via the map_mmd launcher on PATH (shebang probe).
if [ -z "$MAP_MMD_PYTHON" ]; then
    MAP_MMD_BIN=$(command -v map_mmd 2>/dev/null)
    if [ -n "$MAP_MMD_BIN" ]; then
        case "$MAP_MMD_BIN" in
            *.exe) _SHEBANG="" ;;
            *)     _SHEBANG=$(head -1 "$MAP_MMD_BIN" | sed 's/^#![[:space:]]*//') ;;
        esac
        case "$_SHEBANG" in
            */env\\ *) MAP_MMD_PYTHON="${_SHEBANG#*/env }" ;;
            *)         MAP_MMD_PYTHON="$_SHEBANG" ;;
        esac
        # Allowlist: only keep characters valid in a filesystem path to prevent
        # injection if the shebang contains shell metacharacters.
        case "$MAP_MMD_PYTHON" in
            *[!a-zA-Z0-9/_.@-]*) MAP_MMD_PYTHON="" ;;
        esac
        if [ -n "$MAP_MMD_PYTHON" ] && ! "$MAP_MMD_PYTHON" -c "import map_mmd" 2>/dev/null; then
            MAP_MMD_PYTHON=""
        fi
    fi
fi
...
if [ -z "$MAP_MMD_PYTHON" ]; then
    if command -v python3 >/dev/null 2>&1 && python3 -c "import map_mmd" 2>/dev/null; then
        MAP_MMD_PYTHON="python3"
    elif command -v python >/dev/null 2>&1 && python -c "import map_mmd" 2>/dev/null; then
        MAP_MMD_PYTHON="python"
    else
        echo "[map_mmd hook] could not locate a Python with map_mmd installed. Add the map_mmd bin dir to PATH or re-run 'map_mmd hook install' from the env where map_mmd lives." >&2
        exit 0
    fi
fi
"""
...
changed_raw = os.environ.get('MAP_MMD_CHANGED', '')
changed = [Path(f.strip()) for f in changed_raw.strip().splitlines() if f.strip()]

if not changed:
    sys.exit(0)

print(f'[map_mmd hook] {len(changed)} file(s) changed - rebuilding graph...')

try:
    from map_mmd.watch import _rebuild_code, _apply_resource_limits
    _apply_resource_limits()
    _timeout = int(os.environ.get('MAP_MMD_REBUILD_TIMEOUT', '600'))
    if _timeout > 0 and hasattr(signal, 'SIGALRM'):
        signal.signal(signal.SIGALRM, lambda *_: (_ for _ in ()).throw(TimeoutError(f'map_mmd rebuild exceeded {_timeout}s')))
        signal.alarm(_timeout)
    _force = os.environ.get('MAP_MMD_FORCE', '').lower() in ('1', 'true', 'yes')
    _root = Path('.')
    _out = os.environ.get('MAP_MMD_OUT', 'map.mmd-out')
    _saved = Path(_out) / '.map_mmd_root'
    if _saved.exists():
    ...
    try:
        ...
        if _md.is_dir() and any(_md.glob('*.md')):
            from map_mmd.reflect import reflect as _reflect
    ...
    print(f'[map_mmd hook] {exc}')
    sys.exit(1)
except Exception as exc:
    print(f'[map_mmd hook] Rebuild failed: {exc}')
    sys.exit(1)
"""
...
try:
    ...
    print(f'[map_mmd] Rebuild failed: {exc}')
    sys.exit(1)
"""

# Cross-platform detached-launch shim (#1161). The hooks used to background the
# rebuild with `nohup "$MAP_MMD_PYTHON" -c "..." &`, but Git for Windows' bundled
...
# still returned 0, so the graph just went stale with no signal. map_mmd already
...
__REBUILD_BODY__
'''
_log = os.environ.get('MAP_MMD_REBUILD_LOG') or os.path.join(os.path.expanduser('~'), '.cache', 'map_mmd-rebuild.log')
try:
...
def _detached_launch(rebuild_body: str) -> str:
    ...
    Python process via ``$MAP_MMD_PYTHON``.
    ...
    The launcher writes the child's output to ``$MAP_MMD_REBUILD_LOG`` and
    ...
    return '"$MAP_MMD_PYTHON" -c "' + launcher + '"\n'


_HOOK_SCRIPT = """\
# map_mmd-hook-start
# Auto-rebuilds the knowledge graph after each commit (code files only, no LLM needed).
# Installed by: map_mmd hook install
...
# churn run-to-run. Pinning it makes map.mmd-out reproducible.
...
# Skip when only map.mmd-out/ artifacts changed (avoids rebuild loop when graph outputs are tracked in git)
_NON_GRAPH=$(echo "$CHANGED" | grep -v '^map.mmd-out/' || true)
if [ -z "$_NON_GRAPH" ]; then
    exit 0
fi

""" + _PYTHON_DETECT + """
export MAP_MMD_CHANGED="$CHANGED"
...
_MAP_MMD_LOG="${HOME}/.cache/map_mmd-rebuild.log"
mkdir -p "$(dirname "$_MAP_MMD_LOG")"
export MAP_MMD_REBUILD_LOG="$_MAP_MMD_LOG"
echo "[map_mmd hook] launching background rebuild (log: $_MAP_MMD_LOG)"
""" + _detached_launch(_REBUILD_BODY_COMMIT) + """# map_mmd-hook-end
"""


_CHECKOUT_SCRIPT = """\
# map_mmd-checkout-hook-start
...
# Only run if map.mmd-out/ exists (graph has been built before)
if [ ! -d "map.mmd-out" ]; then
    exit 0
fi
...
mkdir -p "$(dirname "$_MAP_MMD_LOG")"
export MAP_MMD_REBUILD_LOG="$_MAP_MMD_LOG"
echo "[map_mmd] Branch switched - launching background rebuild (log: $_MAP_MMD_LOG)"
""" + _detached_launch(_REBUILD_BODY_CHECKOUT) + """# map_mmd-checkout-hook-end
"""
...
def _hooks_dir(root: Path) -> Path:
    ...
    try:
        ...
        print(
            f"[map_mmd hooks] could not read core.hooksPath from "
...
def _uninstall_hook(hooks_dir: Path, name: str, marker: str, marker_end: str) -> str:
    """Remove map_mmd section from a git hook using start/end markers."""
    hook_path = hooks_dir / name
    if not hook_path.exists():
        return f"no {name} hook found - nothing to remove."
    content = hook_path.read_text(encoding="utf-8")
    if marker not in content:
        return f"map_mmd hook not found in {name} - nothing to remove."
    new_content = re.sub(
    ...
    return f"map_mmd removed from {name} at {hook_path} (other hook content preserved)"
...
def install(path: Path = Path(".")) -> str:
    """Install map_mmd post-commit and post-checkout hooks in the nearest git repo."""
    root = _git_root(path)
    if root is None:
    ...
    # Pin the current interpreter so the hook works even when the map_mmd
...
def uninstall(path: Path = Path(".")) -> str:
    """Remove map_mmd post-commit and post-checkout hooks."""
    root = _git_root(path)
    if root is None:
        raise RuntimeError(f"No git repository found at or above {path.resolve()}")
...
def status(path: Path = Path(".")) -> str:
    """Check if map_mmd hooks are installed."""
    root = _git_root(path)
    if root is None:
    ...
    def _check(name: str, marker: str) -> str:
        ...
        return "installed" if marker in p.read_text(encoding="utf-8") else "not installed (hook exists but map_mmd not found)"
