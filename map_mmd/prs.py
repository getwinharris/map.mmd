"""map_mmd prs — graph-aware PR dashboard.
...
Usage:
  map_mmd prs                   # dashboard of all open PRs
  map_mmd prs <number>          # deep dive on one PR
  map_mmd prs --triage          # Opus ranks your review queue
  map_mmd prs --worktrees       # show worktree → branch → PR mapping
  map_mmd prs --conflicts       # PRs sharing graph communities (merge-order risk)
  map_mmd prs --base <branch>   # filter to PRs targeting this base (default: v8)
"""
...
def render_dashboard(prs: list[PRInfo], base: str = "v8", show_wrong_base: bool = False) -> None:
    ...
    print()
    print(bold(f"  map_mmd prs  ·  base: {base}  ·  {len(actionable)} PRs"))
    print()
...
def _resolve_triage_backend() -> tuple[str, str]:
    """Return (backend, model) using MAP_MMD_TRIAGE_BACKEND or first available key."""
    from map_mmd.llm import BACKENDS, _get_backend_api_key, _default_model_for_backend

    explicit = os.environ.get("MAP_MMD_TRIAGE_BACKEND", "").strip()
    if explicit in BACKENDS:
        model = (os.environ.get("MAP_MMD_TRIAGE_MODEL")
...
def triage_with_opus(prs: list[PRInfo], base: str) -> None:
    try:
        from map_mmd.llm import BACKENDS, _get_backend_api_key
    except ImportError:
        print(red("  map_mmd.llm not available - cannot run triage."), file=sys.stderr)
        sys.exit(1)
