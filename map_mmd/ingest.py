...
from map_mmd.security import safe_fetch, safe_fetch_text, validate_url
...
def ingest(url: str, target_dir: Path, author: str | None = None, contributor: str | None = None) -> Path:
    """
    Fetch a URL and save it into target_dir as a map_mmd-ready file.
    ...
    try:
        ...
        if url_type == "youtube":
            from map_mmd.transcribe import download_audio
...
def save_query_result(
    ...
    Files are stored in memory_dir (typically map.mmd-out/memory/) with YAML frontmatter
    that map_mmd's extractor reads as node metadata. This closes the feedback loop:
    ...
    signals: they are written both to the frontmatter (so `map_mmd reflect` can
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fetch a URL into a map_mmd /raw folder")
