"""Query logging for map_mmd — append-only JSONL, fail-silent."""
...
def _log_path() -> Path | None:
    if os.environ.get("MAP_MMD_QUERY_LOG_DISABLE", "").lower() in ("1", "true", "yes"):
        return None
    override = os.environ.get("MAP_MMD_QUERY_LOG", "").strip()
    if override:
        return Path(override).expanduser()
    return Path.home() / ".cache" / "map_mmd-queries.log"


def _log_responses() -> bool:
    return os.environ.get("MAP_MMD_QUERY_LOG_RESPONSES", "").lower() in ("1", "true", "yes")
